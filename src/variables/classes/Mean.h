...
namespace OpenKalman
{
  ...
  Mean(V&&) -> Mean<typename MatrixTraits<V>::RowCoefficients, typename MatrixTraits<V>::BaseMatrix>;
...
///////////////////////////////////
  ...
  inline auto make_Mean(V&& arg) noexcept
  {
    using Coeffs = std::conditional_t<std::is_void_v<Coefficients>, Axes<MatrixTraits<V>::dimension>, Coefficients>;
    static_assert(MatrixTraits<V>::dimension == Coeffs::size);
    return Mean<Coeffs, std::decay_t<V>>(std::forward<V>(arg));
  }
