...
#ifdef __cpp_concepts
    template<typename Arg> requires is_ToEuclideanExpr_v<Arg> or Eigen_matrix<Arg>
#else
...
#ifdef __cpp_concepts
    template<typename C = Coefficients, typename Arg> requires Eigen_matrix<Arg> or is_FromEuclideanExpr_v<Arg>
#else
