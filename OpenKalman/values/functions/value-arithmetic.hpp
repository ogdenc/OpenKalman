...
namespace OpenKalman::values
{
  ...
  template<fixed Arg>
#else
  template<typename Arg, std::enable_if_t<fixed<Arg>, int> = 0>
#endif
...
#ifdef __cpp_concepts
  template<fixed Arg> requires requires(Arg arg) { -to_number(std::move(arg)); }
#else
  template<typename Arg, std::enable_if_t<fixed<Arg>, int> = 0, typename = std::void_t<decltype(-to_number(std::declval<Arg&&>()))>>
#endif
...
#ifdef __cpp_concepts
  template<value Arg1, detail::value_common_with<Arg1> Arg2> requires (fixed<Arg1> or fixed<Arg2>) and
    requires(Arg1 arg1, Arg2 arg2) { to_number(std::move(arg1)) + to_number(std::move(arg2)); }
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<detail::value_common_with<Arg1, Arg2> and (fixed<Arg1> or fixed<Arg2>) and
  ...
  template<value A, detail::value_common_with<A> B> requires (fixed<A> or fixed<B>) and
    requires(A a, B b) { to_number(std::move(a)) <=> to_number(std::move(b)); }
...
#else
  template<typename A, typename B, std::enable_if_t<detail::value_common_with<A, B> and (fixed<A> or fixed<B>), int> = 0,
    typename = std::void_t<decltype(to_number(std::declval<A>()) == to_number(std::declval<B>()))>>
  constexpr bool operator==(const A& a, const B& b)
  {
    using Common = detail::binary_common_type_t<A, B>;
    return static_cast<Common>(to_number(a)) == static_cast<Common>(to_number(b));
