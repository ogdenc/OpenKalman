...
namespace OpenKalman::values::internal
{
  ...
  constexpr std::tuple<T, T> log_scaling_gt(const T& x, const T& corr = T{0})
  {
    ...
    else if (x < T{0x1p+16}) return log_scaling_gt<T>(x * T{0x1p-4}, corr + T{4} * stdex::numbers::ln2_v<T>);
    else if (x < T{0x1p+64}) return log_scaling_gt<T>(x * T{0x1p-16}, corr + T{16} * stdex::numbers::ln2_v<T>);
    else return log_scaling_gt<T>(x * T{0x1p-64}, corr + T{64} * stdex::numbers::ln2_v<T>);
  }
  ...
  {
    if (x > T{0x1p-4}) return {x, corr};
    else if (x > T{0x1p-16}) return log_scaling_lt<T>(x * T{0x1p+4}, corr - T{4} * stdex::numbers::ln2_v<T>);
    else if (x > T{0x1p-64}) return log_scaling_lt<T>(x * T{0x1p+16}, corr - T{16} * stdex::numbers::ln2_v<T>);
    else return log_scaling_lt<T>(x * T{0x1p+64}, corr - T{64} * stdex::numbers::ln2_v<T>);
  }
