...
/**
 * \file
 * \brief Definition for \ref descriptor::internal::get_index_table.
 */
...
{
  /**
   * \brief Get a \ref internal::collection "collection" mapping each index of an \ref indexible vector
  ...
  get_index_table(Arg&& arg)
  {
    ...
    {
      auto ret = interface::coordinate_set_traits<std::decay_t<Arg>>::index_table(std::forward<Arg>(arg));
