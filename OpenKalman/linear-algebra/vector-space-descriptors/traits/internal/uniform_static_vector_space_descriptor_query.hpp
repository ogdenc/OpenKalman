...
namespace OpenKalman::internal
{
    ...
    struct uniform_static_vector_space_descriptor_query<C, std::enable_if_t<euclidean_vector_space_descriptor<C> and
      (dynamic_vector_space_descriptor<C> or atomic_static_vector_space_descriptor<C>) and (dimension_size_of_v<C> != 0)>>
#endif
      : std::true_type { using uniform_type = descriptors::Dimensions<1>; };


#ifdef __cpp_concepts
    template<typename C> requires (dimension_size_of_v<C> == 1)
    struct uniform_static_vector_space_descriptor_query<descriptors::StaticDescriptor<C>>
#else
    template<typename C>
    struct uniform_static_vector_space_descriptor_query<descriptors::StaticDescriptor<C>, std::enable_if_t<dimension_size_of_v<C> == 1>>
#endif
    ...
    template<atomic_static_vector_space_descriptor C, static_vector_space_descriptor...Cs> requires (dimension_size_of_v<C> == 1) and
      (sizeof...(Cs) > 0) and equivalent_to<C, typename uniform_static_vector_space_descriptor_query<descriptors::StaticDescriptor<Cs...>>::uniform_type>
    struct uniform_static_vector_space_descriptor_query<descriptors::StaticDescriptor<C, Cs...>>
#else
    template<typename C, typename...Cs>
    struct uniform_static_vector_space_descriptor_query<descriptors::StaticDescriptor<C, Cs...>, std::enable_if_t<
