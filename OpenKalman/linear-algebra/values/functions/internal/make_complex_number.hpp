...
namespace OpenKalman::internal
{
  ...
  constexpr value::complex_number auto
  make_complex_number(value::number auto&& re, value::number auto&& im = 0)
    requires (not value::complex_number<decltype(re)>) and std::same_as<std::decay_t<decltype(re)>, std::decay_t<decltype(im)>>
#else
  template<typename Re, typename Im, std::enable_if_t<value::number<Re> and (not value::complex_number<Re>) and
...
#endif
  {
    ...
    return interface::number_traits<R>::make_complex(std::forward<decltype(re)>(re), std::forward<decltype(im)>(im));
  }
...
#else
  template<typename T, typename Re, typename Im, std::enable_if_t<value::number<Re> and
    std::is_same_v<std::decay_t<Re>, std::decay_t<Im>>, int> = 0>
  constexpr auto make_complex_number(Re&& re, Im&& im)
#endif
  {
    return interface::number_traits<std::decay_t<T>>::make_complex(std::forward<decltype(re)>(re), std::forward<decltype(im)>(im));
  }
