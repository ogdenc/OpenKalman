...
namespace OpenKalman::interface
{
   ...
   */
#ifdef __cpp_concepts
  template<typename T> requires std::numeric_limits<std::decay_t<T>>::is_specialized
  struct number_traits<T>
#else
  template<typename T>
  struct number_traits<T, std::enable_if_t<std::numeric_limits<std::decay_t<T>>::is_specialized>>
#endif
  {
    static constexpr bool is_complex = false;
