...
namespace OpenKalman::interface
{
...
#endif
  {
    ...
    using library_base = Eigen3::EigenAdapterBase<Derived,
  ...
  public:
    ...
    to_native_matrix(Arg&& arg)
    {
      ...
      else if constexpr (not Eigen3::eigen_general<Arg> and directly_accessible<Arg> and std::is_lvalue_reference_v<Arg>)
      {
        ...
        if constexpr (layout_of_v<Arg> == data_layout::stride)
        {
        ...
        {
          constexpr auto l = layout_of_v<Arg> == data_layout::right ? Eigen::RowMajor : Eigen::ColMajor;
  ...
  private:
    ...
    extract_descriptors(Descriptors&& descriptors)
    {
      if constexpr (pattern_collection<Descriptors>)
      {
        ...
        static_assert(dim <= 2);
        if constexpr (dim == 0) return std::tuple {patterns::Axis{}, patterns::Axis{}};
        else if constexpr (dim == 1) return std::tuple {std::get<0>(std::forward<Descriptors>(descriptors)), patterns::Axis{}};
      ...
      else
      {
        ...
        if (it == e) return std::tuple {patterns::Axis{}, patterns::Axis{}};
        auto i = *it;
        if (++it == e) return std::tuple {i, patterns::Axis{}};
        auto j = *it;
  ...
  public:
...
#ifdef __cpp_concepts
    template<data_layout layout, values::number Scalar, patterns::euclidean_pattern_collection Ds>
#else
    template<data_layout layout, typename Scalar, typename Ds, std::enable_if_t<values::number<Scalar> and
      patterns::euclidean_pattern_collection<Ds>, int> = 0>
#endif
    static auto make_default(Ds&& ds)
    {
      ...
      if constexpr (pattern_collection<Ds> and collections::size_of_v<Ds> > 2)
      {
        ...
        if constexpr (fixed_pattern_collection<Ds>)
        {
          auto sizes = std::apply([](auto&&...d){
            return Eigen::Sizes<static_cast<std::ptrdiff_t>(patterns::dimension_of_v<decltype(d)>)...> {};
          }, std::forward<Ds>(ds));
      ...
      else
      {
        ...
        constexpr auto dim0 = patterns::dimension_of_v<decltype(d0)>;
        constexpr auto dim1 = patterns::dimension_of_v<decltype(d1)>;

        static_assert(layout != data_layout::right or dim0 == 1 or dim1 != 1,
          "Eigen does not allow creation of a row-major column vector.");
        static_assert(layout != data_layout::left or dim0 != 1 or dim1 == 1,
          "Eigen does not allow creation of a column-major row vector.");

        constexpr auto options =
          layout == data_layout::right or (layout == data_layout::none and dim0 == 1 and dim1 != 1) ?
...
#ifdef __cpp_concepts
    template<data_layout layout, writable Arg, std::convertible_to<scalar_type_of_t<Arg>> S, std::convertible_to<scalar_type_of_t<Arg>>...Ss>
      requires (layout == data_layout::right) or (layout == data_layout::left)
#else
    template<data_layout layout, typename Arg, typename S, typename...Ss, std::enable_if_t<writable<Arg> and
      (layout == data_layout::right or layout == data_layout::left) and
    ...
    {
      if constexpr (layout == data_layout::left) ((arg.transpose() << s), ... , ss);
      else ((arg << s), ... , ss);
    }


#ifdef __cpp_lib_ranges
    template<values::dynamic C, patterns::euclidean_pattern_collection Ds> requires
    ...
    template<typename C, typename Ds, std::enable_if_t<values::dynamic<C> and
      patterns::euclidean_pattern_collection<Ds> and
    ...
    make_constant(C&& c, Ds&& ds)
    {
      ...
      auto value = values::to_value_type(std::forward<C>(c));
...
#ifdef __cpp_concepts
    template<typename Scalar, patterns::euclidean_pattern_collection Ds> requires
      (collections::size_of_v<Ds> != stdex::dynamic_extent) and (collections::size_of_v<Ds> <= 2)
    static constexpr identity_matrix auto
#else
    template<typename Scalar, typename...Ds, std::enable_if_t<patterns::euclidean_pattern_collection<Ds> and
      (collections::size_of_v<Ds> != stdex::dynamic_extent) and (collections::size_of_v<Ds> <= 2), int> = 0>
    static constexpr auto
#endif
...
#ifdef __cpp_concepts
    template<triangle_type t, Eigen3::eigen_dense_general Arg> requires std::is_lvalue_reference_v<Arg> or
      (not std::is_lvalue_reference_v<typename Eigen::internal::ref_selector<std::decay_t<Arg>>::non_const_type>)
#else
    template<triangle_type t, typename Arg, std::enable_if_t<Eigen3::eigen_dense_general<Arg> and (std::is_lvalue_reference_v<Arg> or
...
#ifdef __cpp_concepts
    template<triangle_type t, Eigen3::eigen_SelfAdjointView A, Eigen3::eigen_general B> requires (not hermitian_matrix<A>) and
...
#ifdef __cpp_concepts
    template<triangle_type t, typename A, Eigen3::eigen_general B> requires
      ((diagonal_matrix<A> and internal::has_nested_vector<A>) or t == triangle_type::diagonal) and
      std::assignable_from<decltype(OpenKalman::diagonal_of(std::declval<A&&>())), decltype(OpenKalman::diagonal_of(std::declval<B&&>()))>
#else
    template<triangle_type t, typename A, typename B, std::enable_if_t<
      Eigen3::eigen_general<B> and ((diagonal_matrix<A> and internal::has_nested_vector<A>) or t == triangle_type::diagonal) and
    ...
    template<triangle_type t, typename A, Eigen3::eigen_general B> requires
      ...
      set_triangle_defined_for<T, t, decltype(OpenKalman::nested_object(std::declval<A>())), B&&>
#else
    template<triangle_type t, typename A, typename B, std::enable_if_t<Eigen3::eigen_general<B> and
      (Eigen3::eigen_MatrixWrapper<A> or Eigen3::eigen_ArrayWrapper<A>) and
      set_triangle_defined_for<T, t, decltype(OpenKalman::nested_object(std::declval<A>())), B&&>, int> = 0>
#endif
    static void
...
#ifdef __cpp_concepts
    template<triangle_type t, Eigen3::eigen_dense_general A, Eigen3::eigen_general B> requires
      (not Eigen3::eigen_MatrixWrapper<A>) and (not Eigen3::eigen_ArrayWrapper<A>) and
      writable<A&&> and (t != triangle_type::diagonal)
#else
    template<triangle_type t, typename A, typename B, std::enable_if_t<
      Eigen3::eigen_dense_general<A> and Eigen3::eigen_general<B> and
      (not Eigen3::eigen_MatrixWrapper<A>) and (not Eigen3::eigen_ArrayWrapper<A>) and
      writable<A&&> and (t != triangle_type::diagonal), int> = 0>
#endif
    static void
    set_triangle(A&& a, B&& b)
    {
      a.template triangularView<t == triangle_type::upper ? Eigen::Upper : Eigen::Lower>() = std::forward<B>(b);
    }
...
#ifdef __cpp_concepts
    template<Eigen3::eigen_TriangularView Arg> requires (triangle_type_of_v<Arg> == triangle_type::lower)
#else
    template<typename Arg, std::enable_if_t<Eigen3::eigen_TriangularView<Arg> and (triangle_type_of_v<Arg> == triangle_type::lower), int> = 0>
#endif
    static constexpr auto
    to_diagonal(Arg&& arg)
    {
  ...
  public:
...
#ifdef __cpp_concepts
    template<patterns::pattern...Ds, typename Operation, indexible...Args> requires
    ...
    template<typename...Ds, typename Operation, typename...Args, std::enable_if_t<sizeof...(Ds) <= 2 and sizeof...(Args) <= 3 and
      (patterns::pattern<Ds> and ...) and (indexible<Args> and ...) and
    ...
    n_ary_operation(const std::tuple<Ds...>& tup, Operation&& operation, Args&&...args)
    {
      ...
      {
        using P = dense_writable_matrix_t<T, data_layout::none, Scalar, std::tuple<Ds...>>;
  ...
  private:
    ...
    scalar_op_impl(Arg&& arg, S&& s, Op&& op)
    {
      ...
      auto c {Eigen::CwiseNullaryOp<ConstOp, PlainObjectType> {
        ...
        ConstOp {static_cast<Scalar>(values::to_value_type(s))}}};
  ...
  template<typename A, typename B, std::enable_if_t<(Eigen3::eigen_general<A> and Eigen3::eigen_general<B>), int> = 0>
    ...
    template<bool on_the_right, typename A, typename B, std::enable_if_t<writable<A> and
      (Eigen3::eigen_dense_general<A> or Eigen3::eigen_DiagonalMatrix<A> or (Eigen3::eigen_DiagonalWrapper<A> and diagonal_matrix<A> and internal::has_nested_vector<A, 0>)), int> = 0>
#endif
    static A&
...
#ifdef __cpp_concepts
    template<triangle_type tri, Eigen3::eigen_SelfAdjointView A>
#else
    template<triangle_type tri, typename A, std::enable_if_t<Eigen3::eigen_SelfAdjointView<A>, int> = 0>
#endif
    static constexpr auto
    cholesky_factor(A&& a)
    {
    ...
    rank_update_triangular(A&& a, U&& u, const Alpha alpha)
    {
      ...
      {
        static_assert(writable<A>);
        constexpr auto t = triangle == triangle_type::lower ? Eigen::Lower : Eigen::Upper;
        using Scalar = scalar_type_of_t<A>;
    ...
    solve(A&& a, B&& b)
    {
      ...
      {
        constexpr auto uplo = hermitian_adapter_type_of_v<A> == triangle_type::upper ? Eigen::Upper : Eigen::Lower;
