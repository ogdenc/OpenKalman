...
namespace OpenKalman::test
{
  ...
  using B1x = Eigen::Array<bool, 1, Eigen::Dynamic>;
  using Bx1 = Eigen::Array<bool, Eigen::Dynamic, 1>;
  using B22 = Eigen::Array<bool, 2, 2>;
  using B2x = Eigen::Array<bool, 2, Eigen::Dynamic>;
  using Bx2 = Eigen::Array<bool, Eigen::Dynamic, 2>;
  using Bxx = Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>;

  using B11_true = Eigen::CwiseNullaryOp<Eigen::internal::scalar_identity_op<bool>, B11>;
  using B1x_true = Eigen::CwiseNullaryOp<Eigen::internal::scalar_identity_op<bool>, B1x>;
  using Bx1_true = Eigen::CwiseNullaryOp<Eigen::internal::scalar_identity_op<bool>, Bx1>;
  using B11_false = decltype(not std::declval<B11_true>());
  using B1x_false = decltype(not std::declval<B1x_true>());
  using Bx1_false = decltype(not std::declval<Bx1_true>());
  using B22_true = decltype(std::declval<B11_true>().replicate<2,2>());
  using B2x_true = decltype(std::declval<B1x_true>().replicate<2,2>());
  using Bx2_true = decltype(std::declval<Bx1_true>().replicate<2,2>());
  using B22_false = decltype(std::declval<B11_false>().replicate<2,2>());
  using B2x_false = decltype(std::declval<B1x_false>().replicate<2,2>());
  using Bx2_false = decltype(std::declval<Bx1_false>().replicate<2,2>());
  using Bxx_true = Eigen::CwiseNullaryOp<Eigen::internal::scalar_identity_op<bool>, Bxx>;
