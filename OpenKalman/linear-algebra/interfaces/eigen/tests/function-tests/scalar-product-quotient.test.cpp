...
TEST(eigen3, scalar_product)
{
  ...
  static_assert(value::dynamic_scalar<constant_coefficient<decltype(scalar_product(std::declval<C22_2>(), std::declval<double>()))>>);
...
TEST(eigen3, scalar_quotient)
{
  ...
  // Constant / runtime value
  static_assert(value::dynamic_scalar<constant_coefficient<decltype(scalar_quotient(std::declval<C22_2>(), std::declval<double>()))>>);
