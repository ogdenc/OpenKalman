...
TEST(eigen3, rank_update_hermitian)
{
  // u is zero:

  const auto m22_93310 = make_dense_object_from<M22>(9, 3, 3, 10);
  M22 m22 = m22_93310;
  const auto z22 = M22::Identity() - M22::Identity(); static_assert(zero<decltype(z22)>);

  auto hl22 = Eigen::SelfAdjointView<M22, Eigen::Lower> {m22};
  static_assert(eigen_SelfAdjointView<decltype(rank_update_hermitian(hl22, z22, 4))>);
  static_assert(eigen_SelfAdjointView<decltype(rank_update_hermitian(std::as_const(hl22), z22, 4))>);
  EXPECT_TRUE(is_near(rank_update_hermitian(hl22, z22, 4), m22_93310));
  EXPECT_TRUE(is_near(m22, m22_93310));

  auto hu22 = Eigen::SelfAdjointView<M22, Eigen::Upper> {m22};
  static_assert(eigen_SelfAdjointView<decltype(rank_update_hermitian(hu22, z22, 4))>);
  static_assert(eigen_SelfAdjointView<decltype(rank_update_hermitian(std::as_const(hu22), z22, 4))>);
  ...
  EXPECT_TRUE(is_near(rank_update_hermitian(std::as_const(m11), m11_2, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(std::as_const(m11), M1x {m11_2}, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(M11 {m11}, Mx1 {m11_2}, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(M11 {m11}, Mxx {m11_2}, 4), m11_25));

  EXPECT_TRUE(is_near(rank_update_hermitian(std::as_const(m11), m14_1, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(std::as_const(m11), M1x {m14_1}, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(M11 {m11}, Mx4 {m14_1}, 4), m11_25));
  EXPECT_TRUE(is_near(rank_update_hermitian(M11 {m11}, Mxx {m14_1}, 4), m11_25));

  rank_update_hermitian(m11, m11_2, 4); EXPECT_TRUE(is_near(m11, m11_25));
  m11 = M11(9);
  rank_update_hermitian(m11, m14_1, 4); EXPECT_TRUE(is_near(m11, m11_25));

  // Other tests require creation of to_diagonal_adapter. See special_matrix tests.
}
...
TEST(eigen3, rank_update_triangular)
{
  ...
  const auto m22_3013 = make_dense_object_from<M22>(3, 0, 1, 3);
  const auto m22_3103 = make_dense_object_from<M22>(3, 1, 0, 3);
  const auto z22 = M22::Identity() - M22::Identity(); static_assert(zero<decltype(z22)>);
