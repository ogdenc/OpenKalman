 ...
 * \file
 * \internal
 * \brief definition of \ref hermitian_accessor (an mdspan accessor)
 */

#ifndef OPENKALMAN_HERMITIAN_ACCESSOR_HPP
#define OPENKALMAN_HERMITIAN_ACCESSOR_HPP
...
namespace OpenKalman::interface
{
  /**
   * \internal
   * \brief An accessor that returns either an element of a nested accessor or its conjugate.
  ...
  struct hermitian_accessor
  {
    ...
    using offset_policy = hermitian_accessor;
     ...
     */
    constexpr
    hermitian_accessor(NestedAccessor acc) : nested_accessor_(std::move(acc)) {}

    hermitian_accessor() = delete;
...
#endif
    constexpr hermitian_accessor(const hermitian_accessor<OtherNestedAccessor>& other) noexcept
    ...
    template<typename OtherNestedAccessor, std::enable_if_t<
      ...
      (not std::is_same_v<NestedAccessor, OtherNestedAccessor>), int> = 0>
#endif
    constexpr hermitian_accessor(hermitian_accessor<OtherNestedAccessor>&& other) noexcept
    ...
    access(data_handle_type p, std::size_t i) const noexcept
    {
      ...
      if (i < partition_offset) return static_cast<reference>(values::real(nested_accessor_.access(nested_p, i)));
      i -= partition_offset;
      if (i < partition_offset) return static_cast<reference>(nested_accessor_.access(std::move(nested_p), i));
      if constexpr (values::complex<reference>)
        return static_cast<reference>(values::conj(nested_accessor_.access(nested_p, i - partition_offset)));
      else
        return static_cast<reference>(nested_accessor_.access(nested_p, i - partition_offset));
    }
