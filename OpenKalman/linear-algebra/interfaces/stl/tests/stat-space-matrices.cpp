...
/**
 * \file
 * \brief Tests relating to \ref to_stat_space_adapter.
 */
...
#include "linear-algebra/functions/to_stat_space.hpp"
#include "linear-algebra/functions/from_stat_space.hpp"
...
namespace
{
  ...
  using M32 = stdex::mdspan<double, stdex::extents<std::size_t, 2, 3>>;
  ...
  using M3x = stdex::mdspan<double, stdex::extents<std::size_t, 3, stdex::dynamic_extent>>;
  ...
  template<typename T> using To = to_stat_space_adapter<T>;
...
TEST(stl_interfaces, to_stat_space_adapter_2D)
{
  ...
  A1 a1 {
    ...
    {pi/6, pi/3},
    {3, 4}};
  auto pc1 = std::tuple {std::tuple {Dimensions<1>{}, angle::Radians{}, Distance{}}, Polar<>{}};
  auto p1 = attach_patterns(a1, pc1);

  using SA1 = double[4][2];
  SA1 sa1 {
    {1, 2},
    {std::sqrt(3.)/2, 0.5}, {0.5, std::sqrt(3.)/2},
    {3, 4}};

  auto p1_to = to_stat_space(p1);
  static_assert(index_count_v<decltype(p1_to)> == 2);
  static_assert(index_dimension_of_v<decltype(p1_to), 0> == 4);
  static_assert(index_dimension_of_v<decltype(p1_to), 1> == 2);
  static_assert(index_dimension_of_v<decltype(get_mdspan(p1_to)), 0> == 4);
  static_assert(index_dimension_of_v<decltype(get_mdspan(p1_to)), 1> == 2);

  EXPECT_TRUE(is_near(p1_to, sa1));

  auto sa1_from = from_stat_space(sa1, pc1);
  static_assert(index_count_v<decltype(sa1_from)> == 2);
  static_assert(index_dimension_of_v<decltype(sa1_from), 0> == 3);
  static_assert(index_dimension_of_v<decltype(sa1_from), 1> == 2);
  static_assert(index_dimension_of_v<decltype(get_mdspan(sa1_from)), 0> == 3);
  static_assert(index_dimension_of_v<decltype(get_mdspan(sa1_from)), 1> == 2);
  static_assert(compares_with_pattern_collection<decltype(sa1_from), decltype(pc1), &stdex::is_eq, applicability::guaranteed>);

  EXPECT_TRUE(is_near(sa1_from, p1));

}

TEST(stl_interfaces, to_stat_space_adapter_3D)
{
  using namespace patterns;

  using A1 = double[3][2][4];
  A1 a1 {
    {{1, 2, 3, 4}, {5, 6, 7, 8}},
    {{pi/6, pi/3, 4*pi/3, 3*pi/2}, {pi/4, pi/3, -3*pi, -7*pi/6}},
    {{9, 10, 11, 12}, {13, 14, 15, 16}}};
  auto pc1 = std::tuple {std::tuple {Dimensions<1>{}, angle::Radians{}, Distance{}}, Dimensions<2>{}, std::tuple{Polar<>{}, Dimensions<2>{}}};
  auto p1 = attach_patterns(a1, pc1);

  A1 w1 {
      {{1, 2, 3, 4}, {5, 6, 7, 8}},
      {{pi/6, pi/3, -2*pi/3, -pi/2}, {pi/4, pi/3, -pi, 5*pi/6}},
      {{9, 10, 11, 12}, {13, 14, 15, 16}}};
  auto pw1 = attach_patterns(w1, pc1);

  using SA1 = double[4][2][4];
  SA1 sa1 {
    {{1, 2, 3, 4}, {5, 6, 7, 8}},
    {{std::sqrt(3.)/2, 0.5, -0.5, 0}, {std::sqrt(2.)/2, 0.5, -1, -std::sqrt(3.)/2}},
    {{0.5, std::sqrt(3.)/2, -std::sqrt(3.)/2, -1}, {std::sqrt(2.)/2, std::sqrt(3.)/2, 0, 0.5}},
    {{9, 10, 11, 12}, {13, 14, 15, 16}}};

  auto p1_to = to_stat_space(p1);
  static_assert(index_count_v<decltype(p1_to)> == 3);
  static_assert(index_dimension_of_v<decltype(p1_to), 0> == 4);
  static_assert(index_dimension_of_v<decltype(p1_to), 1> == 2);
  static_assert(index_dimension_of_v<decltype(p1_to), 2> == 4);
  static_assert(index_dimension_of_v<decltype(get_mdspan(p1_to)), 0> == 4);
  static_assert(index_dimension_of_v<decltype(get_mdspan(p1_to)), 1> == 2);
  static_assert(index_dimension_of_v<decltype(get_mdspan(p1_to)), 2> == 4);

  EXPECT_TRUE(is_near(p1_to, sa1));
  EXPECT_TRUE(is_near(to_stat_space(pw1), sa1));

  auto sa1_from = from_stat_space(sa1, pc1);
  static_assert(index_count_v<decltype(sa1_from)> == 3);
  static_assert(index_dimension_of_v<decltype(sa1_from), 0> == 3);
  static_assert(index_dimension_of_v<decltype(sa1_from), 1> == 2);
  static_assert(index_dimension_of_v<decltype(sa1_from), 2> == 4);
  static_assert(index_dimension_of_v<decltype(get_mdspan(sa1_from)), 0> == 3);
  static_assert(index_dimension_of_v<decltype(get_mdspan(sa1_from)), 1> == 2);
  static_assert(index_dimension_of_v<decltype(get_mdspan(sa1_from)), 2> == 4);
  static_assert(compares_with_pattern_collection<decltype(sa1_from), decltype(pc1), &stdex::is_eq, applicability::guaranteed>);

  EXPECT_TRUE(is_near(sa1_from, pw1));
  EXPECT_TRUE(is_near(pw1, from_stat_space(to_stat_space(pw1), pc1)));
  EXPECT_TRUE(is_near(pw1, from_stat_space(to_stat_space(p1), pc1)));
...
TEST(stl_interfaces, stat_space_adapter_dynamic_properties)
{
