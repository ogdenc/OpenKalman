 ...
 * \file
 * \internal
 * \brief definition of \ref triangular_accessor (an mdspan accessor)
 */

#ifndef OPENKALMAN_TRIANGULAR_ACCESSOR_HPP
#define OPENKALMAN_TRIANGULAR_ACCESSOR_HPP
...
namespace OpenKalman::interface
{
  /**
   * \internal
   * \brief An accessor that returns either an element of a nested accessor or 0.
  ...
  struct triangular_accessor
  {
    ...
    using offset_policy = triangular_accessor;
     ...
     */
    constexpr
    triangular_accessor(NestedAccessor acc) : nested_accessor_(std::move(acc)) {}

    triangular_accessor() = delete;
...
#endif
    constexpr triangular_accessor(const triangular_accessor<OtherNestedAccessor>& other) noexcept
    ...
    template<typename OtherNestedAccessor, std::enable_if_t<
      ...
      (not std::is_same_v<NestedAccessor, OtherNestedAccessor>), int> = 0>
#endif
    constexpr triangular_accessor(triangular_accessor<OtherNestedAccessor>&& other) noexcept
    ...
    access(data_handle_type p, std::size_t i) const noexcept
    {
      ...
      if (i < partition_offset) return static_cast<reference>(nested_accessor_.access(nested_p, i));
      i -= partition_offset;
