...
namespace OpenKalman
{
  ...
  template<indexible T, Layout layout = Layout::none, value::number Scalar = scalar_type_of_t<T>, vector_space_descriptor_collection Descriptors>
  ...
  template<typename T, Layout layout = Layout::none, typename Scalar = scalar_type_of_t<T>, typename Descriptors, std::enable_if_t<
    indexible<T> and value::number<Scalar> and vector_space_descriptor_collection<D> and (layout != Layout::stride) and
  ...
  /**
   * \overload
   * \brief \ref vector_space_descriptor object are specified as parameters.
   */
#ifdef __cpp_concepts
  ...
  template<typename T, Layout layout = Layout::none, typename Scalar = scalar_type_of_t<T>, typename...Ds, std::enable_if_t<
    indexible<T> and value::number<Scalar> and (... and vector_space_descriptor<Ds>) and (layout != Layout::stride) and
