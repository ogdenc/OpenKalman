...
namespace OpenKalman
{
   ...
   */
#ifdef __cpp_concepts
  template<indexible T, Layout layout = Layout::none, value::number Scalar = scalar_type_of_t<T>, std::convertible_to<const Scalar> ... Args>
  ...
  template<typename T, Layout layout = Layout::none, typename Scalar = scalar_type_of_t<T>, typename ... Args, std::enable_if_t<
    indexible<T> and value::number<Scalar> and (std::is_convertible_v<Args, const Scalar> and ...) and
