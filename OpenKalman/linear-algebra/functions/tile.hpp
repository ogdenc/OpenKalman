...
namespace OpenKalman
{
   ...
   * \tparam Ds A set of \ref patterns::pattern for the resulting matrix or tensor.
   * \tparam Block The first block
   * \tparam Blocks Subsequent blocks
   */
#ifdef __cpp_concepts
  template<patterns::pattern...Ds, indexible Block, indexible...Blocks>
  ...
  template<typename...Ds, typename Block, typename...Blocks, std::enable_if_t<
    (patterns::pattern<Ds> and ...) and (indexible<Block> and ... and indexible<Blocks>) and
  ...
  constexpr decltype(auto) tile(const std::tuple<Ds...>& ds_tuple, Block&& block, Blocks&&...blocks)
  {
    ...
    else
    {
      ...
      auto current_position = std::tuple{(patterns::pattern<Ds> ? std::size_t(0) : std::size_t(-1))...};
