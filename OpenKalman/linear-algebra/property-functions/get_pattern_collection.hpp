...
namespace OpenKalman
{
  ...
  template<interface::get_pattern_collection_defined_for T>
  constexpr coordinates::pattern auto
#else
  template<typename T, std::enable_if_t<interface::get_pattern_collection_defined_for<T>, int> = 0>
  constexpr auto
#endif
  get_pattern_collection(const T& t)
  {
    return stdcompat::invoke(interface::indexible_object_traits<T>::get_pattern_collection, t);
  }
