...
namespace OpenKalman::coordinate::internal
{
  /**
   * \internal
   * \brief Return the largest \ref coordinate::pattern
  ...
  template<value::number Scalar, pattern V, pattern...Vs>
  constexpr coordinate::pattern decltype(auto)
#else
  template<typename Scalar, typename V, typename...Vs, std::enable_if_t<
    value::number<Scalar> and (pattern<V> and ... and pattern<Vs>), int> = 0>
  ...
  largest_vector_space_descriptor(V&& v, Vs&&...vs)
  {
    ...
    else
    {
      ...
      {
        return coordinate::Dimensions {std::max<std::size_t>(get_size(v), get_size(tail))};
      }
      else
      {
