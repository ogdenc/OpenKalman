 ...
 * \file
 * \internal
 * \brief Definition for \ref internal::largest_pattern function.
 */

#ifndef OPENKALMAN_LARGEST_PATTERN_HPP
#define OPENKALMAN_LARGEST_PATTERN_HPP
...
namespace OpenKalman::coordinates::internal
{
  namespace details
  {
    template<std::size_t i = 0, typename P,
      ...
      typename Max = std::integral_constant<std::size_t, 0>>
    constexpr auto
    largest_pattern_fixed(const P& p, MI mi = {}, Max max = {})
    {
  ...
  /**
   * \internal
   * \brief Return an index to the largest \ref pattern within a \ref pattern_collection
   * \details If there are multiple candidates, it will choose the earliest-listed one.
   * \returns an index to the maximum value
   */
  ...
  largest_pattern(const P& p)
  {
    if constexpr (collections::size_of_v<P> == dynamic_size)
    {
      ...
      auto it = std::ranges::max_element(pr, fn);
#else
      auto it = std::max_element(pr.begin(), pr.end(), fn);
#endif
      return static_cast<std::size_t>(std::distance(pr.begin(), it));
    }
    else
    {
      return details::largest_pattern_fixed(p);
    }
  }
