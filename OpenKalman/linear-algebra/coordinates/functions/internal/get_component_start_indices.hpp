...
/**
 * \file
 * \brief Definition for \ref coordinates::internal::get_component_start_indices.
 */

#ifndef OPENKALMAN_DESCRIPTORS_GET_COMPONENT_START_INDICES_HPP
#define OPENKALMAN_DESCRIPTORS_GET_COMPONENT_START_INDICES_HPP
...
namespace OpenKalman::coordinates::internal
{
  namespace detail
  {
    ...
    static constexpr auto component_start_indices_tuple(const Tup& tup, CurrLoc currloc = {}, Locs...locs)
    {
      if constexpr (i < std::tuple_size_v<Tup>)
      {
        auto next_loc = values::operation {std::plus{}, currloc, get_descriptor_dimension(std::get<i>(tup))};
        return component_start_indices_tuple<i + 1>(tup, std::move(next_loc), std::move(locs)..., std::move(currloc));
      }
  ...
  get_component_start_indices(Arg&& arg)
  {
    ...
    {
      return detail::component_start_indices_tuple(arg);
    }
    else
    {
      ...
      {
        indices[i++] = loc;
        loc += get_descriptor_dimension(c);
      }
      return indices;
    }
  }
...
#endif //OPENKALMAN_DESCRIPTORS_GET_COMPONENT_START_INDICES_HPP
