...
namespace OpenKalman::coordinates::internal
{
  ...
  template<pattern_collection P> requires values::fixed_number_compares_with<collections::size_of<P>, 0, std::greater<>>
  constexpr pattern decltype(auto)
#else
  template<typename P, std::enable_if_t<values::fixed_number_compares_with<collections::size_of<P>, 0, std::greater<>>, int> = 0>
  constexpr decltype(auto)
#endif
