...
namespace OpenKalman::coordinate
{
  ...
  template<pattern T, value::value X, value::index L>
  constexpr void
  ...
  template<typename T, typename Setter, typename Getter, typename X, typename L, std::enable_if_t<
    pattern<T> and value::value<X> and value::index<L> and
...
#endif
  {
    ...
    else // if constexpr (descriptor_collection<T>)
    {
      ...
      auto new_s = [&s, start_i](const Scalar& x, auto i) { s(x, value::operation {std::plus{}, start_i, i}); };
