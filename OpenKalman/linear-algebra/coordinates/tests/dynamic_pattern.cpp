...
TEST(coordinates, Any)
{
  ...
  static_assert(size_of_v<Any<double>> == dynamic_size);
  static_assert(euclidean_size_of_v<Any<float>> == dynamic_size);
...
TEST(coordinates, dynamic_pattern_traits)
{
  ...
  static_assert(size_of_v<std::tuple<Axis, angle::Degrees, Dimensions<>>> == dynamic_size);
  static_assert(size_of_v<std::tuple<Axis, angle::Degrees, Dimensions<3>>> != dynamic_size);
  static_assert(euclidean_size_of_v<std::tuple<Axis, angle::Degrees, Dimensions<>>> == dynamic_size);
  static_assert(euclidean_size_of_v<std::tuple<Axis, angle::Degrees, Dimensions<3>>> != dynamic_size);
}


TEST(coordinates, dynamic_pattern_functions)
{
  EXPECT_EQ(get_size(std::tuple{4u}), 4u);
  EXPECT_EQ(get_size(std::tuple{Dimensions{0}}), 0u);
  EXPECT_EQ(get_size(std::tuple{Dimensions{5}}), 5u);
  EXPECT_EQ(get_size(std::vector{Axis{}}), 1u);
  EXPECT_EQ(get_size(std::vector{angle::Degrees{}}), 1u);
  EXPECT_EQ(get_euclidean_size(std::vector{angle::Degrees{}}), 2u);
  EXPECT_EQ(get_size(std::vector{Dimensions<5>{}}), 5u);
  EXPECT_EQ(get_size(std::vector{Dimensions{5}}), 5u);
  EXPECT_EQ(get_size(std::vector{Polar{}}), 2u);
  EXPECT_EQ(get_size(std::vector{Polar<Distance, angle::PositiveRadians>{}}), 2u);
  EXPECT_EQ(get_euclidean_size(std::vector{Polar<angle::PositiveDegrees, Distance>{}}), 3u);
  EXPECT_EQ(get_size(std::vector{Spherical<Distance, inclination::Radians, angle::PositiveDegrees>{}}), 3u);
  EXPECT_EQ(get_euclidean_size(std::vector{Spherical<inclination::Radians, Distance, angle::PositiveDegrees>{}}), 4u);

  EXPECT_EQ(get_size(std::tuple{Dimensions{5}, Dimensions<1>{}, angle::Degrees{}}), 7u);
  EXPECT_EQ(get_size(std::tuple{Dimensions{5}, std::integral_constant<unsigned, 1>{}, angle::Degrees{}}), 7u);
  EXPECT_EQ(get_euclidean_size(std::tuple{Axis{}, Dimensions{5}, angle::Degrees{}}), 8u);
  EXPECT_EQ(get_size(std::tuple{Axis{}, inclination::Radians{}, angle::Degrees{}, Dimensions{5}}), 8u);
  EXPECT_EQ(get_euclidean_size(std::tuple{Dimensions{5}, Axis{}, inclination::Radians{}, angle::Degrees{}}), 10u);

  EXPECT_EQ(get_euclidean_size(std::array{Any{Dimensions{5}}, Any{Axis{}}, Any{inclination::Radians{}}, Any{angle::Degrees{}}}), 10u);
