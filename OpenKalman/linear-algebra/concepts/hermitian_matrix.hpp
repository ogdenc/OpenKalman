...
namespace OpenKalman
{
  ...
  namespace detail
  {
    ...
    template<typename T>
    struct is_inferred_hermitian_matrix<T, std::enable_if_t<not value::complex<typename scalar_type_of<T>::type> or
      value::not_complex<constant_coefficient<T>> or value::not_complex<constant_diagonal_coefficient<T>>>>
      : std::true_type {};
  };
#endif
  ...
  concept hermitian_matrix = indexible<T> and square_shaped<T, b> and
    (interface::indexible_object_traits<std::decay_t<T>>::is_hermitian or
      ...
      (not value::complex<scalar_type_of_t<T>> or
        value::not_complex<constant_coefficient<T>> or value::not_complex<constant_diagonal_coefficient<T>>)));
#else
