 ...
 * \internal
 * \file
 * \brief A pass-through interface.
 */

#ifndef OPENKALMAN_PASS_THROUGH_INTERFACE_HPP
#define OPENKALMAN_PASS_THROUGH_INTERFACE_HPP
...
{
  /**
   * \brief Pass-through object traits.
   * \details This is a base class that simply passes through all object traits to a nested class interface.
   * The object must have a <code>nested_object</code> member function.
   */
  template<typename Object, typename Nested>
  struct pass_through_object_traits
  {
  private:

    using NestedTraits = object_traits<Nested>;
  ...
  public:
    ...
    {
      assert_valid_arg_type(t);
      return NestedTraits::get_constant(std::forward<T>(t).nested_object());
    };
   ...
   */
  template<typename Object, typename Nested>
  struct pass_through_library_interface
  {
  private:
  ...
  public:
    ...
    {
      assert_valid_arg_type(t);
      OpenKalman::copy_from(t.nested_object(), std::forward<decltype(other)>(other));
    };
...
#ifdef __cpp_concepts
    template<patterns::pattern...Vs, typename Operation> requires
...
#ifdef __cpp_concepts
    template<patterns::pattern...Vs, typename Operation, indexible Arg, indexible...Args> requires
