...
namespace
{
  ...
  using To23 = to_stat_space_adapter<Car, M23>;
  using To32 = to_stat_space_adapter<Cara, M32>;
  using To02 = to_stat_space_adapter<DynamicTypedIndex, Mx2>;
  ...
  template<typename C, typename T> using To = to_stat_space_adapter<C, T>;
  
}


TEST(special_matrices, to_stat_space_adapter_static_checks)
{
...
TEST(special_matrices, to_stat_space_adapter_class)
{
  M42 m;
  ...
  EXPECT_TRUE(is_near(make_identity_matrix_like<to_stat_space_adapter<Dimensions<2>, eigen_matrix_t<double, 2, 2>>>(), eigen_matrix_t<double, 2, 2>::Identity()));
}


TEST(special_matrices, to_stat_space_adapter_subscripts)
{
  ...
  to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 1>> e1 = {3, pi/4};
  EXPECT_EQ(e1[0], 3);
  EXPECT_NEAR(e1(1), std::sqrt(2.)/2, 1e-6);
  EXPECT_NEAR(e1(2), std::sqrt(2.)/2, 1e-6);
  to_stat_space_adapter<Dimensions<2>, eigen_matrix_t<double, 2, 2>> e2 = {1, 2, 3, 4};
  e2(0,0) = 5;
  EXPECT_EQ(e2(0, 0), 5);
  e2(0,1) = 6;
  EXPECT_EQ(e2(0, 1), 6);
  ...
  EXPECT_NEAR((to_stat_space_adapter<Cara, eigen_matrix_t<double, 3, 1>>{1, pi/6, 3})(1), std::sqrt(3)/2, 1e-6);
  EXPECT_NEAR((to_stat_space_adapter<Cara, eigen_matrix_t<double, 3, 1>>{1, pi/6, 3})(2), 0.5, 1e-6);
...
TEST(special_matrices, to_stat_space_adapter_traits)
{
  ...
  static_assert(not from_stat_space_expr<decltype(To32 {1, 2, pi/6, pi/3, 3, 4})>);
...
TEST(special_matrices, to_stat_space_adapter_overloads)
{
  ...
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(m23), m23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(m2x_3), m23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(mx3_2), m23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(mxx_23), m23));

  auto m33_to_ra = make_dense_writable_matrix_from<M33>(std::cos(1.), std::cos(2.), std::cos(3.), std::sin(1.), std::sin(2.), std::sin(3.), 4, 5, 6);

  EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis>>(m23), m33_to_ra));
  //EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis>>(m2x_3), m33_to_ra)); \todo
  //EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis>>(mx3_2), m33_to_ra));
  //EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis>>(mxx_23), m33_to_ra));
  ...
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<3>>(c534), c534));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<3>>(c530_4), c534));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<3>>(c504_3), c534));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<3>>(c500_34), c534));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {Dimensions<3>{}}, c534), c534));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {Dimensions<3>{}}, c530_4), c534));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {Dimensions<3>{}}, c504_3), c534));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {Dimensions<3>{}}, c500_34), c534));
  ...
  EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis, Axis>>(c534), m44_to_raa));
  EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis, Axis>>(c530_4), m44_to_raa));
  EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis, Axis>>(c504_3), m44_to_raa));
  EXPECT_TRUE(is_near(to_stat_space<std::tuple<angle::Radians, Axis, Axis>>(c500_34), m44_to_raa));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c534), m44_to_raa));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c530_4), m44_to_raa));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c504_3), m44_to_raa));
  //EXPECT_TRUE(is_near(to_stat_space(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c500_34), m44_to_raa));
  ...
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(z23), z23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(z20_3), z23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(z03_2), z23));
  EXPECT_TRUE(is_near(to_stat_space<Dimensions<2>>(z00_23), z23));

  auto z33 = zero_adapter<eigen_matrix_t<double, 3, 3>> {};

  EXPECT_TRUE(is_near(to_stat_space<Axis, angle::Radians>(z23), z33));
  EXPECT_TRUE(is_near(to_stat_space<Axis, angle::Radians>(z20_3), z33));
  EXPECT_TRUE(is_near(to_stat_space<Axis, angle::Radians>(z03_2), z33));
  EXPECT_TRUE(is_near(to_stat_space<Axis, angle::Radians>(z00_23), z33));

  EXPECT_TRUE(is_near(from_stat_space(To32 {1, 2, 2*pi + pi/6, -4*pi + pi/3, 3, 4}), mat3(1, 2, pi/6, pi/3, 3, 4)));
  EXPECT_TRUE(is_near(from_stat_space<Cara>(To32 {1, 2, 2*pi + pi/6, -4*pi + pi/3, 3, 4}), mat3(1, 2, pi/6, pi/3, 3, 4)));

  EXPECT_TRUE(is_near(to_diagonal(to_stat_space_adapter<Cara, eigen_matrix_t<double, 3, 1>>{1, pi/6, 3}), to_diagonal_adapter {1., std::sqrt(3)/2, 0.5, 3}));
  ...
  for (int i=0; i<100; i++)
  {
    m = (m * i + from_stat_space(randomize<To32>(N {1.0, 0.3}))) / (i + 1);
  }
...
TEST(special_matrices, to_stat_space_adapter_blocks)
{
  EXPECT_TRUE(is_near(concatenate_vertical(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {1., 2, 3, pi/6, pi/3, pi/4},
    to_stat_space_adapter<Cra, eigen_matrix_t<double, 2, 3>> {pi/4, pi/3, pi/6, 4, 5, 6}),
  ...
  EXPECT_TRUE(is_near(concatenate_horizontal(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {1., 2, 3, pi/6, pi/3, pi/4},
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {4., 5, 6, pi/4, pi/3, pi/6}),
  ...
  EXPECT_TRUE(is_near(split_vertical(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 2>> {1., 2, pi/6, pi/3}), std::tuple {}));
  EXPECT_TRUE(is_near(split_vertical<3, 3>(
    to_stat_space_adapter<std::tuple<Axis, angle::Radians, angle::Radians, Axis>, eigen_matrix_t<double, 4, 3>> {
      1., 2, 3,
      pi/6, pi/3, pi/4,
      pi/4, pi/3, pi/6,
      4, 5, 6}),
    std::tuple {make_eigen_matrix<double, 3, 3>(
      ...
      0.5, std::sqrt(3)/2, std::sqrt(2)/2),
               make_eigen_matrix<double, 3, 3>(
                 ...
                 std::sqrt(2)/2, std::sqrt(3)/2, 0.5,
                 4, 5, 6)}));
  auto a1 = to_stat_space_adapter<std::tuple<Polar<>, angle::Radians, Axis>, eigen_matrix_t<double, 4, 3>> {
    1., 2, 3,
    pi/6, pi/3, pi/4,
    pi/4, pi/3, pi/6,
    4, 5, 6};
  ...
  EXPECT_TRUE(is_near(split_horizontal<3, 3>(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 6>> {
      1., 2, 3, 4, 5, 6,
  ...
  EXPECT_TRUE(is_near(split_horizontal<3, 2>(
    to_stat_space_adapter<Polar<>, eigen_matrix_t<double, 2, 6>> {
      1., 2, 3, 4, 5, 6,
      pi/6, pi/3, pi/4, pi/4, pi/3, pi/6}),
    std::tuple {
    make_eigen_matrix<double, 3, 3>(
  ...
  EXPECT_TRUE(is_near(split_diagonal<Axis, angle::Radians>(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {
      1., 2, 3,
      pi/6, pi/3, pi/4}),
  ...
  EXPECT_TRUE(is_near(split_diagonal<1, 2>(
    to_stat_space_adapter<Polar<>, eigen_matrix_t<double, 2, 3>> {
      1., 2, 3,
      pi/6, pi/3, pi/4}),
    std::tuple {make_eigen_matrix<double, 1, 1>(1),
               make_eigen_matrix<double, 2, 2>(
  ...
  EXPECT_TRUE(is_near(column(
    to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {1., 2, 3, pi/6, pi/3, pi/4}, 2),
  ...
  auto b = to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {1., 2, 3, pi/6, pi/3, pi/4};
  EXPECT_TRUE(is_near(apply_columnwise([](auto& col){ col *= 3; }, b),
    make_eigen_matrix<double, 3, 3>(
      3, 6, 9,
      std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
      0.5, std::sqrt(3)/2, std::sqrt(2)/2)));
  b = to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 3>> {1., 2, 3, pi/6, pi/3, pi/4};
  ...
  EXPECT_TRUE(is_near(apply_columnwise<3>([](){ return to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 1>> {1., pi/6}; }),
    make_eigen_matrix<double, 3, 3>(
      1, 1, 1,
      std::sqrt(3)/2, std::sqrt(3)/2, std::sqrt(3)/2,
      0.5, 0.5, 0.5)));
  EXPECT_TRUE(is_near(apply_columnwise<3>([](std::size_t i){ return make_self_contained(to_stat_space_adapter<Car, eigen_matrix_t<double, 2, 1>> {1., pi/6} * (i + 1)); }),
    make_eigen_matrix<double, 3, 3>(
      1, 2, 3,
  ...
  EXPECT_TRUE(is_near(apply_rowwise<2>([](){ return to_stat_space_adapter<Axis, eigen_matrix_t<double, 1, 2>> {1, 2}; }),
    make_eigen_matrix<double, 2, 2>(1, 2, 1, 2)));
  EXPECT_TRUE(is_near(apply_rowwise<3>([](std::size_t i){ return make_self_contained(to_stat_space_adapter<Axis, eigen_matrix_t<double, 1, 2>> {1, 2} * (i + 1)); }),
...
TEST(special_matrices, to_stat_space_adapter_arithmetic)
{
  ...
  EXPECT_TRUE(is_near(To32 {1, 2, pi/6, pi/3, 3, 4} * to_diagonal_adapter {1., 2}, mat4(1, 4, std::sqrt(3)/2, 1, 0.5, std::sqrt(3), 3, 8)));
}


TEST(special_matrices, to_stat_space_adapter_references)
{
  M22 m, n;
  m << pi/6, pi/4, 1, 2;
  n << pi/4, pi/3, 3, 4;
  ...
  using To = to_stat_space_adapter<Cra, M22>;
  To x = To {m};
  to_stat_space_adapter<Cra, M22&> x_lvalue = x;
