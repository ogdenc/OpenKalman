...
TEST(special_matrices, ToEuclideanExpr_subscripts)
{
  ...
  ToEuclideanExpr<Dimensions<2>, eigen_matrix_t<double, 2, 2>> e2 = {1, 2, 3, 4};
  e2(0,0) = 5;
  EXPECT_EQ(e2(0, 0), 5);
  e2(0,1) = 6;
  EXPECT_EQ(e2(0, 1), 6);
...
TEST(special_matrices, ToEuclideanExpr_overloads)
{
  ...
  EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis>>(m23), m33_to_ra));
  //EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis>>(m2x_3), m33_to_ra)); \todo
  ...
  EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis, Axis>>(c534), m44_to_raa));
  EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis, Axis>>(c530_4), m44_to_raa));
  EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis, Axis>>(c504_3), m44_to_raa));
  EXPECT_TRUE(is_near(to_euclidean<std::tuple<angle::Radians, Axis, Axis>>(c500_34), m44_to_raa));
  //EXPECT_TRUE(is_near(to_euclidean(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c534), m44_to_raa));
  //EXPECT_TRUE(is_near(to_euclidean(DynamicTypedIndex {std::tuple<angle::Radians, Axis, Axis>{}}, c530_4), m44_to_raa));
  ...
  EXPECT_TRUE(is_near(to_euclidean<Axis, angle::Radians>(z03_2), z33));
  EXPECT_TRUE(is_near(to_euclidean<Axis, angle::Radians>(z00_23), z33));
...
TEST(special_matrices, ToEuclideanExpr_blocks)
{
  ...
  EXPECT_TRUE(is_near(split_vertical<3, 3>(
    ToEuclideanExpr<std::tuple<Axis, angle::Radians, angle::Radians, Axis>, eigen_matrix_t<double, 4, 3>> {
      1., 2, 3,
      pi/6, pi/3, pi/4,
      pi/4, pi/3, pi/6,
      4, 5, 6}),
    std::tuple {make_eigen_matrix<double, 3, 3>(
      ...
      0.5, std::sqrt(3)/2, std::sqrt(2)/2),
               make_eigen_matrix<double, 3, 3>(
                 ...
                 std::sqrt(2)/2, std::sqrt(3)/2, 0.5,
                 4, 5, 6)}));
  auto a1 = ToEuclideanExpr<std::tuple<Polar<>, angle::Radians, Axis>, eigen_matrix_t<double, 4, 3>> {
    1., 2, 3,
    pi/6, pi/3, pi/4,
    pi/4, pi/3, pi/6,
    4, 5, 6};
