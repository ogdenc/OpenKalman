...
namespace OpenKalman
{
  ...
  template<fixed_pattern RowCoefficients, typed_matrix_nestable NestedMatrix> requires
    (patterns::stat_dimension_of_v<RowCoefficients> == index_dimension_of_v<NestedMatrix, 0>) and
    ...
    RowCoefficients, Dimensions<index_dimension_of_v<NestedMatrix, 1>>>
  {
    ...
    static_assert(patterns::stat_dimension_of_v<RowCoefficients> == index_dimension_of_v<NestedMatrix, 0>);
  ...
  public:
    ...
    template<typed_matrix Arg> requires (not std::derived_from<std::decay_t<Arg>, EuclideanMean>) and
      (euclidean_transformed<Arg> or patterns::euclidean_pattern<RowCoefficients>) and
      compares_with<vector_space_descriptor_of_t<Arg, 0>, RowCoefficients> and
    ...
    template<typed_matrix Arg> requires
      compares_with<vector_space_descriptor_of_t<Arg, 0>, RowCoefficients> and
      compares_with<vector_space_descriptor_of_t<Arg, 1>, ColumnCoefficients> and
      (patterns::euclidean_pattern<RowCoefficients> or euclidean_transformed<Arg>)
#else
    template<typename Arg, std::enable_if_t<typed_matrix<Arg> and
      compares_with<vector_space_descriptor_of_t<Arg, 0>, RowCoefficients> and
      compares_with<vector_space_descriptor_of_t<Arg, 1>, ColumnCoefficients> and
      (patterns::euclidean_pattern<RowCoefficients> or euclidean_transformed<Arg>), int> = 0>
#endif
    auto& operator+=(Arg&& other)
    {
...
#ifdef __cpp_concepts
    template<distribution Arg> requires patterns::euclidean_pattern<RowCoefficients> and
      (compares_with<typename DistributionTraits<Arg>::StaticDescriptor, RowCoefficients>)
#else
    template<typename Arg, std::enable_if_t<distribution<Arg> and patterns::euclidean_pattern<RowCoefficients> and
  ...
  template<typed_matrix V> requires (not euclidean_transformed<V>) and has_untyped_index<V, 1> and
    (patterns::stat_dimension_of_v<vector_space_descriptor_of_t<V, 0>> == index_dimension_of_v<V, 0>)
#else
  template<typename V, std::enable_if_t<typed_matrix<V> and not euclidean_transformed<V> and has_untyped_index<V, 1> and
patterns::stat_dimension_of_v<vector_space_descriptor_of_t<V, 0>> == index_dimension_of_v<V, 0>, int> = 0>
#endif
  EuclideanMean(V&&)
  ...
  template<fixed_pattern StaticDescriptor, typed_matrix_nestable M> requires
    (patterns::stat_dimension_of_v<StaticDescriptor> == index_dimension_of_v<M, 0>)
#else
  template<typename StaticDescriptor, typename M, std::enable_if_t<fixed_pattern<StaticDescriptor> and
    typed_matrix_nestable<M> and (patterns::stat_dimension_of_v<StaticDescriptor> == index_dimension_of<M, 0>::value), int> = 0>
#endif
...
#ifdef __cpp_lib_concepts
      template<typename Arg> requires raw_data_defined_for<NestedMatrix> and patterns::euclidean_pattern<Coeffs>
#else
      template<typename Arg, std::enable_if_t<raw_data_defined_for<NestedMatrix> and patterns::euclidean_pattern<Coeffs>, int> = 0>
#endif
      static constexpr auto * const
      raw_data(Arg& arg) { return internal::raw_data(OpenKalman::nested_object(arg)); }


      static constexpr data_layout layout = patterns::euclidean_pattern<Coeffs> ? layout_of_v<NestedMatrix> : data_layout::none;
