...
namespace OpenKalman
{
  ...
  public:
...
#endif
    Mean(Arg&& arg) : Base {from_stat_space<RowCoefficients>(nested_object(std::forward<Arg>(arg)))} {}
    ...
    template<typename Arg, std::enable_if_t<euclidean_transformed<Arg> and
      compares_with<vector_space_descriptor_of_t<Arg, 0>, RowCoefficients> and
      std::is_assignable_v<std::add_lvalue_reference_t<NestedMatrix>, decltype(from_stat_space<RowCoefficients>(std::declval<nested_object_of_t<Arg>>()))>, int> = 0>
#endif
    auto& operator=(Arg&& other)
    {
      if constexpr (not zero<NestedMatrix> and not identity_matrix<NestedMatrix>)
...
#endif
  Mean(V&&) -> Mean<vector_space_descriptor_of_t<V, 0>, std::decay_t<decltype(
    from_stat_space<vector_space_descriptor_of_t<V, 0>>(nested_object(std::forward<V>(std::declval<V>()))))>>;
  ...
  inline auto make_mean(Arg&& arg)
  {
    ...
    if constexpr(euclidean_transformed<Arg>)
      return make_mean<C>(nested_object(from_stat_space<C>(std::forward<Arg>(arg))));
    else
