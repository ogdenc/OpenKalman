...
namespace OpenKalman
{
  ...
  private:
    ...
    sigma_points_impl(const D& d, const Ds&...ds)
    {
      ...
      const auto delta = make_vector_space_adapter(to_dense_object(square_root(gamma_L * covariance_of(d))), Coeffs{}, Dimensions<dim_i>{});

      if constexpr(1 + frame_size == points_count)
      {
        // | 0 | delta | -delta |
        static_assert(sizeof...(ds) == 0);
        using M0 = dense_writable_matrix_t<M, data_layout::none, Scalar, std::tuple<Coeffs, Axis>>;
