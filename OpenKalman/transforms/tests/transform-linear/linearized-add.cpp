...
TEST(transform_linear, linearized_additive1)
{
  ...
  const GaussianDistribution noise0 {make_zero_matrix_like<M2>(), make_identity_matrix_like<Mat2>()};
  ...
  //
  const GaussianDistribution input1 {M2(1, 2), make_covariance<TriangleType::lower>(make_identity_matrix_like<Mat2>())};
  const GaussianDistribution noise1 {make_zero_matrix_like<M2>(), make_covariance<TriangleType::lower>(make_identity_matrix_like<Mat2>())};
