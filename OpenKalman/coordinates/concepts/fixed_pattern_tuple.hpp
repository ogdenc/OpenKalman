...
namespace OpenKalman::coordinates
{
  ...
  namespace detail
  {
    ...
    template<typename T>
    struct is_fixed_pattern_tuple<T, std::enable_if_t<
      collections::uniformly_gettable<T> and stdcompat::default_initializable<std::decay_t<T>>>>
      : std::bool_constant<is_fixed_pattern_tuple_impl<T>(std::make_index_sequence<collections::size_of_v<T>>{})> {};
  ...
  concept fixed_pattern_tuple =
    pattern_tuple<T> and std::default_initializable<std::decay_t<T>> and
    []<std::size_t...Ix>(std::index_sequence<Ix...>)
      { return (... and fixed_pattern<collections::collection_element_t<Ix, std::decay_t<T>>>); }
