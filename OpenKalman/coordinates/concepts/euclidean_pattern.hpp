...
namespace OpenKalman::coordinates
{
  ...
  namespace detail
  {
    ...
    template<typename T>
    struct euclidean_pattern_impl<T, std::enable_if_t<values::fixed_value_of<decltype(coordinates::get_is_euclidean(std::declval<T>()))>::value>>
      : std::true_type {};
  }
#endif
  ...
  concept euclidean_pattern = pattern<T> and
    values::fixed_value_of<decltype(coordinates::get_is_euclidean(std::declval<T>()))>::value;
#else
  template<typename T>
