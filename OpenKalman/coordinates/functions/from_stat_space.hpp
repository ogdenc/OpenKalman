  ...
  from_stat_space(const T& t, R&& stat_data_view)
  {
    ...
    else
    {
      ...
      if constexpr (std::is_same_v<U, T>)
        return stdcompat::invoke(Traits::from_stat_space, t, std::forward<R>(stat_data_view));
      else
        return stdcompat::invoke(Traits::from_stat_space, t.get(), std::forward<R>(stat_data_view));
    }
  }
