...
namespace OpenKalman::coordinates
{
  namespace detail
  {
    ...
    struct range_value_has_fixed_dimension : std::false_type {};

    template<typename T>
    struct range_value_has_fixed_dimension<T, std::enable_if_t<stdcompat::ranges::range<T>>>
      : std::bool_constant<values::fixed_value_compares_with<decltype(internal::get_descriptor_dimension(
    ...
    static constexpr auto get_dimension_fixed(const T& t)
    {
      if constexpr (i < collections::size_of_v<T>)
      {
        return values::operation(
          std::plus{},
          internal::get_descriptor_dimension(collections::get(t, std::integral_constant<std::size_t, i>{})),
  ...
  template<pattern Arg> requires descriptor<Arg> or collections::sized<Arg> or
    values::fixed_value_compares_with<decltype(internal::get_descriptor_dimension(std::declval<stdcompat::ranges::range_value_t<Arg>>())), 0>
  constexpr values::index auto
#else
  template<typename Arg, std::enable_if_t<descriptor<Arg> or
    (descriptor_collection<Arg> and (collections::sized<Arg> or detail::range_value_has_fixed_dimension<Arg>::value)), int> = 0>
  constexpr auto
#endif
  get_dimension(const Arg& arg)
  {
    ...
    {
      using C = decltype(internal::get_descriptor_dimension(std::declval<stdcompat::ranges::range_value_t<Arg>>()));
