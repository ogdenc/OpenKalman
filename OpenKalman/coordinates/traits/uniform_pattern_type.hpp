...
#ifndef OPENKALMAN_COORDINATES_UNIFORM_PATTERN_TYPE_HPP
#define OPENKALMAN_COORDINATES_UNIFORM_PATTERN_TYPE_HPP
...
namespace OpenKalman::coordinates
{
   ...
   * \details The uniform component is either \ref Dimension "Dimension<1>", if T is a \ref euclidean_pattern,
   * or \ref common_descriptor_type_t, which will be a 1D \ref descriptor.
   */
  ...
  namespace detail
  {
    ...
    struct common_descriptor_dimension_is_1<T, std::void_t<typename common_descriptor_type<T>::type>>
      : std::bool_constant<values::fixed_value_compares_with<dimension_of<common_descriptor_type_t<T>>, 1>> {};
  }
#endif
  ...
  template<pattern T> requires
    ...
    values::fixed_value_compares_with<dimension_of<typename common_descriptor_type<T>::type>, 1>
  struct uniform_pattern_type<T>
#else
  template<typename T>
  struct uniform_pattern_type<T, std::enable_if_t<
    pattern<T> and (euclidean_pattern<T> or detail::common_descriptor_dimension_is_1<T>::value)>>
#endif
    : std::conditional_t<
      euclidean_pattern<T>,
