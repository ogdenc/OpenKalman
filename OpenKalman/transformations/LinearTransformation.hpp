...
namespace OpenKalman
{
  ...
  template<fixed_pattern InputCoefficients, fixed_pattern OutputCoefficients, typed_matrix_nestable TransformationMatrix,
    ...
    (index_dimension_of_v<TransformationMatrix, 0> == patterns::dimension_of_v<OutputCoefficients>) and
    (index_dimension_of_v<TransformationMatrix, 1> == patterns::dimension_of_v<InputCoefficients>) and
    ((index_dimension_of_v<PerturbationTransformationMatrices, 0> == patterns::dimension_of_v<OutputCoefficients>) and ...) and
  ...
  namespace internal
  {
    ...
    namespace detail
    {
      ...
      template<typename T, typename R, typename C>
      struct is_linear_transformation_input<T, R, C, std::enable_if_t<
        typed_matrix_nestable<T> and (index_dimension_of<T, 0>::value == patterns::dimension_of_v<R>) and
        (index_dimension_of<T, 1>::value == patterns::dimension_of_v<C>)>> : std::true_type {};
    }
#endif
    ...
    concept linear_transformation_input =
      ...
      (not typed_matrix_nestable<T> or (index_dimension_of_v<T, 0> == patterns::dimension_of_v<RowCoefficients> and
        index_dimension_of_v<T, 1> == patterns::dimension_of_v<ColumnCoefficients>));
#else
    template<typename T, typename RowCoefficients, typename ColumnCoefficients = RowCoefficients>
  ...
  struct LinearTransformation
  {
    ...
    static_assert(index_dimension_of_v<TransformationMatrix, 0> == patterns::dimension_of_v<OutputCoefficients>);
    static_assert(index_dimension_of_v<TransformationMatrix, 1> == patterns::dimension_of_v<InputCoefficients>);
    static_assert(((index_dimension_of_v<PerturbationTransformationMatrices, 0> == patterns::dimension_of_v<OutputCoefficients>) and ...));
  ...
  public:
    ...
    auto jacobian(const In&, const Perturbations& ...) const
    {
      ...
      if constexpr(sizeof...(Perturbations) + 1 > mat_count)
      {
        ...
        auto id = attach_patterns(make_identity_matrix_like<TransformationMatrix>(), OutputCoefficients{}, OutputCoefficients{});
