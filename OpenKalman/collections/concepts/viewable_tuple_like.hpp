...
namespace OpenKalman::collections
{
  namespace detail
  {
    ...
    template<typename T, std::size_t...i> requires
      (... and move_constructible_object_or_lvalue_ref<collection_element_t<i, T>>)
    ...
    struct has_viewable_elements_impl<T, std::index_sequence<i...>, std::enable_if_t<
      (... and move_constructible_object_or_lvalue_ref<typename collection_element<i, T>::type>)>> : std::true_type {};
    ...
    template<typename T>
    struct has_viewable_elements<T, std::enable_if_t<uniformly_gettable<T>>> : has_viewable_elements_impl<T> {};
  ...
  /**
   * \brief A \ref uniformly_gettable object that has a \ref common_collection_type and can be converted into
  ...
  concept viewable_tuple_like =
    ...
    (size_of_v<T> == 0 or requires { typename common_collection_type<T>::type; }) and
    (std::is_lvalue_reference_v<T> or detail::has_viewable_elements<std::decay_t<T>>::value);
#else
  inline constexpr bool viewable_tuple_like =
    uniformly_gettable<T> and
    ((sized<T> and values::fixed_value_compares_with<size_of<T>, 0_uz>) or detail::has_common_collection_type<T>::value) and
    (std::is_lvalue_reference_v<T> or detail::has_viewable_elements<std::decay_t<T>>::value);
#endif
