...
namespace OpenKalman::collections
{
  ...
  namespace detail
  {
    ...
    {
      if constexpr (values::fixed<I>) { return gettable<values::fixed_value_of_v<I>, T>; }
      else { return false; }
    }();
  }
#endif
   ...
   * \details This function takes an \ref values::index "index" parameter instead of a template parameter like std::get.
  ...
  template<collection Arg, values::index I> requires
    (stdcompat::ranges::random_access_range<Arg> or (values::fixed<I> and gettable<values::fixed_value_of_v<I>, Arg>))
#else
  ...
  get(Arg&& arg, I i)
  {
    ...
    { static_assert(size_of_v<Arg> == dynamic_size or values::fixed_value_of_v<I> < size_of_v<Arg>, "Index out of range"); }

#ifdef __cpp_lib_ranges
    if constexpr (values::fixed<I> and requires { requires gettable<values::fixed_value_of<I>::value, Arg>; })
#else
    if constexpr (detail::gettable_with_i<Arg, I>)
#endif
    {
      return OpenKalman::internal::generalized_std_get<values::fixed_value_of_v<I>>(std::forward<Arg>(arg));
    }
    else
    {
      std::size_t n = values::to_value_type(std::move(i));
