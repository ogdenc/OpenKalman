...
#ifndef OPENKALMAN_SPECIAL_MATRIX_CHOLESKY_HPP
#define OPENKALMAN_SPECIAL_MATRIX_CHOLESKY_HPP
...
namespace OpenKalman
{
  ...
  Cholesky_factor(A&& a)
  {
      ...
      const NestedMatrix, const typename Eigen::MatrixBase<NestedMatrix>::ConstantReturnType>)
    {
      ...
      {
        static_assert(diagonal_matrix<A>);
        auto vec = std::sqrt(s) * make_constant_matrix_like<A, 1>(Dimensions<dim>{}, Dimensions<1>{});
        return DiagonalMatrix<decltype(vec)> {vec};
      }
      else if constexpr(triangle_type == TriangleType::lower)
      {
        auto col0 = std::sqrt(s) * make_constant_matrix_like<A, 1>(Dimensions<dim>{}, Dimensions<1>{});
      ...
      {
        static_assert(triangle_type == TriangleType::upper);
        auto row0 = std::sqrt(s) * make_constant_matrix_like<A, 1>(Dimensions<1>{}, Dimensions<dim>{});
...
#endif //OPENKALMAN_SPECIAL_MATRIX_CHOLESKY_HPP
