...
namespace OpenKalman
{
  ...
  template<indexible T, scalar_constant C, vector_space_descriptor_collection Ds>
  constexpr constant_matrix auto
#else
  template<typename T, typename C, typename Ds, std::enable_if_t<
    indexible<T> and scalar_constant<C> and vector_space_descriptor_collection<Ds>, int> = 0>
  constexpr auto
#endif
  make_constant(C&& c, Ds&& ds)
  {
    if constexpr (interface::make_constant_defined_for<T, C&&, Ds&&>)
    {
      using Trait = interface::library_interface<std::decay_t<T>>;
      if constexpr (euclidean_vector_space_descriptor_collection<Ds>)
        return Trait::template make_constant(std::forward<C>(c), std::forward<Ds>(ds));
      else
        return make_vector_space_adapter(Trait::template make_constant(std::forward<C>(c), ds), std::forward<Ds>(ds));
    }
    else
    {
      // Default behavior if interface function not defined:
      using Scalar = std::decay_t<decltype(get_scalar_constant_value(c))>;
      using U = std::decay_t<decltype(make_dense_object<T, Layout::none, Scalar>(std::forward<Ds>(ds)))>;
      return ConstantAdapter<U, std::decay_t<C>> {std::forward<C>(c), internal::remove_trailing_1D_descriptors(std::forward<Ds>(ds))};
    }
  }
   ...
   */
#ifdef __cpp_concepts
  template<indexible T, scalar_constant C, vector_space_descriptor...Ds>
  constexpr constant_matrix auto
#else
  template<typename T, typename C, typename...Ds, std::enable_if_t<
    indexible<T> and scalar_constant<C> and (vector_space_descriptor<Ds> and ...), int> = 0>
  constexpr auto
#endif
  make_constant(C&& c, Ds&&...ds)
  {
    return make_constant<T>(std::forward<C>(c), std::forward_as_tuple(std::forward<Ds>(ds)...));
  }
   ...
   */
#ifdef __cpp_concepts
  template<indexible T, scalar_constant C, auto...constant, vector_space_descriptor_collection Ds> requires
  ...
  template<typename T, typename C, auto...constant, typename Ds, std::enable_if_t<
    indexible<T> and scalar_constant<C> and vector_space_descriptor_collection<Ds> and
    ((scalar_constant<C, ConstantType::static_constant> and sizeof...(constant) == 0) or
   ...
   */
#ifdef __cpp_concepts
  template<indexible T, auto constant, vector_space_descriptor_collection Ds> requires scalar_type<decltype(constant)>
  constexpr constant_matrix auto
#else
  template<typename T, auto constant, typename Ds, std::enable_if_t<
    indexible<T> and scalar_type<decltype(constant)> and vector_space_descriptor_collection<Ds>, int> = 0>
  constexpr auto
#endif
  make_constant(Ds&& ds)
  {
   ...
   */
#ifdef __cpp_concepts
  template<indexible T, auto constant, vector_space_descriptor...Ds> requires scalar_type<decltype(constant)>
  constexpr constant_matrix auto
#else
  template<typename T, auto constant, typename...Ds, std::enable_if_t<
    indexible<T> and scalar_type<decltype(constant)> and (vector_space_descriptor<Ds> and ...), int> = 0>
  constexpr auto
#endif
  make_constant(Ds&&...ds)
  {
