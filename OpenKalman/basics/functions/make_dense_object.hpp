...
namespace OpenKalman
{
...
#endif
  {
    ...
    {
      return std::apply(
        [](const auto&...d) { return make_dense_object<T, layout, Scalar>(d...); },
 ...
 */
  ...
  make_dense_object(Arg&& arg)
  {
    if constexpr (writable<Arg> and std::is_same_v<Scalar, scalar_type_of_t<Arg>>)
      ...
      return make_dense_object<std::decay_t<Arg>, layout, Scalar>(std::forward<Arg>(arg));
  }
  ...
  make_dense_object(Arg&& arg)
  {
    if constexpr (writable<Arg>)
      ...
      return make_dense_object<std::decay_t<Arg>, layout, scalar_type_of_t<Arg>>(std::forward<Arg>(arg));
  }
