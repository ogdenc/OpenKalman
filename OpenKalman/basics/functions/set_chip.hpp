...
namespace OpenKalman
{
  ...
  template<std::size_t...indices, writable Arg, indexible Chip, index_value...Ixs> requires
    (sizeof...(indices) == sizeof...(Ixs))
#else
  template<std::size_t...indices, typename Arg, typename Chip, typename...Ixs, std::enable_if_t<
    writable<Arg> and indexible<Chip> and (index_value<Ixs> and ...) and (sizeof...(indices) == sizeof...(Ixs)), int> = 0>
#endif
  constexpr Arg&&
