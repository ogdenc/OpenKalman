    ...
    constexpr auto constant_reduce_operation(const BinaryOperation& op, const Constant& c, const Dim& dim)
    {
      ...
      else
      {
        if constexpr (dynamic_index_value<Dim>)
        {
          ...
          else return op(constant_reduce_operation(op, c, get_scalar_constant_value(dim) - 1), get_scalar_constant_value(c));
        }
        else if constexpr (Dim::value <= 1) return c;
        else
        {
          auto dim_m1 = std::integral_constant<std::size_t, Dim::value - 1>{};
          return values::scalar_constant_operation {op, constant_reduce_operation(op, c, dim_m1), c};
        }
      }
    }
