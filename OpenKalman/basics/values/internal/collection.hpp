...
namespace OpenKalman::internal
{
  ...
  namespace detail
  {
    ...
    template<typename T>
    struct is_collection_impl<T, std::void_t<decltype(*std::declval<T>().begin())>>
      : std::true_type {}; 
  } // namespace detail
#endif 
   ...
   * \details This will be a \ref internal::tuple_like "tuple_like" object or a std::ranges::input_range.
   */
  template<typename T>
#if defined(__cpp_lib_ranges) and defined(__cpp_lib_remove_cvref)
  concept collection = internal::tuple_like<T> or std::ranges::input_range<std::remove_cvref_t<T>>;
#else
