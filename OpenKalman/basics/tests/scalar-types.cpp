...
namespace
{
  auto tolerance = [](const auto& a, const auto& b, const auto& err){ return are_within_tolerance(a, b, err); };
...
{
  static_assert(std::is_floating_point_v<decltype(constexpr_real(3))>);
  static_assert(std::is_floating_point_v<decltype(constexpr_imag(3))>);
  static_assert(std::is_floating_point_v<decltype(constexpr_real(constexpr_conj(3)))>);
  static_assert(std::is_floating_point_v<decltype(constexpr_real(std::complex<double>{3., 4.}))>);
  static_assert(constexpr_real(3.) == 3);
  static_assert(constexpr_real(3.f) == 3);
  static_assert(constexpr_real(3.l) == 3);
  static_assert(constexpr_imag(3.) == 0);
  static_assert(constexpr_imag(3.f) == 0);
  static_assert(constexpr_imag(3.l) == 0);
  static_assert(constexpr_conj(3.) == 3.);
  static_assert(constexpr_conj(3.f) == 3.f);
  static_assert(constexpr_conj(3.l) == 3.l);
...
{
  static_assert(constexpr_signbit(-3));
  static_assert(not constexpr_signbit(3));
  static_assert(not constexpr_signbit(3.));
  static_assert(constexpr_signbit(-3.));
  static_assert(constexpr_signbit(-3.));
  static_assert(constexpr_signbit(-3.f));
  static_assert(constexpr_signbit(-3.l));
  static_assert(not constexpr_signbit(0.));
#ifdef __cpp_lib_constexpr_cmath
  static_assert(constexpr_signbit(-0.));
  static_assert(not constexpr_signbit(+0.));
  static_assert(constexpr_signbit(-NAN));
  static_assert(not constexpr_signbit(NAN));
#elif defined(__cpp_lib_is_constant_evaluated)
  EXPECT_TRUE(constexpr_signbit(-0.));
  EXPECT_TRUE(not constexpr_signbit(+0.));
...
{
  static_assert(constexpr_copysign(5, -3) == -5.);
  static_assert(constexpr_copysign(5, 3) == 5.);
  static_assert(constexpr_copysign(5U, -3) == -5.);
  static_assert(std::is_floating_point_v<decltype(constexpr_copysign(5U, -3))>);
  static_assert(constexpr_copysign(3., -5.) == -3.);
  static_assert(constexpr_copysign(-3., 5.) == 3.);
  static_assert(constexpr_copysign(-3.f, 5.f) == 3.f);
  static_assert(constexpr_copysign(3.l, -5.l) == -3.l);
  static_assert(constexpr_copysign(3.f, -INFINITY) == -3.f);
  static_assert(constexpr_copysign(3.f, INFINITY) == 3.f);
  static_assert(constexpr_copysign(INFINITY, -3.f) == -INFINITY);
  static_assert(constexpr_copysign(-INFINITY, 3.f) == INFINITY);
...
TEST(basics, constexpr_sqrt)
{
  if constexpr (std::numeric_limits<double>::is_iec559)
  {
    ...
    EXPECT_TRUE(std::isnan(constexpr_sqrt(constexpr_NaN<double>())));
    EXPECT_EQ(constexpr_sqrt(constexpr_infinity<double>()), constexpr_infinity<double>());
    EXPECT_TRUE(std::signbit(constexpr_sqrt(-0.)));
    EXPECT_FALSE(std::signbit(constexpr_sqrt(+0.)));
    EXPECT_TRUE(constexpr_sqrt(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}) != constexpr_sqrt(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}));
  }
  ...
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{-4}), std::sqrt(std::complex<double>{-4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{3, 4}), std::sqrt(std::complex<double>{3, 4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{3, -4}), (std::complex<double>{2, -1}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{3, 4}), (std::complex<double>{2, 1}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{3, -4}), std::sqrt(std::complex<double>{3, -4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{-3, 4}), std::sqrt(std::complex<double>{-3, 4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_sqrt(std::complex<double>{-3, -4}), std::sqrt(std::complex<double>{-3, -4}), 1e-9);
...
{
  static_assert(std::is_integral_v<decltype(constexpr_abs(-3))>);
  static_assert(std::is_floating_point_v<decltype(constexpr_abs(3.))>);
  static_assert(std::is_floating_point_v<decltype(constexpr_abs(std::complex<double>{3., 4.}))>);
  static_assert(constexpr_abs(3) == 3);
  static_assert(constexpr_abs(-3) == 3);
  static_assert(constexpr_abs(3.) == 3);
  static_assert(constexpr_abs(-3.) == 3);
  static_assert(constexpr_abs(-3.f) == 3);
  static_assert(constexpr_abs(-3.l) == 3);
  static_assert(constexpr_abs(INFINITY) == INFINITY);
  static_assert(constexpr_abs(-INFINITY) == INFINITY);
  EXPECT_TRUE(std::isnan(constexpr_abs(-NAN)));
  EXPECT_FALSE(std::signbit(constexpr_abs(NAN)));
#if defined(__cpp_lib_constexpr_cmath) or defined(__cpp_lib_is_constant_evaluated)
  EXPECT_FALSE(std::signbit(constexpr_abs(-0.)));
  EXPECT_FALSE(std::signbit(constexpr_abs(-NAN)));
#endif
...
TEST(basics, constexpr_exp)
{
  ...
  if constexpr (std::numeric_limits<double>::is_iec559)
  {
    EXPECT_TRUE(std::isnan(constexpr_exp(constexpr_NaN<double>())));
    EXPECT_TRUE(constexpr_exp(-constexpr_infinity<double>()) == 0);
    EXPECT_TRUE(constexpr_exp(constexpr_infinity<double>()) == constexpr_infinity<double>());
    EXPECT_TRUE(constexpr_exp(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}) != constexpr_exp(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}));
  }

  static_assert(are_within_tolerance<10>(constexpr_exp(0), 1));
  static_assert(are_within_tolerance<10>(constexpr_exp(1), e));
  static_assert(are_within_tolerance<10>(constexpr_exp(2), e*e));
  static_assert(are_within_tolerance<100>(constexpr_exp(3), e*e*e));
  static_assert(are_within_tolerance<10>(constexpr_exp(-1), 1/e));
  static_assert(are_within_tolerance<10>(constexpr_exp(-2), 1/(e*e)));
  static_assert(are_within_tolerance<10>(constexpr_exp(1.0), e));
  static_assert(are_within_tolerance<10>(constexpr_exp(2.0), e*e));
  static_assert(are_within_tolerance<10>(constexpr_exp(1.0L), eL));
  static_assert(are_within_tolerance<10>(constexpr_exp(2.0L), eL*eL));
  static_assert(are_within_tolerance<100>(constexpr_exp(3.0L), eL*eL*eL));
...
TEST(basics, constexpr_expm1)
{
  ...
  if constexpr (std::numeric_limits<double>::is_iec559)
  {
    EXPECT_TRUE(constexpr_expm1(constexpr_NaN<double>()) != constexpr_expm1(constexpr_NaN<double>()));
    EXPECT_TRUE(constexpr_expm1(constexpr_infinity<double>()) == constexpr_infinity<double>());
    EXPECT_TRUE(constexpr_expm1(-constexpr_infinity<double>()) == -1);
    EXPECT_TRUE(std::signbit(constexpr_expm1(-0.)));
    EXPECT_FALSE(std::signbit(constexpr_expm1(+0.)));
    EXPECT_TRUE(constexpr_expm1(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}) != constexpr_expm1(std::complex<double>{constexpr_NaN<double>(), constexpr_NaN<double>()}));
  }

  static_assert(constexpr_expm1(0) == 0);
  static_assert(are_within_tolerance(constexpr_expm1(1), e - 1));
  static_assert(are_within_tolerance(constexpr_expm1(2), e*e - 1, 1e-9));
  static_assert(are_within_tolerance(constexpr_expm1(3), e*e*e - 1, 1e-9));
  static_assert(are_within_tolerance(constexpr_expm1(-1), 1 / e - 1, 1e-9));
  static_assert(are_within_tolerance(constexpr_expm1(3.L), eL*eL*eL - 1, 1e-9));
  static_assert(std::real(constexpr_expm1(std::complex<double>{3e-12, 0})) == constexpr_expm1(3e-12));
  EXPECT_PRED3(tolerance, constexpr_expm1(1e-4), std::expm1(1e-4), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_expm1(1e-8), std::expm1(1e-8), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_expm1(1e-32), std::expm1(1e-32), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_expm1(5.2), std::expm1(5.2), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_expm1(10.2), std::expm1(10.2), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_expm1(-10.2), std::expm1(-10.2), 1e-9);
...
TEST(basics, constexpr_sin)
{
  ...
  if constexpr (std::numeric_limits<double>::is_iec559)
  {
    EXPECT_TRUE(std::isnan(constexpr_sin(constexpr_NaN<double>())));
  ...
  static_assert(are_within_tolerance(constexpr_sin(2*pi), 0));
  static_assert(are_within_tolerance(constexpr_sin(-2*pi), 0));
  static_assert(are_within_tolerance(constexpr_sin(pi), 0));
  static_assert(are_within_tolerance(constexpr_sin(-pi), 0));
  static_assert(are_within_tolerance(constexpr_sin(32*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(-32*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(0x1p16*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p16*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(0x1p16L*piL), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p16L*piL), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_sin(0x1p16F*piF), 0, 1e-2));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p16F*piF), 0, 1e-2));
  static_assert(are_within_tolerance(constexpr_sin(0x1p100L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p100L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_sin(0x1p180L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p180L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_sin(0x1p250L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_sin(-0x1p250L*piL), 0, 1.));
  ...
  static_assert(are_within_tolerance(constexpr_sin(pi/4 + 32*pi), numbers::sqrt2_v<double>/2, 1e-9));
...
TEST(basics, constexpr_cos)
{
  ...
  static_assert(are_within_tolerance(constexpr_cos(-0x1p16L*piL), 1));
  static_assert(are_within_tolerance(constexpr_cos(0x1p16F*piF), 1, 1e-4));
  static_assert(are_within_tolerance(constexpr_cos(-0x1p16F*piF), 1, 1e-4));
  static_assert(are_within_tolerance(constexpr_cos(0x1p100L*piL), 1, 1.));
  static_assert(are_within_tolerance(constexpr_cos(-0x1p100L*piL), 1, 1.));
  static_assert(are_within_tolerance(constexpr_cos(0x1p180L*piL), 1, 1.));
  static_assert(are_within_tolerance(constexpr_cos(-0x1p180L*piL), 1, 1.));
  static_assert(are_within_tolerance(constexpr_cos(0x1p250L*piL), 1, 1.));
  static_assert(are_within_tolerance(constexpr_cos(-0x1p250L*piL), 1, 1.));
...
TEST(basics, constexpr_tan)
{
  ...
  static_assert(are_within_tolerance(constexpr_tan(2*pi), 0));
  static_assert(are_within_tolerance(constexpr_tan(-2*pi), 0));
  static_assert(are_within_tolerance(constexpr_tan(pi), 0));
  static_assert(are_within_tolerance(constexpr_tan(-pi), 0));
  static_assert(are_within_tolerance(constexpr_tan(32*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(-32*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(0x1p16*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p16*pi), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(0x1p16L*piL), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p16L*piL), 0, 1e-9));
  static_assert(are_within_tolerance(constexpr_tan(0x1p16F*piF), 0, 1e-2));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p16F*piF), 0, 1e-2));
  static_assert(are_within_tolerance(constexpr_tan(0x1p100L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p100L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_tan(0x1p180L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p180L*piL), 0, 1.));
  static_assert(are_within_tolerance(constexpr_tan(0x1p250L*piL), 0, 2.));
  static_assert(are_within_tolerance(constexpr_tan(-0x1p250L*piL), 0, 2.));
  ...
  static_assert(are_within_tolerance(constexpr_tan(pi/4 + 32*pi), 1, 1e-9));
...
TEST(basics, constexpr_log)
{
  ...
  static_assert(are_within_tolerance(constexpr_log(e*e*e*e*e*e*e*e*e*e*e*e*e*e*e*e), 16));
  static_assert(are_within_tolerance(constexpr_log(1 / e), -1));
...
TEST(basics, constexpr_asin)
{
  ...
  static_assert(are_within_tolerance(constexpr_asin(-numbers::sqrt2_v<double>/2), -pi/4));
  static_assert(constexpr_asin(0.99995) > 0);
  static_assert(constexpr_asin(-0.99995) < 0);
  ...
  EXPECT_NEAR(constexpr_asin(0.99), std::asin(0.99), 1e-9);
...
TEST(basics, constexpr_acos)
{
  ...
  static_assert(are_within_tolerance(constexpr_acos(-numbers::sqrt2_v<double>/2), 3*pi/4));
...
TEST(basics, constexpr_pow)
{
  if constexpr (std::numeric_limits<double>::is_iec559)
  {
    EXPECT_FALSE(std::signbit(constexpr_pow(+0., 3U)));
  ...
  static_assert(constexpr_pow(-0., 3U) == 0);
  static_assert(constexpr_pow(+0., 2U) == 0);
  static_assert(constexpr_pow(-0., 2U) == 0);
  ...
  static_assert(std::is_floating_point_v<decltype(constexpr_pow(2, 16U))>);
  ...
  static_assert(constexpr_pow(-0., 3) == 0);
  static_assert(constexpr_pow(+0., 2) == 0);
  static_assert(constexpr_pow(-0., 2) == 0);
  ...
  EXPECT_PRED3(tolerance, constexpr_pow(2., std::complex<double>{-4}), std::pow(2., std::complex<double>{-4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(2, std::complex<double>{-4}), std::pow(2, std::complex<double>{-4}), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(std::complex<double>{3, 4}, 2.), std::pow(std::complex<double>{3, 4}, 2.), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(std::complex<double>{3, 4}, 2), std::pow(std::complex<double>{3, 4}, 2), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(std::complex<double>{3, 4}, -2), std::pow(std::complex<double>{3, 4}, -2), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(std::complex<double>{3, 4}, 3), std::pow(std::complex<double>{3, 4}, 3), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(std::complex<double>{3, 4}, -3), std::pow(std::complex<double>{3, 4}, -3), 1e-9);
  EXPECT_PRED3(tolerance, constexpr_pow(2., std::complex<double>{3, -4}), std::pow(2., std::complex<double>{3, -4}), 1e-9);
