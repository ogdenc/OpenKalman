...
{
  static_assert(std::is_same_v<decltype(rg::to<std::vector<int>>(std::declval<std::array<int, 5>&>())), std::vector<int>>);
  static_assert(std::is_same_v<decltype(rg::to<std::vector<int>>(std::declval<std::array<int, 5>>())), std::vector<int>>);
  static_assert(std::is_same_v<decltype(rg::to<std::vector>(std::declval<std::array<int, 5>>())), std::vector<int>>);
  static_assert(std::is_same_v<decltype(rg::to<std::vector>(std::declval<std::array<int, 5>&>())), std::vector<int>>);

  auto a0 = std::array {1, 2, 3, 4, 5};
  auto vec0a = rg::to<std::vector<int>>(a0);
  static_assert(std::is_same_v<decltype(vec0a), std::vector<int>>);
  EXPECT_EQ(vec0a[2], 3);
  auto vec0b = a0 | rg::to<std::vector<int>>();
  static_assert(std::is_same_v<decltype(vec0b), std::vector<int>>);
  EXPECT_EQ(vec0b[4], 5);

  auto vec1a = rg::to<std::vector<int>>(std::array {1, 2, 3, 4, 5});
  static_assert(std::is_same_v<decltype(vec1a), std::vector<int>>);
  EXPECT_EQ(vec1a[2], 3);
  auto vec1b = std::array {1, 2, 3, 4, 5} | rg::to<std::vector<int>>();
  static_assert(std::is_same_v<decltype(vec1b), std::vector<int>>);
  EXPECT_EQ(vec1b[4], 5);

  auto vec2a = rg::to<std::vector>(a0);
  static_assert(std::is_same_v<decltype(vec2a), std::vector<int>>);
  EXPECT_EQ(vec2a[2], 3);
  auto vec2b = a0 | rg::to<std::vector>();
  static_assert(std::is_same_v<decltype(vec2b), std::vector<int>>);
  EXPECT_EQ(vec2b[4], 5);

  auto vec3a = rg::to<std::vector>(std::array {1, 2, 3, 4, 5});
  static_assert(std::is_same_v<decltype(vec3a), std::vector<int>>);
  EXPECT_EQ(vec3a[2], 3);
  auto vec3b = std::array {1, 2, 3, 4, 5} | rg::to<std::vector>();
  static_assert(std::is_same_v<decltype(vec3b), std::vector<int>>);
  EXPECT_EQ(vec3b[4], 5);

  auto vec4a = rg::to<std::tuple<std::array<int, 5>, int, int>>(a0, 8, 9);
  ...
  auto vec4b = a0 | rg::to<std::tuple<std::array<int, 5>, int, int>>(8, 9);
  ...
  auto vec5a = rg::to<std::tuple>(a0, 8, 9);
  ...
  auto vec5b = a0 | rg::to<std::tuple>(8, 9);
