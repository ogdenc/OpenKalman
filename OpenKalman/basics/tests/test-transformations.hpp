  ...
  inline auto sum_of_squares = Transformation
    {
      ...
      {
        static_assert(equivalent_to<row_index_descriptor_of_t<decltype(x)>, Dimensions<n>>);
        static_assert(equivalent_to<column_index_descriptor_of_t<decltype(x)>, Axis > );
        static_assert((equivalent_to<row_index_descriptor_of_t<decltype(ps)>, Axis > and ...));
        static_assert((equivalent_to<column_index_descriptor_of_t<decltype(ps)>, Axis > and ...));
  ...
  inline auto time_of_arrival = Transformation
    {
      ...
      [](const auto& x, const auto& ...ps) // Jacobians
      {
        ...
        return std::make_tuple(make_self_contained(adjoint(x) / std::sqrt(trace(adjoint(x) * x))),
          Matrix<Axis, Axis, untyped_dense_writable_matrix_t<decltype(ps), 1, 1>> {1.}...);
      },
      [](const auto& x, const auto& ...ps) // Hessians
      {
        static_assert(equivalent_to<row_index_descriptor_of_t<decltype(x)>, Dimensions<n>>);
  ...
  inline auto Cartesian2polar = Transformation
    {
      ...
      [](const M2t& x, const auto& ...ps) // Jacobians
      {
        ...
        MP2t ret = {
          ...
          -x(1) * h2, x(0) * h2};
        return std::tuple_cat(std::tuple {
          std::move(ret)}, OpenKalman::internal::tuple_replicate<sizeof...(ps)>(make_identity_matrix_like<MPPt>()));
      },
      [](const M2t& x, const auto& ...ps) // Hessians
      {
        const auto h = 1 / std::hypot(x(0), x(1));
  ...
  inline auto Cylindrical2spherical = Transformation
    {
      ...
      [](const MC1t& x, const auto& ...ps) // Jacobians
      {
        ...
        MSCt ret = {
          ...
          -x(2) * h2, 0, x(0) * h2};
        return std::tuple_cat(std::tuple {
          std::move(ret)}, OpenKalman::internal::tuple_replicate<sizeof...(ps)>(make_identity_matrix_like<MSSt>()));
      },
