...
namespace OpenKalman::values
{
  ...
  namespace detail
  {
    ...
    concept has_static_constant =
      interface::get_constant_defined_for<T, ConstantType::static_constant> or
    ...
    template<typename T>
    struct has_static_constant_impl<T, std::enable_if_t<interface::get_constant_diagonal_defined_for<T, ConstantType::static_constant>>>
      : std::bool_constant<one_dimensional<T> or internal::are_within_tolerance(const_diag_value<T>, 0)> {};

    template<typename T>
    constexpr bool has_static_constant = interface::get_constant_defined_for<T, ConstantType::static_constant> or
  ...
  template<indexible T> requires (not detail::has_static_constant<T>) and
    (interface::get_constant_defined_for<T, ConstantType::dynamic_constant> or one_dimensional<T>)
  struct constant_coefficient<T>
#else
  template<typename T>
  struct constant_coefficient<T, std::enable_if_t<
    (interface::get_constant_defined_for<T, ConstantType::dynamic_constant> or one_dimensional<T>) and
  ...
  public:
    ...
    explicit constexpr constant_coefficient(const std::decay_t<T>& t) : m_value {[](const auto& t){
        ...
        else if constexpr (interface::get_constant_diagonal_defined_for<std::decay_t<T>, ConstantType::dynamic_constant>)
