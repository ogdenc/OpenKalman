...
namespace OpenKalman
{
  ...
  concept writable_by_component = indexible<T> and (not std::is_const_v<std::remove_reference_t<T>>) and (not empty_object<T>) and
    interface::set_component_defined_for<T, std::add_lvalue_reference_t<T>, const scalar_type_of_t<T>&, const Indices&>;
#else
  constexpr bool writable_by_component = indexible<T> and (not std::is_const_v<std::remove_reference_t<T>>) and (not empty_object<T>) and
    interface::set_component_defined_for<T, std::add_lvalue_reference_t<T>, const typename scalar_type_of<T>::type&, const Indices&>;
#endif
