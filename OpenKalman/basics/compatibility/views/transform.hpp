...
namespace OpenKalman::stdex::ranges
{
  ...
  struct transform_view : stdex::ranges::view_interface<transform_view<V, F>>
  {
  private:

    static_assert(stdex::ranges::input_range<V>);
    static_assert(std::is_move_constructible_v<F>);
    static_assert(stdex::ranges::view<V>);
    static_assert(std::is_object_v<F>);
    static_assert(std::is_invocable_v<F&, stdex::ranges::range_reference_t<V>>);
  ...
  public:
    ...
    public:
      ...
      using iterator_concept = std::conditional_t<
        stdex::ranges::random_access_range<Base>, std::random_access_iterator_tag,
        std::conditional_t<stdex::ranges::bidirectional_range<Base>, std::bidirectional_iterator_tag,
        std::conditional_t<stdex::ranges::forward_range<Base>, std::forward_iterator_tag, std::input_iterator_tag>>>;

      using iterator_category = std::conditional_t<
        not std::is_reference_v<std::invoke_result_t<MCF&, stdex::ranges::range_reference_t<Base>>>,
        std::input_iterator_tag,
        typename stdex::iterator_traits<stdex::ranges::iterator_t<Base>>::iterator_category>;
      ...
      template<bool Enable = true, std::enable_if_t<Enable and stdex::default_initializable<stdex::ranges::iterator_t<Base>>, int> = 0>
      constexpr iterator() {};

      constexpr iterator(Parent& parent, stdex::ranges::iterator_t<Base> current) : current_ {std::move(current)}, parent_ {std::addressof(parent)} {}
      ...
      constexpr const stdex::ranges::iterator_t<Base>& base() const & noexcept { return current_; }

      constexpr stdex::ranges::iterator_t<Base> base() && { return std::move(current_); }

      constexpr decltype(auto) operator*() const
      {
        return stdex::invoke(*parent_->fun_, *current_);
      }

      template<bool Enable = true, std::enable_if_t<Enable and stdex::ranges::random_access_range<Base>, int> = 0>
      constexpr decltype(auto) operator[](difference_type n) const
      {
        return stdex::invoke(*parent_->fun_, current_[n]);
      }
      ...
      template<bool Enable = true, std::enable_if_t<Enable and stdex::ranges::random_access_range<Base>, int> = 0>
      friend constexpr decltype(auto) iter_move(const iterator& i) noexcept(noexcept(stdex::invoke(*i.parent_->fun_, *i.current_)))
      {
    ...
    private:
      ...
      stdex::ranges::iterator_t<Base> current_;
      Parent* parent_;
    };
    ...
    public:
      ...
      constexpr explicit sentinel(stdex::ranges::sentinel_t<Base> end) : end_ {end} {};

      constexpr const stdex::ranges::sentinel_t<Base>& base() const { return end_; }
      ...
      template<bool Enable = true, std::enable_if_t<Enable and
        stdex::convertible_to<decltype(std::declval<stdex::ranges::iterator_t<Base>>() - std::declval<stdex::ranges::sentinel_t<Base>>()), difference_type>, int> = 0>
      friend constexpr difference_type operator-(const iterator<Const>& x, const sentinel& y)
      { return x.current_ - y.end_; }

      template<bool Enable = true, std::enable_if_t<Enable and
        stdex::convertible_to<decltype(-(std::declval<stdex::ranges::sentinel_t<Base>>() - std::declval<stdex::ranges::iterator_t<Base>>())), difference_type>, int> = 0>
      friend constexpr difference_type operator-(const sentinel& y, const iterator<Const>& x)
      { return y.end_ - x.current_; }

    private:

      stdex::ranges::sentinel_t<Base> end_;
    };


    template<bool Enable = true, std::enable_if_t<Enable and stdex::default_initializable<V> and stdex::default_initializable<F>, int> = 0>
    constexpr
    transform_view() {}
    ...
    constexpr auto
    begin() { return iterator<false> {*this, stdex::ranges::begin(base_)}; }

    template<bool Enable = true, std::enable_if_t<Enable and
      stdex::ranges::range<const V> and std::is_invocable_v<const F&, stdex::ranges::range_reference_t<const V>>, int> = 0>
    constexpr auto
    begin() const { return iterator<true> {*this, stdex::ranges::begin(base_)}; }


    template<bool Enable = true, std::enable_if_t<Enable and not stdex::ranges::common_range<V>, int> = 0>
    constexpr auto
    end() { return sentinel<false> {stdex::ranges::end(base_)}; }

    template<bool Enable = true, std::enable_if_t<Enable and stdex::ranges::common_range<V>, int> = 0>
    constexpr auto
    end() { return iterator<false> {*this, stdex::ranges::end(base_)}; }

    template<bool Enable = true, std::enable_if_t<Enable and stdex::ranges::range<const V> and not stdex::ranges::common_range<const V> and
      std::is_invocable_v<const F&, stdex::ranges::range_reference_t<const V>>, int> = 0>
    constexpr auto
    end() const { return sentinel<true> {stdex::ranges::end(base_)}; }

    template<bool Enable = true, std::enable_if_t<Enable and stdex::ranges::common_range<const V> and
      std::is_invocable_v<const F&, stdex::ranges::range_reference_t<const V>>, int> = 0>
    constexpr auto
    end() const { return iterator<true> {*this, stdex::ranges::end(base_)}; }
  ...
  namespace views
  {
    namespace detail
    {
      ...
      struct transform_closure
        : stdex::ranges::range_adaptor_closure<transform_closure<F>>
      {
        constexpr transform_closure(F&& f) : my_f {std::forward<F>(f)} {};

        template<typename R, std::enable_if_t<stdex::ranges::viewable_range<R>, int> = 0>
        constexpr auto
      ...
      struct transform_adaptor
      {
        ...
        template<typename R, typename F, std::enable_if_t<stdex::ranges::viewable_range<R>, int> = 0>
        constexpr auto
