...
namespace OpenKalman::internal
{
  ...
  smallest_vector_space_descriptor(V&& v, Vs&&...vs)
  {
    ...
    {
      decltype(auto) tail = smallest_vector_space_descriptor<Scalars...>(std::forward<Vs>(vs)...);
      ...
      {
        return Dimensions {std::min<std::size_t>(get_dimension_size_of(v), get_dimension_size_of(tail))};
      }
      else
      {
