...
{
  artillery_2D<hermitian_adapter<M22>>(LinearizedTransform<1>());
}

TEST(kalman, artillery_2d_linearized1_T)
{
  artillery_2D<triangular_adapter<M22>>(LinearizedTransform<1>());
}

TEST(kalman, artillery_2d_linearized2_SA)
{
  artillery_2D<hermitian_adapter<M22>>(LinearizedTransform<2>());
}

TEST(kalman, artillery_2d_cubature_SA)
{
  artillery_2D<hermitian_adapter<M22>>(CubatureTransform());
}

TEST(kalman, artillery_2d_cubature_T)
{
  artillery_2D<triangular_adapter<M22>>(CubatureTransform());
}

TEST(kalman, artillery_2d_unscented_SA)
{
  artillery_2D<hermitian_adapter<M22>>(UnscentedTransformParameterEstimation());
}

TEST(kalman, artillery_2d_unscented_T)
{
  artillery_2D<triangular_adapter<M22>>(UnscentedTransformParameterEstimation());
}

TEST(kalman, artillery_2d_simplex_SA)
{
  artillery_2D<hermitian_adapter<M22>>(SamplePointsTransform<SphericalSimplexSigmaPoints>());
}

TEST(kalman, artillery_2d_simplex_T)
{
  artillery_2D<triangular_adapter<M22>>(SamplePointsTransform<SphericalSimplexSigmaPoints>());
}
