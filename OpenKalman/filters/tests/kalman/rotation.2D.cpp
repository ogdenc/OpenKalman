...
{
  rotation_2D<hermitian_adapter<M22>>(LinearTransform());
}

TEST(kalman, rotation_2D_linear_T)
{
  rotation_2D<triangular_adapter<M22>>(LinearTransform());
}

TEST(kalman, rotation_2D_linearized_SA)
{
  rotation_2D<hermitian_adapter<M22>>(LinearizedTransform<2>());
}

TEST(kalman, rotation_2D_linearized_T)
{
  rotation_2D<triangular_adapter<M22>>(LinearizedTransform<2>());
}

TEST(kalman, rotation_2D_cubature_SA)
{
  rotation_2D<hermitian_adapter<M22>>(CubatureTransform());
}

TEST(kalman, rotation_2D_cubature_T)
{
  rotation_2D<triangular_adapter<M22>>(CubatureTransform());
}

TEST(kalman, rotation_2D_MCT_SA)
{
  rotation_2D<hermitian_adapter<M22>>(MonteCarloTransform(1e6));
}
