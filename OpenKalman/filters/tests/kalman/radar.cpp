...
{
  radar_2D<hermitian_adapter<M22>>(LinearizedTransform<1>());
}

TEST(kalman, radar_2d_linearized1_T)
{
  radar_2D<triangular_adapter<M22>>(LinearizedTransform<1>());
}

TEST(kalman, radar_2d_linearized2_SA)
{
  radar_2D<hermitian_adapter<M22>>(LinearizedTransform<2>());
}

TEST(kalman, radar_2d_cubature_SA)
{
  radar_2D<hermitian_adapter<M22>>(CubatureTransform());
}

TEST(kalman, radar_2d_cubature_T)
{
  radar_2D<triangular_adapter<M22>>(CubatureTransform());
}

TEST(kalman, radar_2d_unscented_SA)
{
  radar_2D<hermitian_adapter<M22>>(UnscentedTransform());
}

TEST(kalman, radar_2d_unscented_T)
{
  radar_2D<triangular_adapter<M22>>(UnscentedTransform());
}

TEST(kalman, radar_2d_simplex_SA)
{
  radar_2D<hermitian_adapter<M22>>(SamplePointsTransform<SphericalSimplexSigmaPoints>());
}

TEST(kalman, radar_2d_simplex_T)
{
  radar_2D<triangular_adapter<M22>>(SamplePointsTransform<SphericalSimplexSigmaPoints>());
}
