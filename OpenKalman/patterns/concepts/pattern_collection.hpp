...
namespace OpenKalman::patterns
{
  ...
  namespace detail
  {
    ...
    {
      return (... and sized_pattern<collections::collection_element_t<Ix, T>>);
    }
    ...
    template<typename T>
    struct is_pattern_range<T, std::enable_if_t<sized_pattern<stdex::ranges::range_value_t<T>>>>
      : std::true_type {};
  }
#endif
  ...
  /**
   * \brief An object describing a \ref collections::sized "sized" collection of /ref sized_pattern objects.
   */
  template<typename T>
  ...
  concept pattern_collection =
    ...
    ( ( std::ranges::random_access_range<T> and
        sized_pattern<std::ranges::range_value_t<T>>) or
      []<std::size_t...Ix>(std::index_sequence<Ix...>)
        { return (... and sized_pattern<collections::collection_element_t<Ix, T>>); }
        (std::make_index_sequence<collections::size_of<T>::value>{})
    );
#else
  constexpr bool pattern_collection =
    collections::collection<T> and
    ( ( stdex::ranges::random_access_range<T> and
        detail::is_pattern_range<T>::value) or
