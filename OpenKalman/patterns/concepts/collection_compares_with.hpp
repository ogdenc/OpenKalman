...
/**
 * \file
 * \brief Definition for \ref collection_compares_with.
 */

#ifndef OPENKALMAN_PATTERNS_COLLECTION_COMPARES_WITH_HPP
#define OPENKALMAN_PATTERNS_COLLECTION_COMPARES_WITH_HPP
...
namespace OpenKalman::patterns
{
  namespace detail
  {
    ...
    constexpr bool
    collection_compares_with_iter_T()
    {
      if constexpr (i < collections::size_of_v<T>)
      {
        ...
        if constexpr (compares_with<Ti, PU, comp, a>)
          return collection_compares_with_iter_T<T, PU, comp, a, i + 1>();
        else
          return false;
      }
      else
    ...
    constexpr bool
    collection_compares_with_iter_U()
    {
      if constexpr (i < collections::size_of_v<U>)
      {
        ...
        if constexpr (compares_with<PT, Ui, comp, a>)
          return collection_compares_with_iter_U<PT, U, comp, a, i + 1>();
        else
          return false;
      }
      else
      ...
      if constexpr (i < collections::size_of_v<T>)
      {
        if constexpr (i < collections::size_of_v<U>)
        {
          ...
          if constexpr (compares_with<Ti, Ui, comp, a>)
            return collection_compares_with_iter<T, U, comp, a, i + 1>();
          else
            return false;
        }
        else
        {
          return collection_compares_with_iter_T<T, Dimensions<1>, comp, a, i>();
        }
      }
      else if constexpr (i < collections::size_of_v<U>)
      {
        return collection_compares_with_iter_U<Dimensions<1>, U, comp, a, i>();
      }
      else
      {
        return true;
  ...
  /**
   * \brief Compares two \ref pattern_collection objects.
   * \details Corresponding \ref pattern "patterns" in each argument are compared.
   * Trailing 1D patterns are ignored.
  ...
  concept collection_compares_with =
#else
