 ...
 * \internal
 * \file
 * \brief Definition of \ref patterns::Any.
 */
...
namespace OpenKalman::patterns
{
  /**
   * \internal
   * \brief A type representing any \ref patterns::descriptor object.
   * \tparam S The value type for elements associated with this \ref patterns::pattern object.
   */
  ...
  private:
    ...
    struct Derived : Base
    {
      ...
      [[nodiscard]] Getter to_stat_space(Getter g) const final
      {
        ...
        {
          return [stat_data = patterns::to_stat_space(my_t, collections::views::generate(std::move(g), get_dimension(my_t)))]
      ...
      [[nodiscard]] Getter from_stat_space(Getter g) const final
      {
        ...
        {
          return [data = patterns::from_stat_space(my_t, collections::views::generate(std::move(g), get_stat_dimension(my_t)))]
      ...
      [[nodiscard]] Getter wrap(Getter g) const final
      {
        ...
        {
          return [data = patterns::wrap(my_t, collections::views::generate(std::move(g), get_dimension(my_t)))]
            (std::size_t i) -> S { return collections::get_element(data, i); };
        }
      }
  ...
  public:
    ...
    /**
     * \brief Construct from a \ref patterns::descriptor.
     */
#ifdef __cpp_concepts
...
#endif
    friend struct interface::pattern_descriptor_traits;
...
namespace OpenKalman::interface
{
  /**
   * \internal
   * \brief traits for patterns::Any.
   */
  template<typename S>
  struct pattern_descriptor_traits<patterns::Any<S>>
  {
  private:

    using T = patterns::Any<S>;
...
namespace std
{
  ...
  struct common_type<OpenKalman::patterns::Any<Scalar1>, OpenKalman::patterns::Any<Scalar2>>
    : std::conditional_t<
  ...
  struct common_type<OpenKalman::patterns::Any<S>, T>
    : std::conditional_t<
      OpenKalman::patterns::descriptor<T>,
      OpenKalman::stdex::type_identity<OpenKalman::patterns::Any<S>>,
      std::monostate> {};
}
