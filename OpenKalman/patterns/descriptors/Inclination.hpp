...
namespace OpenKalman::patterns
{
  ...
  template<values::fixed Down = values::fixed_pi<long double>> requires
    (values::fixed_value_of_v<Down> > 0) and (not values::complex<Down>) and
    std::convertible_to<values::value_type_of_t<Down>, float>
#else
  ...
  namespace inclination
  {
    ...
    using Degrees = Inclination<values::fixed_value<long double, 180>>;
    ...
    /**
     * \brief T is a \ref patterns::pattern object representing an inclination.
     */
    template<typename T>
#ifdef __cpp_concepts
    concept inclination =
...
namespace OpenKalman::interface
{
   ...
   */
  template<typename Down>
  struct pattern_descriptor_traits<patterns::Inclination<Down>>
  {
  private:

    using T = patterns::Inclination<Down>;
  ...
  public:
    ...
    {
      constexpr auto a = static_cast<float>(down);
      constexpr auto bits = std::numeric_limits<std::size_t>::digits;
      if constexpr (bits < 32)
        return std::integral_constant<std::size_t, 0x8CE6_uz + static_cast<std::size_t>(a * a * 0x1.p2f)>{};
      else if constexpr (bits < 64)
        return std::integral_constant<std::size_t, 0x8CE6267E_uz + static_cast<std::size_t>(a * a * 0x1.p4f)>{};
      else
        return std::integral_constant<std::size_t, 0x8CE6267E341642F7_uz + static_cast<std::size_t>(a * a * 0x1.p8f)>{};
    };
...
namespace std
{
  ...
  struct common_type<OpenKalman::patterns::Inclination<Down1>, OpenKalman::patterns::Inclination<Down2>>
    : std::conditional<
  ...
  struct common_type<OpenKalman::patterns::Inclination<Down>, OpenKalman::patterns::Any<Scalar>>
    : common_type<OpenKalman::patterns::Any<Scalar>, OpenKalman::patterns::Inclination<Down>> {};


  template<typename Down, typename T>
  struct common_type<OpenKalman::patterns::Inclination<Down>, T>
    : std::conditional_t<
