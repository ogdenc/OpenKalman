...
namespace OpenKalman::interface
{
   ...
   */
  template<>
  struct pattern_descriptor_traits<patterns::Distance>
  {
  private:

    using T = patterns::Distance;
  ...
  public:
    ...
    static constexpr auto hash_code = [](const T&)
    {
      ...
      if constexpr (bits < 32)
        return std::integral_constant<std::size_t, 0xBD0A_uz>{};
      else if constexpr (bits < 64)
        return std::integral_constant<std::size_t, 0xBD0A6689_uz>{};
      else
        return std::integral_constant<std::size_t, 0xBD0A668977D34578_uz>{};
    };
...
{
  template<>
  struct common_type<OpenKalman::patterns::Distance, OpenKalman::patterns::Distance>
  {
    using type = OpenKalman::patterns::Distance;
  };


  template<typename Scalar>
  struct common_type<OpenKalman::patterns::Distance, OpenKalman::patterns::Any<Scalar>>
    : common_type<OpenKalman::patterns::Any<Scalar>, OpenKalman::patterns::Distance> {};


  template<typename T>
  struct common_type<OpenKalman::patterns::Distance, T>
    : std::conditional_t<
