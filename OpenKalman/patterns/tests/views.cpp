...
/**
 * \file
 * \brief Tests for \ref patterns::pattern views.
 */
...
#include "patterns/views/concat.hpp"

TEST(patterns, fixed_concatenation)
{
  using patterns::views::concat;
...
TEST(patterns, dynamic_concatenation)
{
  using patterns::views::concat;
  static_assert(compare(concat(Dimensions{3}, Dimensions{4}), Dimensions{7}));
  ...
  EXPECT_EQ(patterns::internal::get_descriptor_hash_code(*ita), patterns::internal::get_descriptor_hash_code(Any<>{Axis{}}));
...
#include "patterns/views/replicate.hpp"

TEST(patterns, replicate)
{
  using patterns::views::replicate;

  static_assert(patterns::dimension_of_v<decltype(replicate(Dimensions<3>{}, std::integral_constant<std::size_t, 4>{}))> == 12);
  static_assert(patterns::get_dimension(replicate(Dimensions<3>{}, 4u))== 12);
  static_assert(patterns::get_dimension(replicate(Dimensions{3}, std::integral_constant<std::size_t, 4>{})) == 12);
  static_assert(patterns::get_dimension(replicate(Dimensions{3}, 4_uz)) == 12);
  static_assert(patterns::get_dimension(replicate(Dimensions{3}, 4u)) == 12);
...
#include "patterns/views/dimensions.hpp"

TEST(patterns, views_dimensions)
{
  ...
  using patterns::views::dimensions;
