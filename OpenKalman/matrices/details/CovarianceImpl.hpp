...
namespace OpenKalman::internal
{
  ...
  struct CovarianceImpl : OpenKalman::internal::CovarianceBase<Derived, NestedMatrix>
  {
  ...
  private:
    ...
    using Base = OpenKalman::internal::CovarianceBase<Derived, NestedMatrix>;
  ...
  public:
    ...
    {
      using Derived2 = decltype(MatrixTraits<Derived>::make(f1(nested_matrix())));
      using R = typename MatrixTraits<Derived2>::SelfContainedFrom;
      if (synchronization_direction() >= 0)
      {
        R r {f1(nested_matrix())};
        if constexpr (not OpenKalman::internal::case1or2<Derived, decltype(f1(nested_matrix()))>)
        {
    ...
    auto covariance_op(const F1& f1, const F2& f2) const &&
    {
      using Derived2 = decltype(MatrixTraits<Derived>::make(f1(std::move(*this).nested_matrix())));
      using R = typename MatrixTraits<Derived2>::SelfContainedFrom;
      if (synchronization_direction() >= 0)
      {
        R r {f1(std::move(*this).nested_matrix())};
        if constexpr (not OpenKalman::internal::case1or2<Derived, decltype(f1(std::move(*this).nested_matrix()))>)
        {
          if (r.synchronization_direction() > 0 and synchronization_direction() <= 0)
          {
            r.cholesky_nested_matrix() =
  ...
  public:
    ...
    {
      return this->get_nested_matrix_impl<self_adjoint_matrix<NestedMatrix>>();
    }
    ...
    decltype(auto) get_self_adjoint_nested_matrix() &&
    {
      return std::move(*this).template get_nested_matrix_impl<self_adjoint_matrix<NestedMatrix>>();
    }
    ...
    auto operator() (std::size_t i, std::size_t j) requires
      element_settable<NestedMatrix, 2> and element_gettable<NestedMatrix, 2>
#else
