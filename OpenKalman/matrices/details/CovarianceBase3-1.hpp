...
#ifdef __cpp_concepts
    template<covariance Arg> requires case1or2<Arg> and
      (triangular_matrix<nested_matrix_t<Arg>> != triangular_matrix<NestedMatrix> or diagonal_matrix<Arg>)
#else
    template<typename Arg, std::enable_if_t<covariance<Arg> and case1or2<Arg> and
      (triangular_matrix<nested_matrix_t<Arg>> != triangular_matrix<NestedMatrix> or diagonal_matrix<Arg>), int> = 0>
#endif
    CovarianceBase(Arg&& arg) noexcept
...
#endif //OPENKALMAN_COVARIANCEBASE3_1_HPP
