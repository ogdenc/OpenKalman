...
namespace OpenKalman
{
  ...
  public:
    ...
    template<typename Arg, std::enable_if_t<typed_matrix<Arg> and not euclidean_transformed<Arg> and not mean<Arg> and
      equivalent_to<row_index_descriptor_of_t<Arg>, RowCoefficients> and
      equivalent_to<column_index_descriptor_of_t<Arg>, ColumnCoefficients>, int> = 0>
#endif
    ...
    template<std::convertible_to<const Scalar> ... Args> requires (sizeof...(Args) > 0) and
      requires(Args ... args) { NestedMatrix {wrap_angles<RowCoefficients>(
        make_dense_writable_matrix_from<NestedMatrix>(static_cast<const Scalar>(args)...))}; }
#else
...
#endif
    Mean(Args ... args)
      : Base {wrap_angles<RowCoefficients>(make_dense_writable_matrix_from<NestedMatrix>(static_cast<const Scalar>(args)...))} {}
    ...
    template<typed_matrix Arg> requires (not mean<Arg>) and (not euclidean_transformed<Arg>) and
      equivalent_to<row_index_descriptor_of_t<Arg>, RowCoefficients> and has_untyped_index<Arg, 1> and
    ...
    template<typename Arg, std::enable_if_t<typed_matrix<Arg> and
      ...
      equivalent_to<row_index_descriptor_of_t<Arg>, RowCoefficients> and has_untyped_index<Arg, 1>, int> = 0>
#endif
    auto& operator=(Arg&& other) noexcept
    {
      if constexpr (not zero_matrix<NestedMatrix> and not identity_matrix<NestedMatrix>)
...
#endif
  Mean(V&&) -> Mean<row_index_descriptor_of_t<V>, std::decay_t<decltype(
    wrap_angles<row_index_descriptor_of_t<V>>(nested_matrix(std::forward<V>(std::declval<V>()))))>>;
...
#endif
  Mean(V&&) -> Mean<row_index_descriptor_of_t<V>, std::decay_t<decltype(
    from_euclidean<row_index_descriptor_of_t<V>>(nested_matrix(std::forward<V>(std::declval<V>()))))>>;
  ...
  namespace interface
  {
    ...
    struct IndexibleObjectTraits<Mean<Coeffs, NestedMatrix>>
    {
