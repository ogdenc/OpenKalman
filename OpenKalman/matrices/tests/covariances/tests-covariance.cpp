...
TEST(covariance_tests, Covariance_convert_nested_matrix)
{
  ...
  EXPECT_TRUE(is_near(to_covariance_nestable<SA2l>(make_dense_writable_matrix_from<M2>(9, 3, 3, 10)), Mat2 {9, 3, 3, 10}));
  EXPECT_TRUE(is_near(to_covariance_nestable<SA2u>(make_dense_writable_matrix_from<M2>(9, 3, 3, 10)), Mat2 {9, 3, 3, 10}));
  EXPECT_TRUE(is_near(to_covariance_nestable<T2l>(make_dense_writable_matrix_from<M2>(3, 0, 1, 3)), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(to_covariance_nestable<T2u>(make_dense_writable_matrix_from<M2>(3, 1, 0, 3)), Mat2 {3, 1, 0, 3}));
...
TEST(covariance_tests, Covariance_class)
{
  ...
  */
  ...
  CovSA2l clsa7(make_dense_writable_matrix_from<M2>(9, 7, 3, 10)); EXPECT_TRUE(is_near(clsa7, Mat2 {9, 3, 3, 10}));
  CovSA2u cusa7(make_dense_writable_matrix_from<M2>(4, 2, 7, 5)); EXPECT_TRUE(is_near(cusa7, Mat2 {4, 2, 2, 5}));
  CovT2l clt7(make_dense_writable_matrix_from<M2>(9, 7, 3, 10)); EXPECT_TRUE(is_near(clt7, Mat2 {9, 3, 3, 10}));
  CovT2u cut7(make_dense_writable_matrix_from<M2>(4, 2, 7, 5)); EXPECT_TRUE(is_near(cut7, Mat2 {4, 2, 2, 5}));
  ...
  */
  ...
  clsa2 *= 0; EXPECT_TRUE(is_near(clsa2, make_zero_matrix_like<Mat2>()));
  cusa2 *= 0; EXPECT_TRUE(is_near(cusa2, make_zero_matrix_like<Mat2>()));
  clt2 *= 0; EXPECT_TRUE(is_near(clt2, make_zero_matrix_like<Mat2>()));
  cut2 *= 0; EXPECT_TRUE(is_near(cut2, make_zero_matrix_like<Mat2>()));
  cd2 *= 0; EXPECT_TRUE(is_near(cd2, make_zero_matrix_like<Mat2>()));

  // Zero
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovSA2l>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovSA2u>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovT2l>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovT2u>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovD2>(), M2::Zero()));
  EXPECT_TRUE(is_near(square_root(make_zero_matrix_like<CovSA2l>()), M2::Zero()));
  EXPECT_TRUE(is_near(square_root(make_zero_matrix_like<CovSA2u>()), M2::Zero()));
  EXPECT_TRUE(is_near(square_root(make_zero_matrix_like<CovT2l>()), M2::Zero()));
  EXPECT_TRUE(is_near(square_root(make_zero_matrix_like<CovT2u>()), M2::Zero()));
  EXPECT_TRUE(is_near(square_root(make_zero_matrix_like<CovD2>()), M2::Zero()));
...
TEST(covariance_tests, Covariance_deduction_guides)
{
  ...
  static_assert(equivalent_to<row_coefficient_types_of_t<decltype(Covariance(T2l {3, 0, 1, 3}))>, Axes<2>>);

  EXPECT_TRUE(is_near(Covariance(D2 {1, 2}), Mat2 {1, 0, 0, 2}));
  static_assert(equivalent_to<row_coefficient_types_of_t<decltype(Covariance(D2 {1, 2}))>, Axes<2>>);
  ...
  static_assert(equivalent_to<row_coefficient_types_of_t<decltype(Covariance(make_dense_writable_matrix_from<M2>(9, 3, 3, 10)))>, Axes<2>>);

  EXPECT_TRUE(is_near(Covariance {9., 3, 3, 10}, Mat2 {9, 3, 3, 10}));
  static_assert(equivalent_to<row_coefficient_types_of_t<decltype(Covariance {9., 3, 3, 10})>, Axes<2>>);
}
...
TEST(covariance_tests, Covariance_make)
{
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance(CovSA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance(CovSA2u {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(lower_triangular_matrix<decltype(make_covariance(CovT2l {9, 3, 3, 10}).get_triangular_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance(adjoint(CovSA2l {9, 3, 3, 10})).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance(adjoint(CovSA2u {9, 3, 3, 10})).get_self_adjoint_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<CovSA2l>().get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance<CovSA2u>().get_self_adjoint_nested_matrix())>);
  static_assert(lower_triangular_matrix<decltype(make_covariance<CovT2l>().get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<CovT2u>().get_triangular_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C>(SA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance<C>(SA2u {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(lower_triangular_matrix<decltype(make_covariance<C>(T2l {3, 0, 1, 3}).get_triangular_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(adjoint(make_covariance<C>(SA2l {9, 3, 3, 10})).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance<C>(adjoint(SA2u {9, 3, 3, 10})).get_self_adjoint_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C, SA2l>().get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance<C, SA2u>().get_self_adjoint_nested_matrix())>);
  static_assert(lower_triangular_matrix<decltype(make_covariance<C, T2l>().get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<C, T2u>().get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<C, D2>().get_triangular_nested_matrix())>);

  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance(SA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::upper_self_adjoint_matrix<decltype(make_covariance<C>(SA2u {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<T2u>().get_triangular_nested_matrix())>);
  static_assert(lower_triangular_matrix<decltype(make_covariance<C, T2l>().get_triangular_nested_matrix())>);
  static_assert(diagonal_matrix<decltype(make_covariance<C, D2>().get_self_adjoint_nested_matrix())>);

  // Regular matrices:
  EXPECT_TRUE(is_near(make_covariance<C, TriangleType::lower>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(make_covariance<C, TriangleType::upper>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_triangular_nested_matrix(), Mat2 {3, 1, 0, 3}));
  EXPECT_TRUE(is_near(make_covariance<C>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_self_adjoint_nested_matrix(), Mat2 {9, 3, 3, 10}));

  static_assert(lower_triangular_matrix<decltype(make_covariance<C, TriangleType::lower>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<C, TriangleType::upper>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_triangular_nested_matrix())>);

  static_assert(lower_triangular_matrix<decltype(make_covariance<C, TriangleType::lower, M2>().get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<C, TriangleType::upper, M2>().get_triangular_nested_matrix())>);

  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_self_adjoint_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<TriangleType::upper>(Mat2 {9, 3, 3, 10}.nested_matrix()).get_triangular_nested_matrix())>);
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C, M2>().get_self_adjoint_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<TriangleType::upper, M2>().get_triangular_nested_matrix())>);
  ...
  static_assert(lower_triangular_matrix<decltype(make_covariance<TriangleType::lower>(Mat2 {9, 3, 3, 10}).get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<TriangleType::upper>(Mat2 {9, 3, 3, 10}).get_triangular_nested_matrix())>);

  static_assert(lower_triangular_matrix<decltype(make_covariance<TriangleType::lower, Mat2>().get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<TriangleType::upper, Mat2>().get_triangular_nested_matrix())>);

  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance(Mat2 {9, 3, 3, 10}).get_self_adjoint_nested_matrix())>);
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<Mat2>().get_self_adjoint_nested_matrix())>);
  ...
  static_assert(lower_triangular_matrix<decltype(make_covariance<C, TriangleType::lower>(9., 3, 3, 10).get_triangular_nested_matrix())>);
  static_assert(upper_triangular_matrix<decltype(make_covariance<C, TriangleType::upper>(9., 3, 3, 10).get_triangular_nested_matrix())>);
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C>(9., 3, 3, 10).get_self_adjoint_nested_matrix())>);

  static_assert(lower_triangular_matrix<decltype(make_covariance<TriangleType::lower>(9., 3, 3, 10).get_triangular_nested_matrix())>);
  ...
  static_assert(Eigen3::lower_self_adjoint_matrix<decltype(make_covariance<C>().get_self_adjoint_nested_matrix())>);
  static_assert(row_coefficient_types_of_t<decltype(make_covariance<C>())>::dimension == 2);
}
...
TEST(covariance_tests, Covariance_traits)
{
  ...
  // MatrixTraits
  EXPECT_TRUE(is_near(MatrixTraits<CovSA2l>::make(SA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix(), Mat2 {9, 3, 3, 10}));
  ...
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovSA2l>(), eigen_matrix_t<double, 2, 2>::Zero()));
  ...
  EXPECT_TRUE(is_near(make_zero_matrix_like<CovT2l>(), eigen_matrix_t<double, 2, 2>::Zero()));
...
TEST(covariance_tests, Covariance_overloads)
{
  ...
  EXPECT_TRUE(is_near(make_dense_writable_matrix_from(CovSA2l {9, 3, 3, 10}), Mat2 { 9, 3, 3, 10}));
  EXPECT_TRUE(is_near(make_dense_writable_matrix_from(CovSA2u {9, 3, 3, 10}), Mat2 { 9, 3, 3, 10}));
  EXPECT_TRUE(is_near(make_dense_writable_matrix_from(CovT2l {9, 3, 3, 10}), Mat2 { 9, 3, 3, 10}));
  EXPECT_TRUE(is_near(make_dense_writable_matrix_from(CovT2l {9, 3, 3, 10}), Mat2 { 9, 3, 3, 10}));
...
TEST(covariance_tests, Covariance_blocks)
{
  ...
  EXPECT_TRUE(is_near(split_vertical<C, C>(CovSA4l(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), concatenate_horizontal(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(CovSA4u(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), concatenate_horizontal(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(CovT4l(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), concatenate_horizontal(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(CovT4u(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), concatenate_horizontal(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_vertical<C, C::Take<1>>(CovSA4l(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_vertical<C, C::Take<1>>(CovSA4u(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_vertical<C, C::Take<1>>(CovT4l(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_vertical<C, C::Take<1>>(CovT4u(n)), std::tuple {concatenate_horizontal(m1, make_zero_matrix_like<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 4, 2}}));

  EXPECT_TRUE(is_near(split_horizontal(CovSA4l(n)), std::tuple {}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(CovSA4l(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), concatenate_vertical(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(CovSA4u(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), concatenate_vertical(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(CovT4l(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), concatenate_vertical(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(CovT4u(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), concatenate_vertical(make_zero_matrix_like<Mat2>(), m2)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C::Take<1>>(CovSA4l(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_horizontal<C, C::Take<1>>(CovSA4u(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_horizontal<C, C::Take<1>>(CovT4l(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 4, 2}}));
  EXPECT_TRUE(is_near(split_horizontal<C, C::Take<1>>(CovT4u(n)), std::tuple {concatenate_vertical(m1, make_zero_matrix_like<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 4, 2}}));
  ...
  EXPECT_TRUE(is_near(apply_columnwise([](auto col){ return col * 2; }, CovSA2l(m1)), Mat2 {18, 6, 6, 20}));
  EXPECT_TRUE(is_near(apply_columnwise([](const auto col){ return col * 2; }, CovSA2u(m1)), Mat2 {18, 6, 6, 20}));
  EXPECT_TRUE(is_near(apply_columnwise([](auto&& col){ return col * 2; }, CovT2l(m1)), Mat2 {18, 6, 6, 20}));
  EXPECT_TRUE(is_near(apply_columnwise([](const auto& col){ return col * 2; }, CovT2u(m1)), Mat2 {18, 6, 6, 20}));

  EXPECT_TRUE(is_near(apply_columnwise([](auto col, std::size_t i){ return col * i; }, CovSA2l(m1)), Mat2 {0, 3, 0, 10}));
  EXPECT_TRUE(is_near(apply_columnwise([](const auto col, std::size_t i){ return col * i; }, CovSA2u(m1)), Mat2 {0, 3, 0, 10}));
  EXPECT_TRUE(is_near(apply_columnwise([](auto&& col, std::size_t i){ return col * i; }, CovT2l(m1)), Mat2 {0, 3, 0, 10}));
  EXPECT_TRUE(is_near(apply_columnwise([](const auto& col, std::size_t i){ return col * i; }, CovT2u(m1)), Mat2 {0, 3, 0, 10}));

  EXPECT_TRUE(is_near(apply_coefficientwise([](auto x){ return x + 1; }, CovSA2l(m1)), Mat2 {10, 4, 4, 11}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto x){ return x + 1; }, CovSA2u(m1)), Mat2 {10, 4, 4, 11}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](auto&& x){ return x + 1; }, CovT2l(m1)), Mat2 {10, 4, 4, 11}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto& x){ return x + 1; }, CovT2u(m1)), Mat2 {10, 4, 4, 11}));

  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto& x, std::size_t i, std::size_t j){ return x + i + j; }, CovSA2l(m1)), Mat2 {9, 4, 4, 12}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto& x, std::size_t i, std::size_t j){ return x + i + j; }, CovSA2u(m1)), Mat2 {9, 4, 4, 12}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto& x, std::size_t i, std::size_t j){ return x + i + j; }, CovT2l(m1)), Mat2 {9, 4, 4, 12}));
  EXPECT_TRUE(is_near(apply_coefficientwise([](const auto& x, std::size_t i, std::size_t j){ return x + i + j; }, CovT2u(m1)), Mat2 {9, 4, 4, 12}));
}
