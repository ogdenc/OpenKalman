...
TEST(covariance_tests, SquareRootCovariance_convert_nested_matrix)
{
  ...
  EXPECT_TRUE(is_near(to_covariance_nestable(SqCovD2 {1, 2}).nested_object(), Mean {1., 2}));
...
TEST(covariance_tests, SquareRootCovariance_class)
{
  ...
  */
  ...
  SqCovSA2l clsa7(make_dense_object_from<M2>(3, 7, 1, 3));
  EXPECT_TRUE(is_near(clsa7, Mat2 {3, 0, 1, 3}));
  SqCovSA2u cusa7(make_dense_object_from<M2>(2, 1, 7, 2));
  EXPECT_TRUE(is_near(cusa7, Mat2 {2, 1, 0, 2}));
  SqCovT2l clt7(make_dense_object_from<M2>(3, 7, 1, 3));
  EXPECT_TRUE(is_near(clt7, Mat2 {3, 0, 1, 3}));
  SqCovT2u cut7(make_dense_object_from<M2>(2, 1, 7, 2));
  ...
  */
  ...
  clsa2 *= 0;
  EXPECT_TRUE(is_near(clsa2, make_zero<Mat2>()));
  cusa2 *= 0;
  EXPECT_TRUE(is_near(cusa2, make_zero<Mat2>()));
  clt2 *= 0;
  EXPECT_TRUE(is_near(clt2, make_zero<Mat2>()));
  cut2 *= 0;
  EXPECT_TRUE(is_near(cut2, make_zero<Mat2>()));
  cd2 *= 0;
  EXPECT_TRUE(is_near(cd2, make_zero<Mat2>()));
  ...
  // Zero
  EXPECT_TRUE(is_near(make_zero<SqCovSA2l>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero<SqCovSA2u>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero<SqCovT2l>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero<SqCovT2u>(), M2::Zero()));
  EXPECT_TRUE(is_near(make_zero<SqCovD2>(), M2::Zero()));
  EXPECT_TRUE(is_near(square(make_zero<SqCovSA2l>()), M2::Zero()));
  EXPECT_TRUE(is_near(square(make_zero<SqCovSA2u>()), M2::Zero()));
  EXPECT_TRUE(is_near(square(make_zero<SqCovT2l>()), M2::Zero()));
  EXPECT_TRUE(is_near(square(make_zero<SqCovT2u>()), M2::Zero()));
  EXPECT_TRUE(is_near(square(make_zero<SqCovD2>()), M2::Zero()));
...
TEST(covariance_tests, SquareRootCovariance_subscripts)
{
  ...
  EXPECT_NEAR(get_component(SqCovSA2l {3, 0, 1, 3}, 0, 0), 3, 1e-6);
  EXPECT_NEAR(get_component(SqCovSA2u {3, 1, 0, 3}, 0, 1), 1, 1e-6);
  EXPECT_NEAR(get_component(SqCovT2l {3, 0, 1, 3}, 1, 0), 1, 1e-6);
  EXPECT_NEAR(get_component(SqCovT2u {3, 1, 0, 3}, 1, 1), 3, 1e-6);
  EXPECT_NEAR(get_component(SqCovD2 {3, 4}, 0, 0), 3, 1e-6);
  EXPECT_NEAR(get_component(SqCovD2 {3, 4}, 1, 1), 4, 1e-6);
  EXPECT_NEAR(get_component(SqCovD2 {3, 4}, 0), 3, 1e-6);
  EXPECT_NEAR(get_component(SqCovD2 {3, 4}, 1), 4, 1e-6);

  auto sa2l = SqCovSA2l {3, 0, 1, 3};
  sa2l(1, 0) = 1.1; EXPECT_NEAR(get_component(sa2l, 1, 0), 1.1, 1e-6);
  set_component(sa2l, 1.2, 1, 0); EXPECT_NEAR(sa2l(1, 0), 1.2, 1e-6);
  auto sa2u = SqCovSA2u {3, 1, 0, 3};
  sa2u(0, 1) = 1.1; EXPECT_NEAR(get_component(sa2u, 0, 1), 1.1, 1e-6);
  set_component(sa2u, 1.2, 0, 1); EXPECT_NEAR(sa2u(0, 1), 1.2, 1e-6);
  auto t2l = SqCovT2l {3, 0, 1, 3};
  t2l(1, 0) = 1.1; EXPECT_NEAR(get_component(t2l, 1, 0), 1.1, 1e-6);
  set_component(t2l, 1.2, 1, 0); EXPECT_NEAR(t2l(1, 0), 1.2, 1e-6);
  auto t2u = SqCovT2u {3, 1, 0, 3};
  t2u(0, 1) = 1.1; EXPECT_NEAR(get_component(t2u, 0, 1), 1.1, 1e-6);
  set_component(t2u, 1.2, 0, 1); EXPECT_NEAR(t2u(0, 1), 1.2, 1e-6);
  auto d2 = SqCovD2 {3, 4};
  d2(1, 1) = 4.1; EXPECT_NEAR(get_component(d2, 1, 1), 4.1, 1e-6);
  set_component(d2, 4.2, 1, 1); EXPECT_NEAR(d2(1, 1), 4.2, 1e-6);
  d2(0) = 3.1; EXPECT_NEAR(get_component(d2, 0), 3.1, 1e-6);
  set_component(d2, 3.2, 0); EXPECT_NEAR(d2(0), 3.2, 1e-6);
}
...
TEST(covariance_tests, SquareRootCovariance_make)
{
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance(SqCovSA2l {3, 0, 1, 3}).get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance(SqCovSA2u {3, 1, 0, 3}).get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance(SqCovT2l {3, 0, 1, 3}).get_triangular_nested_matrix()), triangle_type::lower>);
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance(conjugate_transpose(SqCovSA2l {3, 0, 1, 3})).get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance(conjugate_transpose(SqCovSA2u {3, 1, 0, 3})).get_self_adjoint_nested_matrix()), triangle_type::lower>);
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<SqCovSA2l>().get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<SqCovSA2u>().get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<SqCovT2l>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<SqCovT2u>().get_triangular_nested_matrix()), triangle_type::upper>);
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C>(SA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C>(SA2u {9, 3, 3, 10}).get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C>(T2l {3, 0, 1, 3}).get_triangular_nested_matrix()), triangle_type::lower>);
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C>(conjugate_transpose(SA2l {9, 3, 3, 10})).get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C>(conjugate_transpose(SA2u {9, 3, 3, 10})).get_self_adjoint_nested_matrix()), triangle_type::upper>);
  ...
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C, SA2l>().get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C, SA2u>().get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, T2l>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, T2u>().get_triangular_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, D2>().get_triangular_nested_matrix()), triangle_type::lower>);

  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance(SA2l {9, 3, 3, 10}).get_self_adjoint_nested_matrix()), triangle_type::lower>);
  static_assert(hermitian_adapter_concept<decltype(make_square_root_covariance<C>(SA2u {9, 3, 3, 10}).get_self_adjoint_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<T2u>().get_triangular_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, T2l>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(diagonal_matrix<decltype(make_square_root_covariance<C, D2>().get_triangular_nested_matrix())>);

  // Regular matrices:
  EXPECT_TRUE(is_near(make_square_root_covariance<C, triangle_type::lower>(Mat2 {3, 0, 1, 3}.nested_object()).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance<C, triangle_type::upper>(Mat2 {3, 1, 0, 3}.nested_object()).get_triangular_nested_matrix(), Mat2 {3, 1, 0, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance<C>(Mat2 {3, 0, 1, 3}.nested_object()).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));

  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::lower>(Mat2 {3, 0, 1, 3}.nested_object()).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::upper>(Mat2 {3, 1, 0, 3}.nested_object()).get_triangular_nested_matrix()), triangle_type::upper>);

  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::lower, M2>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::upper, M2>().get_triangular_nested_matrix()), triangle_type::upper>);

  static_assert(triangular_matrix<decltype(make_square_root_covariance<C>(Mat2 {3, 0, 1, 3}.nested_object()).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::upper>(Mat2 {3, 1, 0, 3}.nested_object()).get_triangular_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, M2>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::upper, M2>().get_triangular_nested_matrix()), triangle_type::upper>);

  // Typed matrices:
  EXPECT_TRUE(is_near(make_square_root_covariance<triangle_type::lower>(Mat2 {3, 0, 1, 3}).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance<triangle_type::upper>(Mat2 {3, 1, 0, 3}).get_triangular_nested_matrix(), Mat2 {3, 1, 0, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance(Mat2 {3, 0, 1, 3}).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));

  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::lower>(Mat2 {3, 0, 1, 3}).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::upper>(Mat2 {3, 1, 0, 3}).get_triangular_nested_matrix()), triangle_type::upper>);

  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::lower, Mat2>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::upper, Mat2>().get_triangular_nested_matrix()), triangle_type::upper>);

  static_assert(triangular_matrix<decltype(make_square_root_covariance(Mat2 {3, 0, 1, 3}).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<Mat2>().get_triangular_nested_matrix()), triangle_type::lower>);

  // Eigen defaults
  EXPECT_TRUE(is_near(make_square_root_covariance<C, triangle_type::lower>(3., 0, 1, 3).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance<C, triangle_type::upper>(3., 1, 0, 3).get_triangular_nested_matrix(), Mat2 {3, 1, 0, 3}));
  EXPECT_TRUE(is_near(make_square_root_covariance<C>(3., 0, 1, 3).get_triangular_nested_matrix(), Mat2 {3, 0, 1, 3}));

  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::lower>(3., 0, 1, 3).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::upper>(3., 1, 0, 3).get_triangular_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C>(9., 3, 3, 10).get_triangular_nested_matrix()), triangle_type::lower>);

  static_assert(triangular_matrix<decltype(make_square_root_covariance<triangle_type::lower>(3., 0, 1, 3).get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::lower>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C, triangle_type::upper>().get_triangular_nested_matrix()), triangle_type::upper>);
  static_assert(triangular_matrix<decltype(make_square_root_covariance<C>().get_triangular_nested_matrix()), triangle_type::lower>);
  static_assert(dimension_size_of_vvector_space_descriptor_of_t<decltype(make_square_root_covariance<C>()), 0>> == 2);
}
...
TEST(covariance_tests, SquareRootCovariance_traits)
{
  ...
  static_assert(not hermitian_matrix<SqCovSA2l>);
  ...
  static_assert(triangular_matrix<SqCovSA2l, triangle_type::lower>);
  static_assert(triangular_matrix<SqCovSA2u, triangle_type::upper>);
  static_assert(not identity_matrix<SqCovSA2l>);
  static_assert(not zero<SqCovSA2l>);
  ...
  static_assert(not hermitian_matrix<SqCovT2l>);
  static_assert(cholesky_form<SqCovT2l>);
  static_assert(triangular_matrix<SqCovT2l>);
  static_assert(triangular_matrix<SqCovT2u>);
  static_assert(triangular_matrix<SqCovT2l, triangle_type::lower>);
  static_assert(not triangular_matrix<SqCovT2l, triangle_type::upper>);
  static_assert(triangular_matrix<SqCovT2u, triangle_type::upper>);
  static_assert(not identity_matrix<SqCovT2l>);
  static_assert(not zero<SqCovT2l>);

  static_assert(triangular_covariance<SqCovD2>);
  static_assert(diagonal_matrix<SqCovD2>);
  static_assert(hermitian_matrix<SqCovD2>);
  static_assert(not cholesky_form<SqCovD2>);
  static_assert(triangular_matrix<SqCovD2>);
  static_assert(triangular_matrix<SqCovD2, triangle_type::lower>);
  static_assert(triangular_matrix<SqCovD2, triangle_type::upper>);
  static_assert(not identity_matrix<SqCovD2>);
  static_assert(not zero<SqCovD2>);

  static_assert(triangular_covariance<SqCovI2>);
  static_assert(diagonal_matrix<SqCovI2>);
  static_assert(hermitian_matrix<SqCovI2>);
  static_assert(not cholesky_form<SqCovI2>);
  static_assert(triangular_matrix<SqCovI2>);
  static_assert(triangular_matrix<SqCovI2, triangle_type::lower>);
  static_assert(triangular_matrix<SqCovI2, triangle_type::upper>);
  static_assert(identity_matrix<SqCovI2>);
  static_assert(not zero<SqCovI2>);

  static_assert(triangular_covariance<SqCovZ2>);
  static_assert(diagonal_matrix<SqCovZ2>);
  static_assert(hermitian_matrix<SqCovZ2>);
  static_assert(not cholesky_form<SqCovZ2>);
  static_assert(triangular_matrix<SqCovZ2>);
  static_assert(triangular_matrix<SqCovZ2, triangle_type::lower>);
  static_assert(triangular_matrix<SqCovZ2, triangle_type::upper>);
  static_assert(not identity_matrix<SqCovZ2>);
  static_assert(zero<SqCovZ2>);

  EXPECT_TRUE(is_near(make_zero<SqCovSA2l>(), eigen_matrix_t<double, 2, 2>::Zero()));
  EXPECT_TRUE(is_near(make_identity_matrix_like<SqCovSA2l>(), eigen_matrix_t<double, 2, 2>::Identity()));
  EXPECT_TRUE(is_near(make_zero<SqCovT2l>(), eigen_matrix_t<double, 2, 2>::Zero()));
...
TEST(covariance_tests, SquareRootCovariance_overloads)
{
  ...
  EXPECT_TRUE(is_near(nested_object(SqCovSA2l {3, 0, 1, 3}), Mat2 {9, 3, 3, 10}));
  EXPECT_TRUE(is_near(nested_object(SqCovSA2u {3, 1, 0, 3}), Mat2 {9, 3, 3, 10}));
  EXPECT_TRUE(is_near(nested_object(SqCovT2l {3, 0, 1, 3}), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(nested_object(SqCovT2u {3, 1, 0, 3}), Mat2 {3, 1, 0, 3}));
  ...
  EXPECT_TRUE(is_near(square(SquareRootCovariance<C, zero_adapter<eigen_matrix_t<double, 2, 2>>>()), Mat2 {0, 0, 0, 0}));
  ...
  EXPECT_TRUE(is_near(make_dense_object_from(SqCovT2l {3, 0, 1, 3}), Mat2 {3, 0, 1, 3}));
  EXPECT_TRUE(is_near(make_dense_object_from(SqCovT2u {3, 1, 0, 3}), Mat2 {3, 1, 0, 3}));
...
TEST(covariance_tests, SquareRootCovariance_blocks)
{
  ...
  using SqCovSA4l = SquareRootCovariance<C4, hermitian_adapter<M4, triangle_type::lower>>;
  using SqCovSA4u = SquareRootCovariance<C4, hermitian_adapter<M4, triangle_type::upper>>;
  using SqCovT4l = SquareRootCovariance<C4, triangular_adapter<M4, triangle_type::lower>>;
  using SqCovT4u = SquareRootCovariance<C4, triangular_adapter<M4, triangle_type::upper>>;
  ...
  EXPECT_TRUE(is_near(split_diagonal<C, angle::Radians>(SqCovSA4l(nl)), std::tuple {m1l, Matrix<angle::Radians, angle::Radians>{2}}));
  EXPECT_TRUE(is_near(split_diagonal<C, angle::Radians>(SqCovSA4u(nu)), std::tuple {m1u, Matrix<angle::Radians, angle::Radians>{2}}));
  EXPECT_TRUE(is_near(split_diagonal<C, angle::Radians>(SqCovT4l(nl)), std::tuple {m1l, Matrix<angle::Radians, angle::Radians>{2}}));
  EXPECT_TRUE(is_near(split_diagonal<C, angle::Radians>(SqCovT4u(nu)), std::tuple {m1u, Matrix<angle::Radians, angle::Radians>{2}}));

  EXPECT_TRUE(is_near(split_vertical(SqCovSA4l(nl)), std::tuple {}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(SqCovSA4l(nl)), std::tuple {concatenate_horizontal(m1l, make_zero<Mat2>()), concatenate_horizontal(make_zero<Mat2>(), m2l)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(SqCovSA4u(nu)), std::tuple {concatenate_horizontal(m1u, make_zero<Mat2>()), concatenate_horizontal(make_zero<Mat2>(), m2u)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(SqCovT4l(nl)), std::tuple {concatenate_horizontal(m1l, make_zero<Mat2>()), concatenate_horizontal(make_zero<Mat2>(), m2l)}));
  EXPECT_TRUE(is_near(split_vertical<C, C>(SqCovT4u(nu)), std::tuple {concatenate_horizontal(m1u, make_zero<Mat2>()), concatenate_horizontal(make_zero<Mat2>(), m2u)}));
  EXPECT_TRUE(is_near(split_vertical<C, angle::Radians>(SqCovSA4l(nl)), std::tuple {concatenate_horizontal(m1l, make_zero<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 2, 0}}));
  EXPECT_TRUE(is_near(split_vertical<C, angle::Radians>(SqCovSA4u(nu)), std::tuple {concatenate_horizontal(m1u, make_zero<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 2, 1}}));
  EXPECT_TRUE(is_near(split_vertical<C, angle::Radians>(SqCovT4l(nl)), std::tuple {concatenate_horizontal(m1l, make_zero<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 2, 0}}));
  EXPECT_TRUE(is_near(split_vertical<C, angle::Radians>(SqCovT4u(nu)), std::tuple {concatenate_horizontal(m1u, make_zero<Mat2>()), Matrix<angle::Radians, C4>{0, 0, 2, 1}}));

  EXPECT_TRUE(is_near(split_horizontal(SqCovSA4l(nl)), std::tuple {}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(SqCovSA4l(nl)), std::tuple {concatenate_vertical(m1l, make_zero<Mat2>()), concatenate_vertical(make_zero<Mat2>(), m2l)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(SqCovSA4u(nu)), std::tuple {concatenate_vertical(m1u, make_zero<Mat2>()), concatenate_vertical(make_zero<Mat2>(), m2u)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(SqCovT4l(nl)), std::tuple {concatenate_vertical(m1l, make_zero<Mat2>()), concatenate_vertical(make_zero<Mat2>(), m2l)}));
  EXPECT_TRUE(is_near(split_horizontal<C, C>(SqCovT4u(nu)), std::tuple {concatenate_vertical(m1u, make_zero<Mat2>()), concatenate_vertical(make_zero<Mat2>(), m2u)}));
  EXPECT_TRUE(is_near(split_horizontal<C, angle::Radians>(SqCovSA4l(nl)), std::tuple {concatenate_vertical(m1l, make_zero<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 2, 1}}));
  EXPECT_TRUE(is_near(split_horizontal<C, angle::Radians>(SqCovSA4u(nu)), std::tuple {concatenate_vertical(m1u, make_zero<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 2, 0}}));
  EXPECT_TRUE(is_near(split_horizontal<C, angle::Radians>(SqCovT4l(nl)), std::tuple {concatenate_vertical(m1l, make_zero<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 2, 1}}));
  EXPECT_TRUE(is_near(split_horizontal<C, angle::Radians>(SqCovT4u(nu)), std::tuple {concatenate_vertical(m1u, make_zero<Mat2>()), Matrix<C4, angle::Radians>{0, 0, 2, 0}}));
