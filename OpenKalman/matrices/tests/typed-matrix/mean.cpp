...
TEST(matrices, Mean_overloads)
{
  ...
  EXPECT_TRUE(is_near(to_stat_space(
  ...
  EXPECT_TRUE(is_near(to_stat_space(ma), ma));
  ...
  EXPECT_TRUE(is_near(to_stat_space(mb).nested_object(), x2));
  ...
  for (int i=0; i<100; i++)
  {
    m = (m * i + to_stat_space(randomize<Mat23>(N {1.0, 0.3}, 2.0))) / (i + 1);
  }
  Mat23 offset = {1, 1, 1, 2, 2, 2};
  EXPECT_TRUE(is_near(from_stat_space(m), offset, 0.1));
  EXPECT_FALSE(is_near(from_stat_space(m), offset, 1e-6));
...
TEST(matrices, Mean_angle_Euclidean_conversion)
{
  ...
  EXPECT_TRUE(is_near(to_stat_space(x1).nested_object(), to_stat_space(x1p).nested_object()));
  EXPECT_TRUE(is_near(to_stat_space(x1), to_stat_space(x1p)));
  ...
  EXPECT_TRUE(is_near(make_mean(std::sqrt(3.)/2, 0.5, 5, 0.5, -std::sqrt(3.)/2), to_stat_space(x1).nested_object()));
...
TEST(matrices, Mean_angle_arithmetic_Euclidean)
{
  ...
  Var3 x1 {pi / 6, 5, -pi / 3};
  Var3 x2 {2 * pi, 0, 6 * pi};
  auto x1e = to_stat_space(x1);
  auto x2e = to_stat_space(x2);
  EXPECT_TRUE(is_near(from_stat_space(x1e + x2e), Var3 {pi / 12, 5, -pi / 6}));
  EXPECT_TRUE(is_near(from_stat_space(x2e + x1e), Var3 {pi / 12, 5, -pi / 6}));
  EXPECT_TRUE(is_near(from_stat_space(x1e - x2e), Var3 {pi * 7 / 12, 5, -pi * 2 / 3}));
  EXPECT_TRUE(is_near(from_stat_space(x2e - x1e), Var3 {-pi * 5 / 12, -5, pi / 3}));
  EXPECT_TRUE(is_near(from_stat_space(-x1e), Var3 {-pi * 5 / 6, -5, pi * 2 / 3}));
  Var3 x50 {pi / 6, 50, -pi / 3};
  EXPECT_TRUE(is_near(from_stat_space(x1e * 10.), x50));
  EXPECT_TRUE(is_near(from_stat_space(10. * x1e), x50));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10), x1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10. + x2e / 10.), Var3 {pi / 12, 5, -pi / 6}));
  EXPECT_TRUE(is_near(from_stat_space((to_stat_space(x1) * 2.0 + make_zero(to_stat_space(x1))) / 2.0), x1));
  auto mean_x = Mean(x1);
  mean_x = from_stat_space((to_stat_space(mean_x) * 2.0 + make_zero(to_stat_space(mean_x))) / 2.0);
  EXPECT_TRUE(is_near(mean_x, x1));
  auto x6 = x1e;
  EXPECT_TRUE(is_near(to_stat_space(Var3{x6}), x6));
}
...
TEST(matrices, Mean_angle2pi_arithmetic_Euclidean)
{
  ...
  const auto x1e = to_stat_space(x1);
  const auto x2e = to_stat_space(x2);
  EXPECT_TRUE(is_near(from_stat_space(x1e + x2e), Var3 {pi / 12, 5, pi * 11 / 6}));
  EXPECT_TRUE(is_near(from_stat_space(x2e + x1e), Var3 {pi / 12, 5, pi * 11 / 6}));
  EXPECT_TRUE(is_near(from_stat_space(x1e - x2e), Var3 {pi * 7 / 12, 5, pi * 4 / 3}));
  EXPECT_TRUE(is_near(from_stat_space(x2e - x1e), Var3 {pi * 19 / 12, -5, pi / 3}));
  EXPECT_TRUE(is_near(from_stat_space(-x1e), Var3 {pi * 7 / 6, -5, pi * 2 / 3}));
  const Var3 x50 {pi / 6, 50, pi * 5 / 3};
  EXPECT_TRUE(is_near(from_stat_space(x1e * 10.), x50));
  EXPECT_TRUE(is_near(from_stat_space(10. * x1e), x50));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10.), x1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10. + x2e / 10.), Var3 {pi / 12, 5, pi * 11 / 6}));
}
...
TEST(matrices, Mean_angle2deg_arithmetic_Euclidean)
{
  ...
  const auto x1e = to_stat_space(x1);
  const auto x2e = to_stat_space(x2);
  EXPECT_TRUE(is_near(from_stat_space(x1e + x2e), Var3 {15, 5, 330}));
  EXPECT_TRUE(is_near(from_stat_space(x2e + x1e), Var3 {15, 5, 330}));
  EXPECT_TRUE(is_near(from_stat_space(x1e - x2e), Var3 {105, 5, 240}));
  EXPECT_TRUE(is_near(from_stat_space(x2e - x1e), Var3 {285, -5, 60}));
  EXPECT_TRUE(is_near(from_stat_space(-x1e), Var3 {210, -5, 120}));
  const Var3 x50 {30, 50, 300};
  EXPECT_TRUE(is_near(from_stat_space(x1e * 10.), x50));
  EXPECT_TRUE(is_near(from_stat_space(10. * x1e), x50));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10.), x1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x50) / 10. + x2e / 10.), Var3 {15, 5, 330}));
}
...
TEST(matrices, Mean_angle_columns_Euclidean)
{
  ...
  const auto x1e = to_stat_space(x1);
  const auto x2e = to_stat_space(x2);
  EXPECT_TRUE(is_near(from_stat_space(x1e + x2e), Var3 {pi / 12, -pi / 6, 5, 2}));
  EXPECT_TRUE(is_near(from_stat_space(x2e + x1e), Var3 {pi / 12, -pi / 6, 5, 2}));
  EXPECT_TRUE(is_near(from_stat_space(x1e - x2e), Var3 {pi * 7 / 12, -pi * 2 / 3, 5, 2}));
  EXPECT_TRUE(is_near(from_stat_space(x2e - x1e), Var3 {-pi * 5 / 12, pi / 3, -5, -2}));
  EXPECT_TRUE(is_near(from_stat_space(-x1e), Var3 {-pi * 5 / 6, pi * 2 / 3, -5, -2}));
  Var3 x10 {pi / 6, -pi / 3, 50, 20};
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x1) * 10.), x10));
  EXPECT_TRUE(is_near(from_stat_space(10. * to_stat_space(x1)), x10));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x10) / 10.), x1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(x10) / 10. + x2e / 10.), Var3 {pi / 12, -pi / 6, 5, 2}));
  Var3 mean_x = x1;
  mean_x = from_stat_space((to_stat_space(mean_x) * 2.0 + make_zero(to_stat_space(mean_x))) / 2.0);
  EXPECT_TRUE(is_near(mean_x, x1));
  auto x6 = to_stat_space(x1);
  EXPECT_TRUE(is_near(to_stat_space(Var3{x6}), x6));
...
TEST(matrices, Wrap_distance)
{
  ...
  EXPECT_TRUE(is_near(from_stat_space(-to_stat_space(x0) + to_stat_space(R {1.2})), eigen_matrix_t<double, 1, 1> {3.8}));
...
TEST(matrices, Wrap_inclination)
{
  ...
  EXPECT_TRUE(is_near(R {-pi * 7 / 12}, R {-pi * 5 / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(R {pi / 6}) + to_stat_space(R {pi})), R {pi / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(R {-pi / 6}) + to_stat_space(R {pi})), R {-pi / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(R {pi * 5 / 6}) + to_stat_space(R {pi / 2})), R {pi / 3}));
  R x0 {pi/2};
...
TEST(matrices, Wrap_polar)
{
  ...
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(P {1., pi / 6}) + to_stat_space(P {-0.5, 0})), P {1.5, pi * 7 / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(P {1., pi / 6}) + to_stat_space(P {-1.5, 0})), P {2.5, pi * 7 / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(P {1., pi * 5 / 6}) + to_stat_space(P {0, -pi * 2 / 3})), P {1., -pi * 11 / 12}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(P {1., pi * 5 / 6}) + to_stat_space(P {-1.5, -pi * 2 / 3})), P {2.5, pi * 7 / 12}));
  P x0 {2, pi/4};
  ...
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(Q {pi / 6, 1}) + to_stat_space(Q {0, -0.5})), Q {pi * 7 / 12, 1.5}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(Q {pi / 6, 1}) + to_stat_space(Q {0, -1.5})), Q {pi * 7 / 12, 2.5}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(Q {pi * 5 / 6, 1}) + to_stat_space(Q {-pi * 2 / 3, 0})), Q {-pi * 11 / 12, 1}));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(Q {pi * 5 / 6, 1}) + to_stat_space(Q {-pi * 2 / 3, -1.5})), Q {pi * 7 / 12, 2.5}));
  Q x1 {pi/4, 2};
...
TEST(matrices, Wrap_spherical)
{
  ...
  const auto x3 = S {1.0, std::atan2(3. / 4, -1. + std::sqrt(3.) / 4),
                     std::asin(0.5 / std::hypot(-1. + std::sqrt(3.) / 4, 3. / 4, 0.5))};
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(S {1., pi / 3, pi / 6}) - to_stat_space(S {-0.5, 0, 0})), x1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(S {1., pi / 3, pi / 6}) - to_stat_space(S {-1.5, 0, 0})), x2));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(S {1., pi / 3, pi / 6}) + to_stat_space(S {0, 0, pi})), x3));
  S x0 {2, pi/4, -pi/4};
  EXPECT_NEAR(get_component(x0, 0, 0), 2, 1e-6);
  EXPECT_NEAR(get_component(x0, 1), pi/4, 1e-6);
  ...
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(T {pi / 3, 1, pi / 6}) - to_stat_space(T {0, -0.5, 0})), y1));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(T {pi / 3, 1, pi / 6}) - to_stat_space(T {0, -1.5, 0})), y2));
  EXPECT_TRUE(is_near(from_stat_space(to_stat_space(T {pi / 3, 1, pi / 6}) + to_stat_space(T {0, 0, pi})), y3));
  T z1 {pi/4, 2, -pi/4};
