...
TEST(matrices, TypedMatrix_deduction_guides)
{
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix(a))>, Dimensions<2>>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix(a))>, Dimensions<3>>);

  //auto b1 = Mat23 {1, 2, 3, 4, 5, 6};
  //EXPECT_TRUE(is_near(Matrix(b1), a));
  //static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix(b1))>, C2>);
  //static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix(b1))>, C3>);

  auto b2 = Mean<C2, M23> {1, 2, 3, 4, 5, 6};
  EXPECT_TRUE(is_near(Matrix(b2), (M23() << 1, 2, 3, 4-2*pi, 5-2*pi, 6-2*pi).finished()));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix(b2))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix(b2))>, Dimensions<3>>);

  auto b3 = EuclideanMean<C2, M33> {1, 2, 3,
                                    0.5, std::sqrt(3)/2, sqrt2/2,
                                    std::sqrt(3)/2, 0.5, sqrt2/2};
  EXPECT_TRUE(is_near(Matrix(b3), Mat23 {1, 2, 3, pi/3, pi/6, pi/4}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix(b3))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix(b3))>, Dimensions<3>>);

  auto c = Covariance<C2, SelfAdjointMatrix<M22, TriangleType::lower>> {9, 3, 3, 10};
  EXPECT_TRUE(is_near(Matrix(c), Mat22 {9, 3, 3, 10}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix(c))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix(c))>, C2>);
}
...
TEST(matrices, TypedMatrix_make_functions)
{
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix<C2, C3>(a))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix<C2, C3>(a))>, C3>);
  EXPECT_TRUE(is_near(make_matrix<C2>(a), a));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix<C2>(a))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix<C2>(a))>, Dimensions<3>>);

  auto b = Mat23 {1, 2, 3, 4, 5, 6};
  EXPECT_TRUE(is_near(make_matrix(b), a));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix(b))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix(b))>, C3>);

  auto c = Covariance<C2, SelfAdjointMatrix<M22, TriangleType::lower>> {9, 3, 3, 10};
  EXPECT_TRUE(is_near(make_matrix(c), Mat22 {9, 3, 3, 10}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix(c))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix(c))>, C2>);

  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix<C2, C3, M23>())>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix<C2, C3, M23>())>, C3>);
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(make_matrix<M23>())>, Dimensions<2>>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(make_matrix<M23>())>, Dimensions<3>>);
}
...
TEST(matrices, TypedMatrix_overloads)
{
  ...
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(to_diagonal(Mat21 {2, 3}))>, C2>);
...
TEST(matrices, TypedMatrix_blocks)
{
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(concatenate_vertical(Mat22 {1, 2, 3, 4}, Mat12 {5, 6}))>, C3>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(concatenate_vertical(Mat22 {1, 2, 3, 4}, Mat12 {5, 6}))>, C2>);

  EXPECT_TRUE(is_near(concatenate_horizontal(Mat22 {1, 2, 4, 5}, Mat21 {3, 6}), Mat23 {1, 2, 3, 4, 5, 6}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(concatenate_horizontal(Mat22 {1, 2, 4, 5}, Mat21 {3, 6}))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(concatenate_horizontal(Mat22 {1, 2, 4, 5}, Mat21 {3, 6}))>, C3>);

  EXPECT_TRUE(is_near(concatenate_diagonal(Mat12 {1, 2}, Mat21 {3, 4}), Mat33 {1, 2, 0, 0, 0, 3, 0, 0, 4}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(concatenate_diagonal(Mat12 {1, 2}, Mat21 {3, 4}))>, TypedIndex<Axis, Axis, angle::Radians>>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(concatenate_diagonal(Mat12 {1, 2}, Mat21 {3, 4}))>, TypedIndex<Axis, angle::Radians, Axis>>);
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(column<0>(Mat22 {1, 2, 3, 4}))>, C2>);
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(column<1>(Mat22 {1, 2, 3, 4}))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(column<0>(Mat22 {1, 2, 3, 4}))>, Axis>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(column<1>(Mat22 {1, 2, 3, 4}))>, angle::Radians>);
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(apply_columnwise<2>(std::declval<Matrix<C2, angle::Radians>()>()))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(apply_columnwise<2>(std::declval<Matrix<C2, angle::Radians>()>()))>, TypedIndex<angle::Radians, angle::Radians>>);
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(apply_columnwise<2>(std::declval<Matrix<C2, angle::Radians>(std::size_t)>()))>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(apply_columnwise<2>(std::declval<Matrix<C2, angle::Radians>(std::size_t)>()))>, TypedIndex<angle::Radians, angle::Radians>>);
...
TEST(matrices, TypedMatrix_arithmetic)
{
  ...
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Mat32 {7, 6, 5, 4, 3, 2} + Mat32 {1, 2, 3, 4, 5, 6})>, C2>);
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Mat22 {1, 2, 3, 4} * Mat23 {1, 2, 3, 4, 5, 6})>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Mat22 {1, 2, 3, 4} * Mat23 {1, 2, 3, 4, 5, 6})>, C3>);

  EXPECT_TRUE(is_near(Mat22 {1, 2, 3, 2} * Mean<C2, M23> {1, 2, 3, 3, 2, 1}, Mat23 {7, 6, 5, 9, 10, 11}));
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Mat22 {1, 2, 3, 4} * Mean<C2, M23> {1, 2, 3, 4, 5, 6})>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Mat22 {1, 2, 3, 4} * Mean<C2, M23> {1, 2, 3, 4, 5, 6})>, Dimensions<3>>);
  ...
  static_assert(equivalent_to<row_index_descriptor_of_t<decltype(Matrix<C2, Dimensions<2>, M22> {1, 2, 3, 4} * EuclideanMean<Dimensions<2>, M23> {1, 2, 3, 4, 5, 6})>, C2>);
  static_assert(equivalent_to<column_index_descriptor_of_t<decltype(Matrix<C2, Dimensions<2>, M22> {1, 2, 3, 4} * EuclideanMean<Dimensions<2>, M23> {1, 2, 3, 4, 5, 6})>, Dimensions<3>>);
