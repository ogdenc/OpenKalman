...
namespace OpenKalman::Eigen3
{
  ...
  struct EigenSelfAdjointMatrix
    : OpenKalman::internal::MatrixBase<EigenSelfAdjointMatrix<BaseMatrix, storage_triangle>, BaseMatrix>
  {
    using Base = OpenKalman::internal::MatrixBase<EigenSelfAdjointMatrix, BaseMatrix>;
    ...
    /// Move constructor.
    EigenSelfAdjointMatrix(EigenSelfAdjointMatrix&& other) noexcept
      : EigenSelfAdjointMatrix(std::move(other.base_matrix())) {}
    ...
    template<eigen_self_adjoint_expr Arg> requires
      (is_Eigen_lower_storage_triangle_v<Arg> == is_Eigen_lower_storage_triangle_v<EigenSelfAdjointMatrix>)
#else
    template<typename Arg, std::enable_if_t<is_EigenSelfAdjointMatrix_v<Arg> and
      is_Eigen_lower_storage_triangle_v<Arg> == is_Eigen_lower_storage_triangle_v <EigenSelfAdjointMatrix>, int> = 0>
#endif
    EigenSelfAdjointMatrix(Arg&& arg) noexcept
      : EigenSelfAdjointMatrix(base_matrix(std::forward<Arg>(arg))) {}
...
#endif
    EigenSelfAdjointMatrix(Arg&& arg) noexcept
      : EigenSelfAdjointMatrix(adjoint(base_matrix(std::forward<Arg>(arg)))) {}
     ...
     * @param other Another EigenSelfAdjointMatrix
     ...
     * @param other A EigenSelfAdjointMatrix temporary value.
     * @return Reference to this.
     */
    ...
    auto& operator=(Arg&& arg)
    {
      ...
      {
        if constexpr(is_Eigen_upper_storage_triangle_v<Arg>
          == is_Eigen_upper_storage_triangle_v<EigenSelfAdjointMatrix>)
  ...
  template<typename Arg, std::enable_if_t<is_Eigen_matrix_v<Arg> or is_EigenDiagonal_v<Arg>, int> = 0>
#endif
  EigenSelfAdjointMatrix(Arg&&)
  -> EigenSelfAdjointMatrix<lvalue_or_strict_t<Arg>, TriangleType::lower>;
...
namespace OpenKalman
{
  ...
  struct MatrixTraits<Eigen3::EigenSelfAdjointMatrix<ArgType, storage_triangle>>
  {
    ...
    using MatrixBaseType = Eigen3::internal::EigenMatrixBase<
      Derived, Eigen3::EigenSelfAdjointMatrix<std::decay_t<BaseMatrix>, storage_type>>;
    ...
    using Strict = Eigen3::EigenSelfAdjointMatrix<typename MatrixTraits<BaseMatrix>::Strict, storage_type>;
    ...
    {
      return Eigen3::EigenSelfAdjointMatrix<std::decay_t<Arg>, t>(std::forward<Arg>(arg));
    }
    ...
    static auto zero() { return Eigen3::EigenSelfAdjointMatrix<BaseMatrix, storage_type>::zero(); }

    static auto identity() { return Eigen3::EigenSelfAdjointMatrix<BaseMatrix, storage_type>::identity(); }
...
namespace OpenKalman::Eigen3
{
...
#else
  template<typename Arg, std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<Arg>, int> = 0>
#endif
  inline auto
  transpose(Arg && arg)
  {
    constexpr auto t = Eigen3::is_Eigen_lower_storage_triangle_v<Arg> ? TriangleType::upper : TriangleType::lower;
    auto b = transpose(base_matrix(std::forward<Arg>(arg)));
    return Eigen3::EigenSelfAdjointMatrix<decltype(b), t>(std::move(b));
  }
  ...
  template<typename Arg, typename U,
    std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<Arg> and
...
#else
  template<typename A, std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<A>, int> = 0>
#endif
  inline auto
...
///////////////////////////////
  ...
  template<typename Arg1, typename Arg2,
    std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2> and
  ...
  inline auto operator+(Arg1 && arg1, Arg2 && arg2)
  {
    ...
    if constexpr(Eigen3::is_Eigen_lower_storage_triangle_v < Arg1 > != Eigen3::is_Eigen_lower_storage_triangle_v < Arg2 >)
    {
...
#else
  template<typename Arg1, typename Arg2,
      std::enable_if_t<((Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_diagonal_v<Arg2>) or
      (is_diagonal_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>)) and
...
#else
  template<typename Arg1, typename Arg2,
    std::enable_if_t<(Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_identity_v<Arg2>) or
      (is_identity_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
  inline auto operator+(Arg1&& arg1, Arg2&& arg2)
  {
    static_assert(MatrixTraits<Arg1>::dimension == MatrixTraits<Arg2>::dimension);
...
#else
  template<typename Arg1, typename Arg2,
    std::enable_if_t<(Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_zero_v<Arg2>) or
      (is_zero_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
...
#else
  template<typename Arg1, typename Arg2,
    std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>, int> = 0>
#endif
  inline auto operator-(Arg1 && arg1, Arg2 && arg2)
  {
    static_assert(MatrixTraits<Arg1>::dimension == MatrixTraits<Arg2>::dimension);
...
requires (Eigen3::eigen_self_adjoint_expr<Arg1> and is_identity_v<Arg2> ) or
  (is_identity_v<Arg1> and Eigen3::eigen_self_adjoint_expr<Arg2> )
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<(Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_identity_v<Arg2>) or
    (is_identity_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
  inline auto operator-(Arg1&& arg1, Arg2&& arg2)
  {
  ...
  requires (Eigen3::eigen_self_adjoint_expr<Arg1> and is_zero_v<Arg2>) or
    (is_zero_v<Arg1> and Eigen3::eigen_self_adjoint_expr<Arg2>)
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<(Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_zero_v<Arg2>) or
    (is_zero_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
  constexpr decltype(auto) operator-(Arg1&& arg1, Arg2&& arg2)
  {
...
////
...
#else
  template<typename Arg, typename S, std::enable_if_t<Eigen3::is_EigenSelfAdjointMatrix_v<Arg> and
  ...
  template<typename Arg1, typename Arg2, std::enable_if_t<
    ((Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>) or
      (Eigen3::is_EigenTriangularMatrix_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>) or
      (Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and Eigen3::is_EigenTriangularMatrix_v<Arg2>)) and
    not is_diagonal_v<Arg1> and not is_diagonal_v<Arg2>, int> = 0>
#endif
  inline auto operator*(Arg1&& arg1, Arg2&& arg2)
  {
...
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<
    ((Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_diagonal_v<Arg2>) or
      (is_diagonal_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>)) and
...
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<
    (Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_identity_v<Arg2>) or
      (is_identity_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
  inline auto operator*(Arg1&& arg1, Arg2&& arg2)
  {
...
#else
  template<typename Arg1, typename Arg2, std::enable_if_t<
    (Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and is_zero_v<Arg2>) or
      (is_zero_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>), int> = 0>
#endif
  constexpr decltype(auto) operator*(Arg1&& arg1, Arg2&& arg2)
  {
...
#else
  template<typename Arg1, typename Arg2,
    std::enable_if_t<((Eigen3::is_EigenSelfAdjointMatrix_v<Arg1> and Eigen3::is_Eigen_matrix_v<Arg2>)
      or (Eigen3::is_Eigen_matrix_v<Arg1> and Eigen3::is_EigenSelfAdjointMatrix_v<Arg2>)) and
      not is_identity_v<Arg1> and not is_identity_v<Arg2> and not is_zero_v<Arg1> and not is_zero_v<Arg2>, int> = 0>
#endif
  inline auto operator*(Arg1&& arg1, Arg2&& arg2)
  {
