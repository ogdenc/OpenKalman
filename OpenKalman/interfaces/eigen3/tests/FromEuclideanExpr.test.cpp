...
TEST(eigen3, FromEuclideanExpr_class)
{
  ...
  EXPECT_TRUE(is_near(From4(ZeroMatrix<double, 4, 2>()), mat3(0, 0, 0, 0, 0, 0)));
  ...
  EXPECT_EQ(From4::rows(), 3);
  EXPECT_EQ(From4::cols(), 2);
  EXPECT_TRUE(is_near(From4::zero(), M3::Zero()));
  EXPECT_TRUE(is_near(FromEuclideanExpr<Axes<2>, eigen_matrix_t<double, 2, 2>>::identity(), eigen_matrix_t<double, 2, 2>::Identity()));

  FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 1>> e1 = {3, std::sqrt(2.)/2, std::sqrt(2.)/2};
...
TEST(eigen3, FromEuclideanExpr_subscripts)
{
  ...
  EXPECT_NEAR((FromEuclideanExpr<C, eigen_matrix_t<double, 4, 1>>{1., std::sqrt(3)/2, 0.5, 3})(1), pi/6, 1e-6);
  EXPECT_NEAR((FromEuclideanExpr<C, eigen_matrix_t<double, 4, 1>>{1., std::sqrt(3)/2, 0.5, 3})(2), 3, 1e-6);
...
TEST(eigen3, FromEuclideanExpr_traits)
{
  ...
  EXPECT_TRUE(is_near(MatrixTraits<From4>::make(1, 2, std::sqrt(3)/2, 0.5, 0.5, std::sqrt(3)/2, 3, 4),
    mat3(1, 2, pi/6, pi/3, 3, 4)));
  EXPECT_TRUE(is_near(MatrixTraits<From4>::zero(), eigen_matrix_t<double, 3, 2>::Zero()));
  EXPECT_TRUE(is_near(FromEuclideanExpr<Axes<2>, eigen_matrix_t<double, 2, 2>>::identity(), eigen_matrix_t<double, 2, 2>::Identity()));
}
...
TEST(eigen3, FromEuclideanExpr_overloads)
{
  ...
  EXPECT_TRUE(is_near(to_diagonal(FromEuclideanExpr<C, eigen_matrix_t<double, 4, 1>>{1., std::sqrt(3)/2, 0.5, 3}), DiagonalMatrix {1, pi/6, 3}));
  ...
  EXPECT_TRUE(is_near(reduce_rows(From4 {1, 2, std::sqrt(3)/2, 0.5, 0.5, std::sqrt(3)/2, 3, 4}), make_native_matrix<double, 1, 2>(4./3 + pi/18, 2 + pi/9)));
  ...
  for (int i=0; i<100; i++)
  {
    m = (m * i + to_euclidean(randomize<From4>(N {1.0, 0.3}))) / (i + 1);
  }
...
TEST(eigen3, FromEuclideanExpr_blocks)
{
  EXPECT_TRUE(is_near(concatenate_vertical(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {1, 2, 3,
                                                                               std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
                                                                               0.5, std::sqrt(3)/2, std::sqrt(2)/2},
    FromEuclideanExpr<Coefficients<angle::Radians, Axis>, eigen_matrix_t<double, 3, 3>> {std::sqrt(2)/2, 0.5, std::sqrt(3)/2,
  ...
  EXPECT_TRUE(is_near(concatenate_horizontal(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {1, 2, 3,
                                                                               std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
                                                                               0.5, std::sqrt(3)/2, std::sqrt(2)/2},
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {4, 5, 6,
                                                                               std::sqrt(2)/2, 0.5, std::sqrt(3)/2,
                                                                               std::sqrt(2)/2, std::sqrt(3)/2, 0.5}),
    make_native_matrix<double, 2, 6>(
      1, 2, 3, 4, 5, 6,
      pi/6, pi/3, pi/4, pi/4, pi/3, pi/6)));
  EXPECT_TRUE(is_near(split_vertical(FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 2>> {
  ...
  EXPECT_TRUE(is_near(split_vertical<Coefficients<Axis, angle::Radians>, Coefficients<angle::Radians>>(
    ...
    std::tuple {make_native_matrix<double, 2, 3>(1., 2, 3, pi/6, pi/3, pi/4),
               make_native_matrix<double, 1, 3>(pi/4, pi/3, pi/6)}));
  EXPECT_TRUE(is_near(split_horizontal(FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 2>> {
    1, 2, std::sqrt(3)/2, 0.5, 0.5, std::sqrt(3)/2}), std::tuple {}));
  ...
  EXPECT_TRUE(is_near(split_diagonal<Axis, angle::Radians>(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, const eigen_matrix_t<double, 3, 2>> {
      1, 2,
      std::sqrt(3)/2, 0.5,
      0.5, std::sqrt(3)/2}),
  ...
  EXPECT_TRUE(is_near(column(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {
      1, 2, 3,
      std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
  ...
  EXPECT_TRUE(is_near(row(
    ...
    make_native_matrix<double, 1, 3>(pi/6, pi/3, pi/4)));
  EXPECT_TRUE(is_near(row<0>(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {
      1, 2, 3,
      std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
      0.5, std::sqrt(3)/2, std::sqrt(2)/2}),
    make_native_matrix<double, 1, 3>(1, 2, 3)));

  auto b = FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {
    1, 2, 3,
    std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
    0.5, std::sqrt(3)/2, std::sqrt(2)/2};
  ...
  b = FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {
    1, 2, 3,
    std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
    0.5, std::sqrt(3)/2, std::sqrt(2)/2};
  EXPECT_TRUE(is_near(apply_columnwise(b,
  ...
  EXPECT_TRUE(is_near(apply_columnwise(
    ...
    make_native_matrix<double, 2, 3>(
      ...
      pi/3, pi*2/3, pi/2)));
  EXPECT_TRUE(is_near(apply_columnwise(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, ToEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 2, 3>>>
  ...
  EXPECT_TRUE(is_near(apply_columnwise<3>(
    [](){ return FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 1>> {1., std::sqrt(3)/2, 0.5}; }),
    make_native_matrix<double, 2, 3>(
      1, 1, 1,
      pi/6, pi/6, pi/6)));
  EXPECT_TRUE(is_near(apply_columnwise<3>(
    [](std::size_t i){ return make_self_contained(FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 1>> {1., std::sqrt(3)/2, 0.5} * (i + 1)); }),
    make_native_matrix<double, 2, 3>(
      1, 2, 3,
      pi/6, pi/3, pi/2)));
  ...
  EXPECT_TRUE(is_near(apply_rowwise(
    FromEuclideanExpr<Coefficients<Axis, angle::Radians>, eigen_matrix_t<double, 3, 3>> {1, 2, 3,
                                                                                         std::sqrt(3)/2, 0.5, std::sqrt(2)/2,
                                                                                         0.5, std::sqrt(3)/2, std::sqrt(2)/2},
    [](const auto& row, std::size_t i){ return make_self_contained(row * (i + 1)); }),
    make_native_matrix<double, 2, 3>(
      1, 2, 3,
      pi/3, pi*2/3, pi/2)));
