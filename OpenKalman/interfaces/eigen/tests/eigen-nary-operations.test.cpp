...
TEST(eigen3, unary_operation)
{
  ...
  auto b23 = make_dense_object_from<eigen_matrix_t<bool, 2, 3>>(true, false, true, false, true, false);
  ...
  auto m31 = make_dense_object_from<M31>(1, 2, 3);
  M3x m3x_1 {m31};
  Mx1 mx1_3 {m31};
  Mxx mxx_31 {m31};
  auto m32 = make_dense_object_from<M32>(1, 1, 2, 2, 3, 3);
  ...
  auto m32p = make_dense_object_from<M32>(0, 1, 1, 2, 2, 3);
  ...
  auto m13 = make_dense_object_from<M13>(1, 2, 3);
  M1x m1x_3 {m13};
  Mx3 mx3_1 {m13};
  Mxx mxx_13 {m13};
  auto m23a = make_dense_object_from<M23>(1, 2, 3, 1, 2, 3);
...
{
  const auto m23c = make_dense_object_from<M23>(1, 2, 3, 4, 5, 6);
  M23 m23 {m23c};
  M2x m2x_3 {m23};
  Mx3 mx3_2 {m23};
  Mxx mxx_23 {m23};
  ...
  const auto m33_123 = make_dense_object_from<M33>(
    1, 0, 0,
    0, 2, 0,
    0, 0, 3);

  const auto m33_234 = make_dense_object_from<M33>(
    2, 1, 1,
    1, 3, 1,
    1, 1, 4);
...
TEST(eigen3, binary_operation)
{
  ...
  auto m11_7 = make_dense_object_from<M11>(7);
  ...
  Mx3 mx3_1 {m13};
  Mxx mxx_13 {m13};
  m23 = make_dense_object_from<M23>(1, 2, 3, 1, 2, 3);
  m2x_3 = m23;
  mx3_2 = m23;
  mxx_23 = m23;
  ...
  auto m13_eq = make_dense_object_from<M13>(1, 3, 3);
  auto m13_gt = make_dense_object_from<M13>(2, 2, 4);
  auto m13_ge = make_dense_object_from<M13>(2, 1, 3);
  auto b23 = make_dense_object_from<eigen_matrix_t<bool, 2, 3>>(true, false, true, true, false, true);
  ...
  auto b13 = make_dense_object_from<eigen_matrix_t<bool, 1, 3>>(true, false, true);
  auto b13_and = make_dense_object_from<eigen_matrix_t<bool, 1, 3>>(true, true, true);
  auto b13_or = make_dense_object_from<eigen_matrix_t<bool, 1, 3>>(false, false, false);
...
{
  auto m23 = make_dense_object_from<M23>(1, 2, 3, 1, 2, 3);
  M2x m2x_3 {m23};
  Mx3 mx3_2 {m23};
  Mxx mxx_23 {m23};
  auto m13 = make_dense_object_from<M13>(1, 2, 3);
...
TEST(eigen3, randomize)
{
  ...
  M23 m23, m23_true;
  M32 m32, m32_true;
  auto m2x_2 = make_dense_object<M22>(Dimensions<2>{}, 2);
  auto m2x_3 = make_dense_object<M23>(Dimensions<2>{}, 3);
  auto mx2_2 = make_dense_object<M22>(2, Dimensions<2>{});
  auto mx2_3 = make_dense_object<M32>(3, Dimensions<2>{});
  auto m3x_2 = make_dense_object<M32>(Dimensions<3>{}, 2);
  auto mx3_2 = make_dense_object<M23>(2, Dimensions<3>{});
  auto mxx_22 = make_dense_object<M22>(2, 2);
  auto mxx_32 = make_dense_object<M32>(3, 2);
  auto mxx_23 = make_dense_object<M23>(2, 3);
  ...
  m22_true = make_dense_object_from<M22>(1, 2, 3, 4);
  EXPECT_TRUE(is_near(m22, m22_true, 0.1));
  EXPECT_FALSE(is_near(m22, m22_true, 1e-8));
  ...
  m32_true = make_dense_object_from<M32>(1, 1, 2, 2, 3, 3);
  ...
  m23_true = make_dense_object_from<M23>(1, 1, 1, 2, 2, 2);
  ...
  m32_true = make_dense_object_from<M32>(1, 2, 1, 2, 1, 2);
  EXPECT_TRUE(is_near(m32, m32_true, 0.1));
  EXPECT_FALSE(is_near(m32, m32_true, 1e-8));
  EXPECT_TRUE(is_near(mx2_3, m32_true, 0.1));
  EXPECT_FALSE(is_near(mx2_3, m32_true, 1e-8));

  m23_true = make_dense_object_from<M23>(1, 2, 3, 1, 2, 3);
  EXPECT_TRUE(is_near(m23, m23_true, 0.1));
  EXPECT_FALSE(is_near(m23, m23_true, 1e-8));
