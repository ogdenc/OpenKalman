...
TEST(eigen3, sum)
{
  ...
  static_assert(not diagonal_matrix<decltype(OpenKalman::detail::sum_impl(mxx_23a))>);
  static_assert(not diagonal_matrix<decltype(OpenKalman::detail::sum_impl(mxx_23a, mxx_23b))>);
  static_assert(not diagonal_matrix<decltype(internal::make_fixed_size_adapter_like<decltype(mxx_23a)>(mxx_23a))>);
  static_assert(triangular_matrix<Mxx, TriangleType::diagonal, Qualification::depends_on_dynamic_shape>);
  //static_assert(triangle_type_of_v<decltype(mxx_23a)> == TriangleType::any);
  //static_assert(not diagonal_matrix<decltype(sum(mxx_23a))>);
  //static_assert(not diagonal_matrix<decltype(internal::make_fixed_size_adapter_like<decltype(mxx_23a), decltype(mxx_23b)>(sum(mxx_23a, mxx_23b)))>);
