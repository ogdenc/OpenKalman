...
#ifndef OPENKALMAN_INDEXIBLE_OBJECT_TRAITS_BASE_HPP
#define OPENKALMAN_INDEXIBLE_OBJECT_TRAITS_BASE_HPP
...
#else
  template<typename T>
  struct indexible_object_traits_base<T, std::enable_if_t<Eigen3::eigen_general<T>>>
#endif
  {
  private:

    using IndexType = typename T::Index; // static_assert(index_value<IndexType>);
...
#ifdef __cpp_lib_concepts
    template<typename U> requires requires(U u) { {u.innerStride()} -> index_value; {u.outerStride()} -> index_value; }
    struct has_strides<U>
#else
    template<typename U>
    struct has_strides<U, std::enable_if_t<index_value<decltype(std::declval<U>().innerStride())> and
      index_value<decltype(std::declval<U>().outerStride())>>>
#endif
      : std::true_type {};
