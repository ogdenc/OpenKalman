...
#ifndef OPENKALMAN_EIGEN_CHOLESKY_HPP
#define OPENKALMAN_EIGEN_CHOLESKY_HPP
...
namespace OpenKalman
{
  ...
  template<Eigen3::eigen_matrix_general<true> D> requires has_dynamic_dimensions<D> or diagonal_matrix<D>
#else
  template<typename D, std::enable_if_t<
    Eigen3::eigen_matrix_general<D, true> and (has_dynamic_dimensions<D> or diagonal_matrix<D>), int> = 0>
#endif
  ...
  Cholesky_square(D&& d) noexcept
  {
    if constexpr ((has_dynamic_dimensions<D> and not diagonal_matrix<D>) or one_by_one_matrix<D>)
    {
      if constexpr (has_dynamic_dimensions<D> and not diagonal_matrix<D>)
        assert(get_vector_space_descriptor<0>(d) == 1 and get_vector_space_descriptor<1>(d) == 1);
   ...
   */
#ifdef __cpp_concepts
  template<TriangleType = TriangleType::diagonal, Eigen3::eigen_matrix_general<true> D>
  requires has_dynamic_dimensions<D> or diagonal_matrix<D>
#else
  template<TriangleType = TriangleType::diagonal,
    typename D, std::enable_if_t<Eigen3::eigen_matrix_general<D, true> and (has_dynamic_dimensions<D> or diagonal_matrix<D>), int> = 0>
#endif
  constexpr decltype(auto)
...
#endif //OPENKALMAN_EIGEN_CHOLESKY_HPP
