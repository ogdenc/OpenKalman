...
namespace OpenKalman::interface
{
  ...
  struct indexible_object_traits<VectorSpaceAdapter<NestedObject, Ds...>>
  {
  private:

    using Xpr = VectorSpaceAdapter<NestedObject, Ds...>;
  ...
  public:
    ...
    static constexpr auto get_vector_space_descriptor(Arg&& arg, const N& n)
    {
      ...
      {
        if constexpr (N::value >= sizeof...(Ds)) return Dimensions<1>{};
        else return std::get<N::value>(std::forward<Arg>(arg).my_descriptors);
      }
      else if (n >= sizeof...(Ds))
      {
        return 1_uz;
      ...
      {
        return std::apply(
          [](auto&&...vs){ return std::array<std::size_t, sizeof...(Ds)> {std::forward<decltype(vs)>(vs)...}; },
...
#ifdef __cpp_concepts
    template<typename Arg, typename BeginTup, typename...Size> requires
      interface::get_slice_defined_for<NestedObject, nested_object_of_t<Arg&&>, const BeginTup&, const std::tuple<Size...>&>
#else
    template<typename Arg, typename BeginTup, typename...Size, std::enable_if_t<
      interface::get_slice_defined_for<NestedObject, typename nested_object_of<Arg&&>::type, const BeginTup&, const std::tuple<Size...>&>, int> = 0>
#endif
    static decltype(auto)
