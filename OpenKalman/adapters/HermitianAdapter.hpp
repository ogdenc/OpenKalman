...
namespace OpenKalman
{
  ...
  template<square_shaped<Qualification::depends_on_dynamic_shape> NestedObject, HermitianAdapterType storage_triangle> requires
    ...
    (not constant_matrix<NestedObject> or real_axis_number<constant_coefficient<NestedObject>>) and
    (not constant_diagonal_matrix<NestedObject> or real_axis_number<constant_diagonal_coefficient<NestedObject>>) and
    ...
    : OpenKalman::internal::AdapterBase<HermitianAdapter<NestedObject, storage_triangle>, NestedObject>
  {
    ...
    static_assert([]{if constexpr (constant_matrix<NestedObject>) return real_axis_number<constant_coefficient<NestedObject>>; else return true; }());
    static_assert([]{if constexpr (constant_diagonal_matrix<NestedObject>) return real_axis_number<constant_diagonal_coefficient<NestedObject>>; else return true; }());
  ...
  public:
...
#ifdef __cpp_concepts
    template<square_shaped<Qualification::depends_on_dynamic_shape> Arg> requires (not hermitian_adapter<Arg>) and (not diagonal_matrix<NestedObject>) and
      std::constructible_from<NestedObject, Arg&&>
#else
...
#ifdef __cpp_concepts
    template<square_shaped<Qualification::depends_on_dynamic_shape> Arg> requires (not hermitian_matrix<Arg>) and diagonal_matrix<NestedObject> and
      requires(Arg&& arg) { NestedObject {diagonal_of(std::forward<Arg>(arg))}; }
#else
    ...
    template<typename ... Args, std::enable_if_t<
      ...
      (std::is_constructible_v<NestedObject,
        untyped_dense_writable_matrix_t<NestedObject, Layout::none, Scalar, static_cast<std::size_t>(
        ...
        (diagonal_matrix<NestedObject> and std::is_constructible_v<NestedObject,
          untyped_dense_writable_matrix_t<NestedObject, Layout::none, Scalar, sizeof...(Args), 1>>)), int> = 0>
#endif
...
#ifdef __cpp_concepts
  template<triangular_matrix M> requires (not hermitian_matrix<M, Qualification::depends_on_dynamic_shape>)
#else
  ...
  template<indexible M> requires
    (not hermitian_matrix<M, Qualification::depends_on_dynamic_shape>) and (not triangular_matrix<M>)
#else
  template<typename M, std::enable_if_t<indexible<M> and
      (not hermitian_matrix<M, Qualification::depends_on_dynamic_shape>) and (not triangular_matrix<M>), int> = 0>
#endif
