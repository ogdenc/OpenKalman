...
using TM22 = TypedMatrix<Axes<2>, Axes<2>, M22>;
using TM23 = TypedMatrix<Axes<2>, Axes<3>, M23>;
using TM32 = TypedMatrix<Axes<3>, Axes<2>, M32>;
using TM33 = TypedMatrix<Axes<3>, Axes<3>, M33>;
using TM42 = TypedMatrix<Axes<4>, Axes<2>, M42>;
...
TEST_F(typed_matrix_tests, EuclideanMean_class)
{
  ...
  // Move constructor
  Mat23 mat23c(std::move(Mat23 {9, 8, 7, 6, 5, 4, 3, 2, 1}));
  EXPECT_TRUE(is_near(mat23c, TM33 {9, 8, 7, 6, 5, 4, 3, 2, 1}));

  // Convert from different covariance types
  EuclideanMean<Axes<2>, M23> mat23_x1(TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6});
  ...
  // Move assignment
  mat23c = std::move(Mat23 {3, 4, 5, 6, 7, 8, 9, 10, 11});
  EXPECT_TRUE(is_near(mat23c, Mat23 {3, 4, 5, 6, 7, 8, 9, 10, 11}));

  // assign from different covariance types
  mat23_x1 = TypedMatrix<Axes<2>, Axes<3>, M23> {6, 5, 4, 3, 2, 1};
...
TEST_F(typed_matrix_tests, EuclideanMean_deduction_guides)
{
  ...
  auto b2 = TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6};
...
TEST_F(typed_matrix_tests, EuclideanMean_make_functions)
{
...
TEST_F(typed_matrix_tests, EuclideanMean_traits)
{
  static_assert(is_typed_matrix_v<Mat23>);
  static_assert(not is_mean_v<Mat23>);
  static_assert(is_Euclidean_mean_v<Mat23>);
  static_assert(is_Euclidean_transformed_v<Mat23>);
  static_assert(not is_Euclidean_transformed_v<EuclideanMean<Axes<2>, I22>>);
...
TEST_F(typed_matrix_tests, EuclideanMean_overloads)
{
  ...
  for (int i=0; i<100; i++)
  {
    m = (m * i + randomize<Mat23>(0.0, 0.3)) / (i + 1);
  }
...
TEST_F(typed_matrix_tests, EuclideanMean_blocks)
{
  EXPECT_TRUE(is_near(concatenate_vertical(Mat22 {1, 2, 3, 4, 5, 6}, Mat12 {7, 8}), TM42 {1, 2, 3, 4, 5, 6, 7, 8}));
  static_assert(is_Euclidean_mean_v<decltype(concatenate_vertical(Mat22 {1, 2, 3, 4, 5, 6}, Mat12 {7, 8}))>);
...
TEST_F(typed_matrix_tests, EuclideanMean_arithmetic)
{
  ...
  EXPECT_TRUE(is_near(EuclideanMean<Axes<3>, M32> {7, 6, 5, 4, 3, 2} + TypedMatrix<Axes<3>, Axes<2>, M32> {1, 2, 3, 4, 5, 6}, TM32 {8, 8, 8, 8, 8, 8}));
  static_assert(is_Euclidean_mean_v<decltype(Mat32 {7, 6, 5, 4, 3, 2, 1, 0} + Mat32 {1, 2, 3, 4, 5, 6, 7, 8}, Mat32 {8, 8, 8, 8, 8, 8, 8, 8})>);
  static_assert(is_Euclidean_mean_v<decltype(EuclideanMean<Axes<3>, M32> {7, 6, 5, 4, 3, 2} + Mean<Axes<3>, M32> {1, 2, 3, 4, 5, 6})>);
  static_assert(is_Euclidean_mean_v<decltype(EuclideanMean<Axes<3>, M32> {7, 6, 5, 4, 3, 2} + TypedMatrix<Axes<3>, Axes<2>, M32> {1, 2, 3, 4, 5, 6})>);
  ...
  static_assert(is_Euclidean_mean_v<decltype(Mat32 {1, 2, 3, 4, 5, 6, 7, 8} * 2, Mat32 {2, 4, 6, 8, 10, 12, 14, 16})>);
  static_assert(is_Euclidean_mean_v<decltype(2 * Mat32 {1, 2, 3, 4, 5, 6, 7, 8}, Mat32 {2, 4, 6, 8, 10, 12, 14, 16})>);
  static_assert(is_Euclidean_mean_v<decltype(Mat32 {2, 4, 6, 8, 10, 12, 14, 16} / 2, Mat32 {1, 2, 3, 4, 5, 6, 7, 8})>);
  ...
  static_assert(is_Euclidean_mean_v<decltype(Mat12a {1, 2, 3, 4} * EuclideanMean<Axes<2>, M23> {1, 2, 3, 4, 5, 6})>);

  EXPECT_TRUE(is_near(Mat12a {1, 2, 3, 4} * TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6}, TM23 {9, 12, 15, 19, 26, 33}));
  static_assert(is_equivalent_v<typename MatrixTraits<decltype(Mat12a {1, 2, 3, 4} * TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6})>::RowCoefficients, Angle>);
  static_assert(is_equivalent_v<typename MatrixTraits<decltype(Mat12a {1, 2, 3, 4} * TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6})>::ColumnCoefficients, Axes<3>>);
  static_assert(is_Euclidean_mean_v<decltype(Mat12a {1, 2, 3, 4} * TypedMatrix<Axes<2>, Axes<3>, M23> {1, 2, 3, 4, 5, 6})>);
...
TEST_F(typed_matrix_tests, EuclideanMean_references_axis)
{
  Mat23 v1 {1., 2, 3,
        2, 4, -6,
        3, 6, -3};
  ...
  EuclideanMean<C2, M33&&> v3 = std::move(v2);
  EXPECT_EQ(v3(1,0), 4.1);
  EuclideanMean<C2, const M33&> v4 = v3;
  v3(2,1) = 7.3;
  EXPECT_EQ(v4(2,1), 7.3);
  EuclideanMean<C2, M33> v5 = v3;
  v3(1,1) = 8.4;
...
TEST_F(typed_matrix_tests, Polar_Spherical_toEuclideanExpr)
{
...
TEST_F(typed_matrix_tests, Polar_Spherical_fromEuclideanExpr)
{
  using P1 = Polar<Distance, Angle>;
  const Mean<P1> m1 {2, M_PI / 6};
  const auto x1 = make_EuclideanMean<P1>(2, std::sqrt(3) / 2, 0.5);
