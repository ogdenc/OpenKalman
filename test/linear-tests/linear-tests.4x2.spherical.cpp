...
TEST_F(linear_tests, Linear4x2SphericalW03)
{
    const auto [g, A] = linear_function<double, C4, C2, NoiseType::additive>();
    const SphericalSimplex<GaussianDistribution, double, C4> s {0.001, 2, 0.3};
    construct_linear_test(s, g, A, 50);
}

TEST_F(linear_tests, Linear4x2SphericalW07)
{
    const auto [g, A] = linear_function<double, C4, C2, NoiseType::additive>();
    const SphericalSimplex<GaussianDistribution, double, C4> s {0.001, 2, 0.7};
    construct_linear_test(s, g, A, 50);
}
