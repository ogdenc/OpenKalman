...
TEST_F(linear_tests, Linear2x2Linear_sqrt) {
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> dist {10};
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> noise;
    for (int i=1; i<50; i++) {
        const Mean<Scalar, C2, 2> A = Eigen::Matrix<Scalar, 2, 2>::Random() / i;
        const LinearTransformation<Scalar, C2, C2, NoiseType::additive> g {A};
        const LinearTransform<SquareRootGaussianDistribution, Scalar, C2, C2, NoiseType::additive, SquareRootGaussianDistribution<Scalar, C2>> t {g};
        run_linear_tests(t, g, dist(), A, noise());
    }
}

TEST_F(linear_tests, Linear2x2TT1_sqrt) {
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> dist {10};
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> noise;
    for (int i=1; i<50; i++) {
        const Mean<Scalar, C2, 2> A = Eigen::Matrix<Scalar, 2, 2>::Random() / i;
        ...
        const LinearizedTransform<SquareRootGaussianDistribution, Scalar, C2, C2, NoiseType::additive, SquareRootGaussianDistribution<Scalar, C2>> t {g};
        run_linear_tests(t, g, dist(), A, noise());
    }
}

TEST_F(linear_tests, Linear2x2TT2_sqrt) {
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> dist {10};
    IndependentNoise<SquareRootGaussianDistribution, Scalar, C2> noise;
    for (int i=1; i<50; i++) {
        const Mean<Scalar, C2, 2> A = Eigen::Matrix<Scalar, 2, 2>::Random() / i;
