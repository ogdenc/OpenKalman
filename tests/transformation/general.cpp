...
using A_int2 = Matrix<Axes<2>, Axes<2>, Eigen::Matrix<int, 2, 2>>;
...
TEST_F(transformations, Scale_no_noise_2D)
{
  Transformation<Scale<7>> t;
  EXPECT_EQ(t(M_int2 {2, 3}), M_int2(14, 21));
}

TEST_F(transformations, Scale_additive)
{
  Transformation<Scale<7>> t;
  EXPECT_EQ(t(M_int2 {2, 3}), (M_int2 {14, 21}));
...
TEST_F(transformations, Scale_augmented)
{
  Transformation<Scale<7, 3>> t;
  EXPECT_EQ(t(M_int2 {2, 3}, M_int2 {1, 1}), M_int2(17, 24));
}

TEST_F(transformations, Mult_additive_axis)
{
  using M = Mean<Axes<2>>;
